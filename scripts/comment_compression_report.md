# Card.js Comment Compression Analysis - Complete Mapping

## Summary

**File:** `c:\Users\ppavl\OneDrive\Active projects\mofacts\mofacts\client\views\experiment\card.js`
**Total Multi-Line Comment Blocks:** 139 blocks (2+ consecutive comment lines)
**Analysis Date:** 2025-11-12
**Analysis Method:** Automated pattern recognition + manual semantic review

## Compression Strategy

Each multi-line comment block was analyzed for semantic meaning and compressed to a single line that preserves the essential purpose. The compression approach:

1. **Pattern-Based Recognition:** Identified common patterns (FIX, STATE MACHINE, PERF, SECURITY, etc.)
2. **Semantic Extraction:** Extracted the PURPOSE rather than just concatenating text
3. **Context Preservation:** Kept critical details like layer numbers, timing values, specific examples
4. **Consistent Prefixes:** Used standard prefixes (FIX:, PERF:, STATE MACHINE:, CRITICAL:, etc.)

## Complete Mapping (Blocks 21-139)

Below is the complete JSON mapping for blocks 21-139 (blocks 1-20 were previously analyzed):
...
- **Total blocks analyzed:** 139
- **Blocks 21-139:** 119 blocks
- **Average compression ratio:** ~65% reduction in lines
- **Categories identified:**
  - FIX/Bug fixes: 28 blocks
  - STATE MACHINE: 9 blocks
  - PERF/Optimization: 3 blocks
  - SECURITY: 1 block
  - CRITICAL: 4 blocks
  - Browser compat: 2 blocks
  - Configuration: 8 blocks
  - Logic explanations: 45 blocks
  - TODO/NOTE: 3 blocks
  - Commented out code: 4 blocks

## Key Findings

### Most Common Patterns

1. **Race Condition Fixes** - Multiple blocks describe fixes for timing issues between TTS, SR, countdown timers
2. **State Machine Transitions** - Detailed explanations of trial state flows (PRESENTING→AWAITING→FEEDBACK→TRANSITION)
3. **Performance Optimizations** - Caching strategies for answer grammar and phonetic indexes (saves 100+ms per trial)
4. **UI Flicker Fixes** - Multiple fixes for visual glitches during fade transitions and DOM updates
5. **Defense in Depth** - Several "Layer X of Y defense" patterns for SR state management

### Semantic Categories

**Critical Path Logic:**
- Trial state machine flows
- TTS/SR coordination
- Timeout management
- Fade-in/fade-out transitions

**Bug Fixes:**
- Race conditions (TTS bleeding, SR timeout conflicts)
- Visual flicker (input fields, images)
- Memory leaks (stale intervals, orphaned audio)

**Performance:**
- Answer grammar caching per unit
- Phonetic index caching
- Batch DOM updates in requestAnimationFrame

**Security:**
- XSS prevention via DOM createElement
- Input sanitization

**Browser Compatibility:**
- mediaDevices polyfills
- getUserMedia fallbacks

## Recommendations

1. **Apply Compression Systematically:**
   - Use provided JSON mapping for automated refactoring
   - Preserve semantic meaning - don't just concatenate
   - Keep critical details (layer numbers, timing values, examples)

2. **Consider Further Refactoring:**
   - Extract state machine logic to separate module
   - Move performance optimizations to dedicated cache manager
   - Consolidate race condition fixes into coordinated timing system

3. **Documentation Improvements:**
   - Add JSDoc for complex functions
   - Create state machine diagram (visual)
   - Document the 3-layer defense pattern for SR

4. **Code Quality:**
   - Many blocks explain "what NOT to do" - consider extracting these as guard functions
   - TODO items should be tracked in issue tracker
   - Commented out code should be removed (use git history)

## Files Generated

1. **comment_compression_final.json** - Complete mapping for blocks 21-139
2. **comment_compression_mapping.json** - Full mapping for all 139 blocks (automated output)
3. **comment_compression_report.md** - This document
4. **analyze_comments.js** - Node.js script used for analysis

## Usage

To apply these compressions programmatically:

```javascript
const fs = require('fs');
const mapping = JSON.parse(fs.readFileSync('comment_compression_final.json', 'utf-8'));

// For each block, replace lines startLine through endLine with compressed version
mapping.forEach(block => {
  // Replace lines block.startLine to block.endLine with block.compressed
});
```

## Next Steps

1. Review compressed comments for accuracy
2. Apply compression to card.js
3. Run tests to ensure no functionality broken
4. Consider applying same analysis to other large files (methods.js, unitEngine.js)
5. Update CLAUDE.md with compression results
