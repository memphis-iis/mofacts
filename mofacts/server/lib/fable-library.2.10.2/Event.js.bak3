...
function filter(predicate, sourceEvent) {
  return choose(x => predicate(x) ? (0, _Option.some)(x) : undefined, sourceEvent);
}
...
function split(splitter, sourceEvent) {
  return [choose(v => (0, _Option.tryValueIfChoice1Of2)(splitter(v)), sourceEvent), choose(v => (0, _Option.tryValueIfChoice2Of2)(splitter(v)), sourceEvent)];
}
