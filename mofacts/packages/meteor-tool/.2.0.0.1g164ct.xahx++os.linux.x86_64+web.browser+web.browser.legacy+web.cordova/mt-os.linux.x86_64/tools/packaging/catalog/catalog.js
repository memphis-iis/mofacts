...
catalog.Refresh = {}; // Refresh strategy: once at program start
...
catalog.Refresh.OnceAtStart.prototype.beforeCommand = function () {
  ...
  if (!catalog.refreshOrWarn(self.options)) {
    if (self.options.ignoreErrors) {
      ...
      Console.error("This command requires an up-to-date package catalog.  Exiting."); // Avoid circular dependency.
...
catalog.refreshOrWarn = function (options) {
  ...
  try {
    ...
    if (err.errorType !== 'DDP.ConnectionError') throw err; // XXX is throwing correct for SQLite errors too? probably.

    Console.warn("Unable to update package catalog (are you offline?)");
    Console.warn();
    Console.warn("If you are using Meteor behind a proxy, set HTTP_PROXY and HTTPS_PROXY environment variables or see this page for more details: ", Console.url("https://github.com/meteor/meteor/wiki/Using-Meteor-behind-a-proxy")); // XXX: Make this Console.debug(err)
...
catalog.runAndRetryWithRefreshIfHelpful = function (attempt) {
  ...
  var canRetry = !(catalog.triedToRefreshRecently || catalog.official.offline); // Run `attempt` in a nested buildmessage context.
  ...
  try {
    ...
    if (err.errorType !== 'DDP.ConnectionError') throw err; // First place the previous errors in the capture.

    buildmessage.mergeMessagesIntoCurrentJob(messages); // Then put an error representing this DDP error.

    buildmessage.enterJob("refreshing package catalog to resolve previous errors", function () {
...
_.extend(LayeredCatalog.prototype, {
  ...
  getSortedVersionRecords: function () {
    for (var _len4 = arguments.length, args = new Array(_len4), _key4 = 0; _key4 < _len4; _key4++) {
      args[_key4] = arguments[_key4];
    }

    return this._returnFirst("getSortedVersionRecords", args, ACCEPT_NON_EMPTY);
  },
...
exports.official = remoteCatalog.official; // This is the catalog that's used to actually drive the constraint solver: it
