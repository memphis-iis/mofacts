module.export({
  updateMeteorToolSymlink: () => updateMeteorToolSymlink
});
...
exports.tryToDownloadUpdate = function (options) {
  options = options || {}; // Don't run more than one check simultaneously. It should be
...
var maybeShowBanners = function () {
  ...
  if (banner) {
    var bannerDate = banner.lastUpdated ? new Date(banner.lastUpdated) : new Date();
  ...
  const catalogUtils = require('./catalog/catalog-utils.js'); // Didn't print a banner? Maybe we have a patch release to recommend.
  ...
  if (patchReleaseVersion) {
    var patchRelease = catalog.official.getReleaseVersion(track, patchReleaseVersion);

    if (patchRelease && patchRelease.recommended) {
      var patchKey = "patchrelease-" + track + "-" + patchReleaseVersion;

      if (shouldShow(patchKey)) {
        runLog.log("=> A patch (" + catalogUtils.displayRelease(track, patchReleaseVersion) + ") for your current release is available!");
  ...
  var futureReleases = catalog.official.getSortedRecommendedReleaseVersions(track, currentReleaseOrderKey);

  if (futureReleases.length) {
    var futureReleaseKey = "futurerelease-" + track + "-" + futureReleases[0];

    if (shouldShow(futureReleaseKey)) {
      runLog.log("=> " + catalogUtils.displayRelease(track, futureReleases[0]) + " is available. Update this project with 'meteor update'.");
    }
...
function updateMeteorToolSymlink(printErrors) {
  ...
  var latestReleaseVersion = catalog.official.getDefaultReleaseVersion(); // Maybe you're on some random track with nothing recommended. That's OK.

  if (!latestReleaseVersion) {
    return;
  }

  var latestRelease = catalog.official.getReleaseVersion(latestReleaseVersion.track, latestReleaseVersion.version);
  ...
  var relativeToolPath = tropohouse.default.packagePath(latestReleaseToolPackage, latestReleaseToolVersion, true);
  ...
  if (!localLatestReleaseLink.startsWith(relativeToolPath + files.pathSep)) {
    ...
    var packageMap = packageMapModule.PackageMap.fromReleaseVersion(latestRelease);
    ...
    toolIsopack.initFromPath(latestReleaseToolPackage, tropohouse.default.packagePath(latestReleaseToolPackage, latestReleaseToolVersion));
    var toolRecord = null;
    ...
    tropohouse.default.linkToLatestMeteor(files.pathJoin(relativeToolPath, toolRecord.path, 'meteor'));
  }
}
