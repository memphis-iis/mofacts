...
function reportMissingOption(name) {
  throw new Error("must provide options.".concat(name, " when creating SourceArch"));
}
/**
 * SourceArch
 *
...
class SourceArch {
  constructor(pkg, _ref) {
    ...
    this.pkg = pkg; // Kind of this sourceArchitecture. At the moment, there are really
    // three options -- package, plugin, and app. We use these in linking.

    this.kind = kind; // The architecture (fully or partially qualified) that can use this
    // unibuild.

    this.arch = arch; // Absolute path of the root directory of this package or application.

    this.sourceRoot = sourceRoot; // Packages used. The ordering is significant only for determining
    ...
    this.uses = uses; // Packages which are "implied" by using this package. If a unibuild X
    ...
    this.implies = implies; // A function that returns the source files for this
    ...
    this.getFiles = getFiles; // Object whose keys are relative paths of local node_modules
    ...
    this.localNodeModulesDirs = Object.create(null); // Symbols that this architecture should export. List of symbols (as
    // strings).

    this.declaredExports = declaredExports; // Files and directories that we want to monitor for changes in
...
//# sourceMappingURL=source-arch.js.map