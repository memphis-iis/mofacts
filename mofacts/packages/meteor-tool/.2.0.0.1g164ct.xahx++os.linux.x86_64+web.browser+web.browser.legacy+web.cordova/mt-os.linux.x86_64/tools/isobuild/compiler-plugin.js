!function (module1) {
  ...
  module1.export({
    CompilerPluginProcessor: () => CompilerPluginProcessor,
    PackageSourceBatch: () => PackageSourceBatch
  });
  let assert;
  module1.link("assert", {
    default(v) {
  ...
  let WatchSet, sha1, readAndWatchFileWithHash;
  module1.link("../fs/watch", {
    ...
    readAndWatchFileWithHash(v) {
      readAndWatchFileWithHash = v;
    }

  }, 1);
  let LRU;
  module1.link("lru-cache", {
    default(v) {
      LRU = v;
    }
  ...
  let optimisticStatOrNull, optimisticReadJsonOrNull, optimisticHashOrNull;
  ...
  const hasOwn = Object.prototype.hasOwnProperty; // This file implements the new compiler plugins added in Meteor 1.2, which are
  ...
  function populateServerLibPackages() {
    ...
    const nodeModulesPath = files.pathJoin(devBundlePath, "server-lib", "node_modules");
  ...
  class CompilerPluginProcessor {
    ...
    runCompilerPlugins() {
      const self = this;
      buildmessage.assertInJob(); // plugin id -> {sourceProcessor, resourceSlots}
      ...
      var sourceBatches = _.map(self.unibuilds, function (unibuild) {
        ...
        const sourceRoot = name && self.isopackCache.getSourceRoot(name, arch) || self.sourceRoot;
      ...
      _.each(sourceBatches, function (sourceBatch) {
        _.each(sourceBatch.resourceSlots, function (resourceSlot) {
          var sourceProcessor = resourceSlot.sourceProcessor; // Skip non-sources.
      ...
      _.each(sourceProcessorsWithSlots, function (data, id) {
        ...
        Profile.time("plugin " + sourceProcessor.isopack.name, () => {
          buildmessage.enterJob({
            ...
            const markedMethod = buildmessage.markBoundary(sourceProcessor.userPlugin.processFilesForTarget, sourceProcessor.userPlugin);
  ...
  class InputFile extends buildPluginModule.InputFile {
    constructor(resourceSlot) {
      super(); // We use underscored attributes here because this is user-visible
      // code and we don't want users to be accessing anything that we don't
      // document.

      this._resourceSlot = resourceSlot; // Map from absolute paths to stat objects (or null if the file does
      // not exist).

      this._statCache = Object.create(null); // Map from control file names (e.g. package.json, .babelrc) to
      // absolute paths, or null to indicate absence.

      this._controlFileCache = Object.create(null); // Map from imported module identifier strings (possibly relative) to
      // fully require.resolve'd module identifiers.

      this._resolveCache = Object.create(null); // Communicate to compiler plugins that methods like addJavaScript
    ...
    getSourceRoot() {
      let tolerant = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    ...
    readAndWatchFileWithHash(path) {
      ...
      return readAndWatchFileWithHash(sourceBatch.unibuild.watchSet, files.convertToOSPath(path));
    }
    ...
    findControlFile(basename) {
      ...
      let dir = files.pathDirname(files.pathJoin(sourceRoot, this.getPathInPackage()));
    ...
    resolve(id, parentPath) {
      parentPath = parentPath || files.pathJoin(this.getSourceRoot(), this.getPathInPackage());
  ...
  class ResourceSlot {
    constructor(unibuildResourceInfo, sourceProcessor, packageSourceBatch) {
      const self = this; // XXX ideally this should be an classy object, but it's not.

      self.inputResource = unibuildResourceInfo; // Everything but JS.

      self.outputResources = []; // JS, which gets linked together at the end.

      self.jsOutputResources = []; // Errors encountered while processing this resource.
      ...
      if (self.inputResource.type === "source") {
        if (sourceProcessor) {// If we have a sourceProcessor, it will handle the adding of the
    ...
    _addDirectlyToJsOutputResources() {
      this.addJavaScript(_objectSpread(_objectSpread({}, this.inputResource.fileOptions || {}), {}, {
    ...
    _isLazy(options, isJavaScript) {
      ...
      if (isJavaScript) {
        if (runningTests) {
          const testModule = this._getOption("testModule", options); // If we set fileOptions.testModule = true in _inferFileOptions,
          ...
          const isEagerTestModule = typeof testModule === "boolean" ? testModule : isTestFilePath(this.inputResource.path);
    ...
    _isBare(options) {
      return !!(this._getOption("bare", options) || // XXX eventually get rid of backwards-compatible "raw" name
    ...
    addStylesheet(options, lazyFinalizer) {
      ...
      if (this.packageSourceBatch.useMeteorInstall && cssResource.lazy) {
        ...
        const jsResource = this.addJavaScript(options, () => {
          const result = {};
          let css = this.packageSourceBatch.processor.minifyCssResource(cssResource);
          ...
          result.data = Buffer.from(cssToCommonJS(css), "utf8"); // The JavaScript module that dynamically loads this CSS should
        ...
        this.addJavaScript(_objectSpread(_objectSpread({}, options), {}, {
          ...
          sourceMap: null,
          data: Buffer.from("// These styles have already been applied to the document.\n", "utf8"),
          lazy: true // If a compiler plugin calls addJavaScript with the same
          // sourcePath, that code should take precedence over this empty
          // stub, so setting .implicit marks the resource as disposable.

        })).implicit = true;

        if (!cssResource.lazy && !Buffer.isBuffer(cssResource.data)) {
    ...
    addJavaScript(options, lazyFinalizer) {
      ...
      if (!this.sourceProcessor && this.inputResource.extension !== "js" && this.inputResource.type !== "js") {
  ...
  class OutputResource {
    constructor(_ref2) {
      ...
      this._finalizerPromise = null; // Share the errors array of the resourceSlot.
      ...
      const servePath = targetPath ? resourceSlot.packageSourceBatch.unibuild.pkg._getServePath(targetPath) : resourceSlot.inputResource.servePath;
      Object.assign(this, {
        type,
    ...
    finalize() {
      if (this._finalizerPromise) {
        ...
        (this._finalizerPromise = // It's important to initialize this._finalizerPromise to the new
    ...
    reportPendingErrors() {
      if (this.hasPendingErrors()) {
        ...
        buildmessage.error(firstError.message, firstError.info);
      }
    ...
    get sourceMap() {
      return this._get("sourceMap");
    }

    set sourceMap(value) {
      return this._set("sourceMap", value);
    ...
    _get(name) {
      ...
      if (!hasOwn.call(this._initialOptions, name)) {
        throw new Error("Unknown JsOutputResource property: ".concat(name));
      }
  ...
  class AssetOutputResource extends OutputResource {
    constructor(params) {
      ...
      this.path = this._initialOptions.path; // Eagerness/laziness should never matter for assets.
  ...
  class PackageSourceBatch {
    constructor(unibuild, processor, _ref3) {
      let {
        sourceRoot,
        linkerCacheDir,
        scannerCacheDir
      ...
      const usesHMRPackage = self.unibuild.pkg.name !== "hot-module-replacement" && self.processor.isopackCache.uses(self.unibuild.pkg, "hot-module-replacement", self.unibuild.arch);
      const supportedArch = self.unibuild.arch === 'web.browser';
      self.hmrAvailable = self.useMeteorInstall && isDevelopment && usesHMRPackage && supportedArch; // These are the options that should be passed as the second argument
    ...
    compileOneJsResource(resource) {
      const slot = this.makeResourceSlot(_objectSpread(_objectSpread({
        type: "source",
        extension: "js"
      }, resource), {}, {
      ...
      if (slot) {
        ...
        if (slot.sourceProcessor) {
          ...
          if (userPlugin) {
            const markedMethod = buildmessage.markBoundary(userPlugin.processFilesForTarget, userPlugin);
    ...
    makeResourceSlot(resource) {
      ...
      if (resource.type === "source") {
        ...
        if (extension === null) {
          ...
          if (!sourceProcessor) {
            buildmessage.error("no plugin found for ".concat(resource.path, " in ") + "".concat(this.unibuild.pkg.displayName(), "; a plugin for ").concat(filename, " ") + "was active when it was published but none is now");
            return null; // recover by ignoring
          }
        } else {
          sourceProcessor = this._getSourceProcessorSet().getByExtension(extension); // If resource.extension === 'js', it's ok for there to be no
          // sourceProcessor, since we #HardcodeJs in ResourceSlot.

          if (!sourceProcessor && extension !== 'js') {
            buildmessage.error("no plugin found for ".concat(resource.path, " in ") + "".concat(this.unibuild.pkg.displayName(), "; a plugin for *.").concat(extension, " ") + "was active when it was published but none is now");
            return null; // recover by ignoring
          }
    ...
    getResolver() {
      ...
      return Resolver.getOrCreate(_objectSpread({
    ...
    _getNodeModulesPaths() {
      if (!this._nodeModulesPaths) {
        ...
        _.each(nmds, (nmd, path) => {
          if (!nmd.local) {
            this._nodeModulesPaths.push(files.convertToOSPath(path.replace(/\/$/g, "")));
          }
        });
      }
    ...
    _getSourceProcessorSet() {
      if (!this._sourceProcessorSet) {
        ...
        this._sourceProcessorSet = new buildPluginModule.SourceProcessorSet(isopack.displayName(), {
          hardcodeJs: true
        });
    ...
    static computeJsOutputFilesMap(sourceBatches) {
      ...
      map.get("modules").files.some(file => {
        ...
        map.forEach((info, name) => {
          if (!name) return;
          const mainModule = info.mainModule && "meteor/".concat(name, "/").concat(info.mainModule.targetPath);
        ...
        file.data = Buffer.from(file.data.toString("utf8") + "\n" + meteorPackageInstalls.join(""), "utf8");
      ...
      const allMissingModules = Object.create(null); // Records the subset of allMissingModules that were successfully
      ...
      sourceBatches.forEach(batch => {
        ...
        if (batch.useMeteorInstall) {
          scanner.scanImports();
          ImportScanner.mergeMissing(allMissingModules, scanner.allMissingModules);
        }
      ...
      function handleMissing(missingModules) {
        ...
        _.each(missingModules, (importInfoList, id) => {
          ...
          ImportScanner.mergeMissing(missingMap.get(name), {
            [id]: importInfoList
          });
        });
      ...
      scannerMap.forEach((scanner, name) => {
        ...
        if (isApp) {
          ...
          outputFiles.forEach(file => {
            ...
            if (nodeModulesIndex === -1 || nodeModulesIndex === 1 && parts[2] === "meteor") {
              ...
              file.meteorInstallOptions = file.meteorInstallOptions || modulesEntry.batch.meteorInstallOptions; // This file is going to be installed in a node_modules
    ...
    static _watchOutputFiles(jsOutputFilesMap) {
      ...
      jsOutputFilesMap.forEach(entry => {
        entry.files.forEach(file => {
          const {
            sourcePath,
            absPath = sourcePath && files.pathJoin(entry.batch.sourceRoot, sourcePath)
          } = file;
          const {
            importScannerWatchSet
          } = entry;

          if (typeof absPath === "string" && // Blindly calling importScannerWatchSet.addFile would be
    ...
    static _warnAboutMissingModules(missingModules) {
      ...
      function maybeWarn(id, info) {
        ...
        if ("./".indexOf(id.charAt(0)) < 0) {
          const packageDir = parts[0].startsWith("@") ? parts[0] + "/" + parts[1] : parts[0];
        ...
        warnings.push("  ".concat(JSON.stringify(id), " in ").concat(info.parentPath, " (").concat(info.bundleArch, ")"));
        return true;
      }
    ...
    _linkJS(jsResources) {
      let onCacheKey = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : () => {};
      const self = this;
      buildmessage.assertInJob();
      var bundleArch = self.processor.arch; // Run the linker.
      ...
      const cacheKey = "".concat(cacheKeyPrefix, "_").concat(cacheKeySuffix);
      ...
      const cacheFilename = self.linkerCacheDir && files.pathJoin(self.linkerCacheDir, cacheKey + '.cache');
      const wildcardCacheFilename = cacheFilename && files.pathJoin(self.linkerCacheDir, cacheKeyPrefix + "_*.cache"); // The return value from _linkJS includes Buffers, but we want everything to
      ...
      const ret = linkedFiles.map(file => {
        const sm = typeof file.sourceMap === 'string' ? JSON.parse(file.sourceMap) : file.sourceMap;
        return {
          type: "js",
  ...
  _.each(["getResources", "_linkJS"], method => {
  ...
  _.each(["computeJsOutputFilesMap"], method => {
