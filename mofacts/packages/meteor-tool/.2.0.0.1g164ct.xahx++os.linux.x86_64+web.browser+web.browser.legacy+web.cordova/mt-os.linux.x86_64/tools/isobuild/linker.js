!function (module1) {
  ...
  let findAssignedGlobals;
  module1.link("./js-analyze.js", {
    findAssignedGlobals(v) {
      findAssignedGlobals = v;
    }
  ...
  // most of the time.
  const CACHE_SIZE = process.env.METEOR_APP_PRELINK_CACHE_SIZE || 1024 * 1024 * 20; // Cache individual files prelinked
  ...
  var Module = function (options) {
    var self = this; // module name or null

    self.name = options.name || null; // The architecture for which this bundle is being linked.

    self.bundleArch = options.bundleArch; // files in the module. array of File
  ...
  _.extend(Module.prototype, {
    ...
    computeAssignedVariables: Profile("linker Module#computeAssignedVariables", function () {
      var self = this; // The assigned variables in the app aren't actually used for anything:
    ...
    getPrelinkedFiles: Profile("linker Module#getPrelinkedFiles", function () {
      var self = this;
      const haveMeteorInstallOptions = self.files.some(file => file.meteorInstallOptions); // If we don't want to create a separate scope for this module,
      // then our job is much simpler. And we can get away with
      // preserving the line numbers.

      if (self.useGlobalNamespace && !haveMeteorInstallOptions) {
        ...
        return _.map(eagerFiles, function (file) {
          ...
          const node = file.getPrelinkedOutput({
            preserveLineNumbers: true
          });
          const results = Profile.time("toStringWithSourceMap (app)", () => {
      ...
      const result = {
        ...
        hash: watch.sha1(JSON.stringify(self.files.map(file => file._inputHash)))
      };
      const results = [result]; // An array of strings and SourceNode objects.

      let chunks = [];
      let fileCount = 0; // Emit each file
    ...
    _buildModuleTrees(results, sourceWidth) {
      ...
      _.each(this.files, file => {
        ...
        if (file.isDynamic()) {
          ...
          if (file.absModuleId.endsWith("/package.json") && file.jsonData) {
            const stub = {};
    ...
    _stringifyInstallOptions(options) {
      ...
      return optionsString.slice(0, optionsString.length - 2) + [",", "  eval: function () {", "    return eval(arguments[0]);", "  }", "}"].join("\n");
    },
    ...
    _chunkifyEagerRequires(chunks, moduleCount, sourceWidth) {
      ...
      assert.ok(_.isNumber(sourceWidth));
      let exportsName; // Now that we have installed everything in this package or
  ...
  function addToTree(value, path, tree) {
    ...
    parts.forEach((part, i) => {
      ...
      tree = _.has(tree, part) ? tree[part] : tree[part] = i < lastIndex ? {} : value;
    });
  }
  ...
  var writeSymbolTree = function (symbolTree, indent) {
    var put = function (node, indent) {
      ...
      var spacing = new Array(indent + 1).join(' '); // XXX prettyprint!
  ...
  function File(inputFile, arch) {
    var self = this; // source code for this file (a string)

    self.source = inputFile.data.toString('utf8'); // hash of source (precalculated for *.js files, calculated here for files
    // produced by plugins)

    self._inputHash = inputFile.hash || watch.sha1(self.source); // The path of the source file, relative to the root directory of the
    // package or application.

    self.sourcePath = inputFile.sourcePath; // Absolute module identifier to use when installing this file via
    // meteorInstall. If the inputFile has no .absModuleId, then this file
    // cannot be installed as a module.

    self.absModuleId = inputFile.absModuleId || null; // the path where this file would prefer to be served if possible
    ...
    self.deps = getNonDynamicDeps(inputFile.deps); // True if the input file should not be evaluated eagerly.
    ...
    self.imported = inputFile.imported; // Boolean indicating whether this file is the main entry point module
    // for its package.

    self.mainModule = !!inputFile.mainModule; // If true, don't wrap this individual file in a closure.

    self.bare = !!inputFile.bare; // A source map (generated by something like CoffeeScript) for the input file.
    // Is an Object, not a string.

    self.sourceMap = inputFile.sourceMap; // If inputFile is a JSON file, its parsed data will be exposed via the
    // .jsonData property.

    self.jsonData = inputFile.jsonData || null; // The arch this file will run in

    self.bundleArch = arch; // Options to pass to meteorInstall when this file is installed.
  ...
  _.extend(File.prototype, {
    ...
    computeAssignedVariables: Profile("linker File#computeAssignedVariables", function () {
      ...
      try {
        ...
        buildmessage.error(e.message, errorOptions); // Recover by pretending that this file is empty (which
    ...
    _getClosureFooter() {
      return this.meteorInstallOptions ? "}" : "}).call(this);\n";
    },
  ...
  const getPrelinkedOutputCached = require("optimism").wrap(function (file, options) {
    ...
    if (result.code) {
      ...
      if (result.map) {
        ...
        chunk = sourcemap.SourceNode.fromStringWithSourceMap(result.code, sourcemapConsumer);
        sourcemapConsumer.destroy();
      }

      chunks.push(chunk); // It's important for the code to end with a newline, so that a
  ...
  function getOutputWithSourceMapCached(file, servePath, options) {
    ...
    const result = file.getPrelinkedOutput(_objectSpread(_objectSpread({}, options), {}, {
      disableCache: true
  ...
  var getHeader = function (options) {
    ...
    const packageVariables = _.filter(options.packageVariables, name => !_.has(options.imports, name));
  ...
  function getImportCode(imports, header, omitVar) {
    ...
    var tree = buildSymbolTree(scratch); // Generate output
  ...
  var fullLink = Profile("linker.fullLink", function (inputFiles, _ref3) {
    let {
    ...
    var prelinkedFiles = module.getPrelinkedFiles(); // If we're in the app, then we just add the import code as its own file in
    // the front.
