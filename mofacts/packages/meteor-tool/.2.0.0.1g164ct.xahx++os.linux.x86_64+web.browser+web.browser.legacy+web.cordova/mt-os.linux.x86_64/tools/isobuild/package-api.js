!function (module1) {
  ...
  let pathRelative, convertToPosixPath;
  ...
  var catalog = require('../packaging/catalog/catalog.js'); // It's important that we import these functions individually instead of
  ...
  function toArchArray(arch) {
    ...
    for (var i = 0; i < arch.length; ++i) {
      ...
      if (!isMatch) {
        buildmessage.error("Invalid 'where' argument: '" + inputArch + "'", // skip toArchArray in addition to the actual API function
        {
          useMyCaller: 1
        });
      }
  ...
  class PackageAPI {
    constructor(options) {
      options = options || {};
      this.buildingIsopackets = !!options.buildingIsopackets; // source files used.
      // It's a multi-level map structured as:
      //   arch -> sources|assets -> relPath -> {relPath, fileOptions}

      this.files = {}; // symbols exported

      this.exports = {}; // packages used and implied (keys are 'package', 'unordered', and
    ...
    use(names, arch, options) {
      var self = this; // Support `api.use(package, {weak: true})` without arch.
      ...
      options = options || {};
      names = toArray(names);
      arch = toArchArray(arch); // A normal dependency creates an ordering constraint and a "if I'm
      ...
      if (options.unordered && options.weak) {
        buildmessage.error("A dependency may not be both unordered and weak.", {
          useMyCaller: true
        }); // recover by ignoring
    ...
    imply(names, arch) {
      var self = this; // We currently disallow build plugins in
      ...
      if (self.debugOnly || self.prodOnly || self.testOnly) {
        buildmessage.error("can't use imply in packages that are debugOnly, prodOnly or testOnly"); // recover by ignoring

        return;
      }

      names = toArray(names);
      arch = toArchArray(arch); // using for loop rather than underscore to help with useMyCaller
      ...
      for (var i = 0; i < names.length; ++i) {
        ...
        if (compiler.isIsobuildFeaturePackage(parsed.package)) {
          buildmessage.error("to declare that your package requires the build tool feature " + "'{parsed.package}', use 'api.use', not 'api.imply'"); // recover by ignoring
    ...
    mainModule(path, arch) {
      let fileOptions = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
      ...
      forAllMatchingArchs(arch, a => {
        ...
        const source = {
          ...
          fileOptions: _objectSpread(_objectSpread({}, fileOptions), {}, {
            mainModule: true
          })
        };
        ...
        if (filesForArch.sources.some(old => source.relPath === old.relPath)) {
          errors.push("Duplicate api.mainModule: ".concat(path));
        }
    ...
    _forbidExportWithLazyMain(arch) {
      ...
      if (filesForArch.main && filesForArch.main.fileOptions.lazy && this.exports[arch].length > 0) {
        buildmessage.error("Architecture " + JSON.stringify(arch) + " cannot both " + "export symbols and have a lazy main module");
      }
    }
    /**
     * @memberOf PackageAPI
    ...
    addAssets(paths, arch) {
      if (!arch) {
        buildmessage.error('addAssets requires a second argument specifying ' + 'where the asset should be available. For example: "client", ' + '"server", or ["client", "server"].', {
          useMyCaller: true
        });
        return;
      } // Watch out - we rely on the levels of stack traces inside this
    ...
    _addFiles(type, paths, arch, fileOptions) {
      if (type !== "sources" && type !== "assets") {
        throw new Error("Can only handle sources and assets, not '".concat(type, "'."));
      }

      var self = this;
      paths = toArray(paths);
      arch = toArchArray(arch); // Convert Dos-style paths to Unix-style paths.
      ...
      _.each(paths, function (path) {
        forAllMatchingArchs(arch, function (a) {
          const filesOfType = self.files[a][type]; // Check if we have already added a file at this path
          ...
          if (filesOfType.some(source => source.relPath === path)) {
            ...
            errors.push("Duplicate ".concat(typeName, " file: ").concat(path));
            return;
          }
    ...
    versionsFrom(releases) {
      var self = this; // Packages in isopackets really ought to be in the core release, by
      ...
      if (self.buildingIsopackets) {
        buildmessage.error("packages in isopackets may not use versionsFrom"); // recover by ignoring

        return;
      }

      releases = toArray(releases); // using for loop rather than underscore to help with useMyCaller

      for (var i = 0; i < releases.length; ++i) {
        var release = releases[i]; // If you don't specify a track, use our default.
        ...
        var releaseRecord = catalog.official.getReleaseVersion(relInf[0], relInf[1]);
    ...
    "export"(symbols, arch, options) {
      var self = this; // Support `api.export("FooTest", {testOnly: true})` without
      // arch.
...
//# sourceMappingURL=package-api.js.map