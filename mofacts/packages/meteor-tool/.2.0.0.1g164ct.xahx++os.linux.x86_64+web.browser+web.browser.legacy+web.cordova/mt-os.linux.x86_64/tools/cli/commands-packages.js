...
main.registerCommand({
  ...
  if (options['from-checkout']) {
    ...
    if (config.getPackageServerFilePrefix() !== 'packages') {
      ...
      try {
        Console.info("Pushing git tag (this should fail if you are not from MDG)");
...
main.registerCommand({
  ...
  if (options.args.length === 0) {
    ...
    if (options["all-packages"]) {
      upgradePackageNames = _.filter(_.keys(projectContext.packageMapFile.getCachedVersions()), packageName => !compiler.isIsobuildFeaturePackage(packageName));
    }
  ...
  if (!options.args.length) {
    ...
    if (nonlatestIndirectDeps.length) {
      ...
      Console.info(["These versions may not be compatible with your project.", "To update one or more of these packages to their latest", "compatible versions, pass their names to `meteor update`,", "or just run `meteor update --all-packages`."].join("\n"));
    }
  }
}); ///////////////////////////////////////////////////////////////////////////////
// admin run-upgrader
...
main.registerCommand({
  ...
  _.each(constraintsToAdd, function (constraint) {
    ...
    Console.info(constraint.package + (versionRecord.description ? ": " + versionRecord.description : ""));
  });
