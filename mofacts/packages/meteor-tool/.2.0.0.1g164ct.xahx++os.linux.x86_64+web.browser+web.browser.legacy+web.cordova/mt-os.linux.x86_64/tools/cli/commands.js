...
function parseServerOptionsForRunCommand(options, runTargets) {
  const parsedServerUrl = parsePortOption(options.port); // XXX COMPAT WITH 0.9.2.2 -- the 'mobile-port' option is deprecated
...
const AVAILABLE_SKELETONS = ["apollo", "bare", "blaze", "full", "minimal", DEFAULT_SKELETON, "typescript", "vue"];
main.registerCommand({
  name: 'create',
  maxArgs: 1,
  options: {
      ...
      for (platform of cordovaPlatforms) {
        buildmessage.enterJob({
          ...
          if (platform === 'ios') {
            ...
            const apkPath = files.pathJoin(buildPath, 'build/outputs/apk', options.debug ? 'android-debug.apk' : 'android-release-unsigned.apk');
...
function doTestCommand(options) {
  ...
  return runTestAppForPackages(projectContext, _.extend(options, {
    mobileServerUrl: utils.formatUrl(parsedMobileServerUrl),
    cordovaServerPort: parsedCordovaServerPort,
    proxyPort: parsedServerUrl.port,
    proxyHost: parsedServerUrl.hostname
