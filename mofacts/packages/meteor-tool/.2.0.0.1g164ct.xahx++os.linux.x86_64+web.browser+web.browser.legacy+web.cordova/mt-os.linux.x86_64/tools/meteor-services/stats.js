...
var Console = require('../console/console.js').Console; // The name of the package that you add to your app to opt out of
// sending stats.


var OPT_OUT_PACKAGE_NAME = "package-stats-opt-out"; // Return a list of packages used by this app, both directly and
...
var recordPackages = function (options) {
  ...
  if (_.findWhere(packages, {
    name: OPT_OUT_PACKAGE_NAME
  })) {
  ...
  var appIdentifier = options.projectContext.appIdentifier; // We do this inside a new fiber to avoid blocking anything on talking
  ...
  Fiber(function () {
    ...
    try {
      ...
      if (auth.isLoggedIn()) {
        try {
          auth.loginWithTokenOrOAuth(conn, accountsConfiguration, config.getPackageStatsServerUrl(), config.getPackageStatsServerDomain(), "package-stats-server");
        } catch (err) {// Do nothing. If we can't log in, we should continue and report
          // stats anonymously.
          //
      ...
      var result = conn.call("recordAppPackages", appIdentifier, packages, details); // If the stats server sent us a new session, save it for use on
      // subsequent requests.
      ...
      logErrorIfInCheckout(err); // Do nothing. A failure to record package stats shouldn't be
...
var logErrorIfInCheckout = function (err) {
  if (Console.isInteractive() && files.inCheckout() || process.env.METEOR_PACKAGE_STATS_TEST_OUTPUT) {
    Console.warn("Failed to record package usage.");
    Console.warn("(This error is hidden when you are not running Meteor from a", "checkout.)");
...
var getPackagesForAppIdInTest = function (projectContext) {
  ...
  try {
    result = conn.call("getPackagesForAppId", projectContext.appIdentifier);
...
var connectToPackagesStatsServer = function () {
  return new ServiceConnection(config.getPackageStatsServerUrl(), {
