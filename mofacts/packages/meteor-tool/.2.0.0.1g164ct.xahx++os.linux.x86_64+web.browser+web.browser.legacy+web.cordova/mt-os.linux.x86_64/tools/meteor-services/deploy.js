!function (module1) {
  ...
  function pollForDeploymentSuccess(versionId, deployPollTimeout, result, site) {
    return Promise.asyncApply(() => {
      ...
      const deploymentPollResult = Promise.await(pollForDeploy(pollingState, versionId, site));
  ...
  function pollForDeploy(pollingState, versionId, site) {
    return Promise.asyncApply(() => {
      const {
        deadline,
        initialWaitTimeMs,
      ...
      if (versionStatusResult && versionStatusResult.payload && versionStatusResult.payload.message) {
        ...
        if (pollingState.errors >= pollingState.maxErrors) {
          ...
          return Promise.await(pollForDeploy(pollingState, versionId, site));
        }
      }
  ...
  function bundleAndDeploy(options) {
    return Promise.asyncApply(() => {
      ...
      var promptIfAuthFails = loggedInUsername() !== null; // Check auth up front, rather than after the (potentially lengthy)
      // bundling process.
      ...
      if (options.waitForDeploy && result.payload.newVersionId) {
        ...
        return Promise.await(pollForDeploymentSuccess(result.payload.newVersionId, options.deployPollingTimeoutMs, result, site));
      }
