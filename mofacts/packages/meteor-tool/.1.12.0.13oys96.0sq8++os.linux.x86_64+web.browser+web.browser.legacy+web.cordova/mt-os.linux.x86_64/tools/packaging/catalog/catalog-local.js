...
module.link("../../isobuild/compiler.js", {
...
var LocalCatalog = function (options) {
  var self = this;
  options = options || {}; // Package server data.  Maps package name to a {packageSource, packageRecord,
  // versionRecord} object.

  self.packages = {};
  self.initialized = false; // Local directories to search for package source trees

  self.localPackageSearchDirs = null; // Package source trees added explicitly through a directory (not through a
  ...
  self.explicitlyAddedLocalPackageDirs = []; // All packages found either by localPackageSearchDirs or
  ...
  self.effectiveLocalPackageDirs = []; // A WatchSet that detects when the set of packages and their locations
...
_.extend(LocalCatalog.prototype, {
  toString: function () {
    var self = this;
    return "LocalCatalog [localPackageSearchDirs=" + JSON.stringify(self.localPackageSearchDirs) + "]";
  },
  ...
  initialize(options) {
    ...
    options = options || {};
    const addPatternsToList = Profile("addPatternsToList", (patterns, list) => {
      if (!patterns) {
        return;
      }
      ...
      patterns.forEach(pattern => {
        ...
        glob(pattern).forEach(p => list.push(files.pathResolve(p)));
      });
    });
    addPatternsToList(options.localPackageSearchDirs, self.localPackageSearchDirs = []);
    addPatternsToList(options.explicitlyAddedLocalPackageDirs, self.explicitlyAddedLocalPackageDirs = []);
  ...
  _requireInitialized: function () {
    var self = this;
    if (!self.initialized) throw new Error("catalog not initialized yet?");
  },
  ...
  getAllNonTestPackageNames: function () {
    ...
    var ret = [];
    const nonCoreDir = files.pathJoin(files.getCurrentToolsDir(), "packages", "non-core") + files.pathSep;

    _.each(self.packages, function (_ref, name) {
      let {
        packageSource: {
          sourceRoot
        },
      ...
      if (!includeNonCore && sourceRoot.startsWith(nonCoreDir)) {
        return;
      }
  ...
  getPackage: function (name, options) {
    ...
    options = options || {};
    if (!_.has(self.packages, name)) return null;
  ...
  getSortedVersionRecords: function (name) {
    ...
    if (_.has(KNOWN_ISOBUILD_FEATURE_PACKAGES, name)) {
      return KNOWN_ISOBUILD_FEATURE_PACKAGES[name].map(version => ({
        version,
        packageName: name,
        dependencies: {}
      }));
  ...
  getVersion: function (name, version) {
    ...
    if (versionRecord.version !== version) return null;
  ...
  _computeEffectiveLocalPackages() {
    ...
    buildmessage.enterJob("looking for packages", function () {
      _.each(self.explicitlyAddedLocalPackageDirs, explicitDir => {
        ...
        if (packageJsHash) {
          self.packageLocationWatchSet.addFile(packageJsPath, packageJsHash);
      ...
      _.each(self.localPackageSearchDirs, function (searchDir) {
        var possiblePackageDirs = watch.readAndWatchDirectory(self.packageLocationWatchSet, {
          absPath: searchDir,
          include: [/\/$/]
        }); // Not a directory? Ignore.

        if (possiblePackageDirs === null) return;
        ...
        _.each(possiblePackageDirs, function (subdir) {
          ...
          var absPackageDir = files.pathJoin(searchDir, subdir); // Consider a directory to be a package source tree if it contains
          ...
          if (packageJsHash) {
            ...
            // directories.
            self.packageLocationWatchSet.addFile(packageJsPath, packageJsHash); // We don't know the name of the package, so we can't deal with
  ...
  _loadLocalPackages(buildingIsopackets) {
    var self = this;
    buildmessage.assertInCapture(); // Load the package source from a directory. We don't know the names of our
    ...
    var initSourceFromDir = function (packageDir, definiteName) {
      ...
      buildmessage.enterJob({
        ...
        var name = packageSource.name; // We should only have one package dir for each name; in this case, we
...
["initialize", "_computeEffectiveLocalPackages", "_loadLocalPackages"].forEach(function (method) {
  this[method] = Profile("LocalCatalog#" + method, this[method]);
}, LocalCatalog.prototype);
exports.LocalCatalog = LocalCatalog;
//# sourceMappingURL=catalog-local.js.map