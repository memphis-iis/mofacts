...
var Profile = require('../../tool-env/profile').Profile; // XXX: Rationalize these flags.  Maybe use the logger?


var DEBUG_SQL = !!process.env.METEOR_DEBUG_SQL; // Developers using Windows Subsystem for Linux (WSL) may want to override
...
_.extend(Txn.prototype, {
  ...
  // Start a transaction
  begin: function (mode) {
    var self = this; // XXX: Use DEFERRED mode?
...
var Db = function (dbFile, options) {
  ...
  self._retry(function () {
    self._execute("PRAGMA journal_mode=".concat(JOURNAL_MODE));
  });
};
...
_.extend(Db.prototype, {
  ...
  runInTransaction: function (action) {
    ...
    var runOnce = Profile("sqlite query", function () {
      ...
      if (DEBUG_SQL) {
        var t2 = Date.now(); // XXX: Hack around not having loggers
    ...
    for (var attempt = 0;; attempt++) {
      try {
        ...
        var retry = false; // Grr... doesn't expose error code; must string-match

        if (err.message && (err.message === "SQLITE_BUSY: database is locked" || err.message === "SQLITE_BUSY: cannot commit transaction - SQL statements in progress")) {
  ...
  open: function (dbFile) {
    ...
    if (!files.exists(files.pathDirname(dbFile))) {
      ...
      if (!files.mkdir_p(folder)) throw new Error("Could not create folder at " + folder);
    }
  ...
  _execute: function (sql, params) {
    var self = this;
    var prepared = null; // We don't prepare non-parametrized statements, because (a) there's not
...
_.extend(Table.prototype, {
  _buildStatements: function () {
    ...
    var queryParams = self._generateQuestionMarks(self.jsonFields.length + (self.noContentColumn ? 0 : 1));
  ...
  upsert: function (txn, objects) {
    var self = this; // XXX: Use sqlite upsert
  ...
  createTable: function (txn) {
    ...
    sql += ")";
    txn.execute(sql); //sql = "CREATE INDEX IF NOT EXISTS idx_" + self.name + "_id ON " + self.name + "(_id)";
    //txn.execute(sql);
  }
...
var RemoteCatalog = function () {
  var self = this; // Set this to true if we are not going to connect to the remote package
...
_.extend(RemoteCatalog.prototype, {
  ...
  getVersion: function (packageName, version) {
    var result = this._contentQuery("SELECT content FROM versions WHERE packageName=? AND version=?", [packageName, version]);

    return filterExactRows(result, {
      packageName,
      version
    });
  },
  ...
  getSortedVersions: function (name) {
    ...
    var match = this._columnsQuery("SELECT version FROM versions WHERE packageName=?", name);
  ...
  getSortedVersionRecords: function (name) {
    ...
    var versionRecords = this._contentQuery("SELECT content FROM versions WHERE packageName=?", [name]);

    if (!versionRecords) return [];

    var pvParse = _.memoize(VersionParser.parse);

    versionRecords.sort(function (a, b) {
      return VersionParser.compare(pvParse(a.version), pvParse(b.version));
    });
  ...
  getPackage: function (name, options) {
    var result = this._contentQuery("SELECT content FROM packages WHERE name=?", name);
  ...
  getAllBuilds: function (name, version) {
    var result = this._contentQuery("SELECT content FROM builds WHERE builds.versionId = " + "(SELECT _id FROM versions WHERE versions.packageName=? AND " + "versions.version=?)", [name, version]);
  ...
  getReleaseTrack: function (name) {
    ...
    var result = self._contentQuery("SELECT content FROM releaseTracks WHERE name=?", name);
  ...
  getReleaseVersion: function (track, version) {
    ...
    var result = self._contentQuery("SELECT content FROM releaseVersions WHERE track=? AND version=?", [track, version]);
  ...
  forceRecommendRelease: function (track, version) {
    ...
    if (!releaseVersionData) {
      throw Error("Can't force-recommend unknown release " + track + "@" + version);
    }
  ...
  getAllReleaseTracks: function () {
    return _.pluck(this._columnsQuery("SELECT name FROM releaseTracks"), 'name');
  },
  ...
  initialize: function (options) {
    var self = this;
    options = options || {}; // We should to figure out if we are intending to connect to the package server.
    ...
    self.tableBannersShown = new Table('bannersShown', ['_id', 'lastShown'], {
      noContentColumn: true
    });
    self.allTables = [self.tableVersions, self.tableBuilds, self.tableReleaseTracks, self.tableReleaseVersions, self.tablePackages, self.tableSyncToken, self.tableMetadata, self.tableBannersShown];
    return self.db.runInTransaction(function (txn) {
      ...
      txn.execute("CREATE INDEX IF NOT EXISTS versionsIdx ON " + "versions(packageName, version)");
      txn.execute("CREATE INDEX IF NOT EXISTS buildsVersionsIdx ON " + "builds(versionId)");
      txn.execute("CREATE INDEX IF NOT EXISTS packagesIdx ON " + "packages(name)");
      txn.execute("CREATE INDEX IF NOT EXISTS releaseTracksIdx ON " + "releaseTracks(name)");
      txn.execute("CREATE INDEX IF NOT EXISTS releaseVersionsIdx ON " + "releaseVersions(track, version)");
    });
  },
  ...
  refresh: function (options) {
    ...
    var updateResult = {}; // XXX This buildmessage.enterJob only exists for showing progress.
  ...
  getSortedRecommendedReleaseVersions: function (track, laterThanOrderKey) {
    var self = this;
    var versions = self.getSortedRecommendedReleaseRecords(track, laterThanOrderKey);
  ...
  getSortedRecommendedReleaseRecords: function (track, laterThanOrderKey) {
    var self = this; // XXX releaseVersions content objects are kinda big; if we put
    // 'recommended' and 'orderKey' in their own columns this could be faster

    var result = self._contentQuery("SELECT content FROM releaseVersions WHERE track=?", track);

    var recommended = _.filter(result, function (v) {
      if (!v.recommended) return false;
  ...
  getNumReleaseVersions: function (track) {
    ...
    var result = self._columnsQuery("SELECT count(*) FROM releaseVersions WHERE track=?", track);
  ...
  getDefaultReleaseVersion: function (track) {
    ...
    if (!versionRecord) throw new Error("Can't get default release version for track " + track);
  ...
  getDefaultReleaseVersionRecord: function (track) {
    var self = this;
    if (!track) track = exports.DEFAULT_TRACK;
  ...
  getBuildWithPreciseBuildArchitectures: function (versionRecord, buildArchitectures) {
    ...
    var matchingBuilds = this._contentQuery("SELECT content FROM builds WHERE versionId=?", versionRecord._id);

    return _.findWhere(matchingBuilds, {
  ...
  getSyncToken: function () {
    ...
    var result = self._contentQuery("SELECT content FROM syncToken WHERE _id=?", [SYNCTOKEN_ID]);
...
exports.RemoteCatalog = RemoteCatalog; // We put this constant here because we don't have any better place that would otherwise cause a cycle

exports.DEFAULT_TRACK = 'METEOR'; // The catalog as provided by troposhere (aka atomospherejs.com)
