let pluginVersionsFromStarManifest;
...
let CordovaBuilder;
module.link("../cordova/builder.js", {
  CordovaBuilder(v) {
    CordovaBuilder = v;
  }

}, 1);
let closeAllWatchers;
...
const hasOwn = Object.prototype.hasOwnProperty; // Parse out s as if it were a bash command line.
  ...
  _computeEnvironment: function () {
    ...
    if (self.settings) {
      ...
      if (env.METEOR_SETTINGS) {
        runLog.log("WARNING: The 'METEOR_SETTINGS' environment variable is ignored " + "when running in development (as you are doing now).  Instead, use " + "the '--settings settings.json' option to see reactive changes " + "when settings are changed.  For more information, see the " + "documentation for 'Meteor.settings': " + "https://docs.meteor.com/api/core.html#Meteor-settings" + "\n");
    ...
    env.METEOR_AUTO_RESTART = self.autoRestart; // We run the server behind our own proxy, so we need to increment
    // the HTTP forwarded count.

    env.HTTP_FORWARDED_COUNT = "" + ((parseInt(process.env['HTTP_FORWARDED_COUNT']) || 0) + 1);

    if (self.inspect && self.inspect.break) {
    ...
    files.mkdir_p(shellDir); // We need to convert to OS path here because the running app doesn't
    // have access to path translation functions

    env.METEOR_SHELL_DIR = files.convertToOSPath(shellDir);
    env.METEOR_PARENT_PID = process.env.METEOR_BAD_PARENT_PID_FOR_TEST ? "foobar" : process.pid;
    env.METEOR_PRINT_ON_LISTEN = 'true';
    return env;
  },
  // Spawn the server process and return the handle from child_process.spawn.
  _spawn: function () {
    var self = this; // Path conversions

    var entryPoint = files.convertToOSPath(files.pathJoin(self.bundlePath, 'main.js')); // Setting options
    ...
    opts.push(entryPoint); // Call node

    var child_process = require('child_process'); // setup the 'ipc' pipe if further communication between app and proxy is
    // expected
...
var AppRunner = function (options) {
  var self = this;
  self.projectContext = options.projectContext; // note: run-all.js updates port directly
  ...
  self.watchForChanges = options.watchForChanges === undefined ? true : options.watchForChanges;
  self.onRunEnd = options.onRunEnd;
  self.noRestartBanner = options.noRestartBanner;
  self.recordPackageUsage = options.recordPackageUsage === undefined ? true : options.recordPackageUsage;
  self.omitPackageMapDeltaDisplayOnFirstRun = options.omitPackageMapDeltaDisplayOnFirstRun;
  self.fiber = null;
  self.startPromise = null;
  self.runPromise = null;
  self.exitPromise = null;
  self.watchPromise = null;
  self._promiseResolvers = {}; // If this promise is set with self.makeBeforeStartPromise, then for the first
  // run, we will wait on it just before self.appProcess.start() is called.

  self._beforeStartPromise = null; // Builders saved across rebuilds, so that targets can be re-written in
...
_.extend(AppRunner.prototype, {
  ...
  _runOnce: function (options) {
    ...
    runLog.clearLog();
    self.proxy.setMode("hold"); // Bundle up the app

    var bundlePath = self.projectContext.getProjectLocalDirectory('build'); // Cache the server target because the server will not change inside
    ...
    var bundleApp = function () {
      if (!firstRun) {
        ...
        // little while ago.
        catalog.triedToRefreshRecently = false; // If this isn't the first time we've run, we need to reset the project
    ...
    firstRun = false; // Read the settings file, if any
    ...
    serverWatchSet.merge(settingsWatchSet); // We only can refresh the client without restarting the server if the
    // client contains the 'autoupdate' package.

    var canRefreshClient = self.projectContext.packageMap && self.projectContext.packageMap.getInfo('autoupdate');
    ...
    var listenPromise = self._makePromise("listen"); // Run the program
    ...
    var setupClientWatcher = function () {
      ...
      clientWatcher = new watch.Watcher({
        ...
        onChange: function () {
          ...
          var outcome = watch.isUpToDate(serverWatchSet, false) ? 'changed-refreshable' // only a client asset has changed
    ...
    function pauseClient(arch) {
      return appProcess.proc.sendMessage("webapp-pause-client", {
        arch
      });
    }
    ...
    function refreshClient(arch) {
      return Promise.asyncApply(() => {
        if (typeof arch === "string") {
          ...
          Promise.await(appProcess.proc.sendMessage("webapp-reload-client", {
            arch
          }));
        ...
        Promise.await(appProcess.proc.sendMessage("client-refresh"));
      });
    }
    ...
    const postStartupResult = Promise.race([listenPromise, runPromise]).then(() => {
      return runPostStartupCallbacks(bundleResult);
    }).await();
    if (postStartupResult) return postStartupResult; // Wait for either the process to exit, or (if watchForChanges) a
    ...
    try {
      while (ret.outcome === 'changed-refreshable') {
        ...
        refreshClient(); // Establish a watcher on the new files.

        setupClientWatcher();
        const postStartupResult = runPostStartupCallbacks(bundleResult);
        if (postStartupResult) return postStartupResult; // Wait until another file changes.
  ...
  _fiber: function () {
    ...
    while (true) {
      ...
      if (runResult.outcome === "wrong-release" || runResult.outcome === "conflicting-versions") {
        ...
        if (self.watchForChanges) {
          runLog.log("Your application has errors. " + "Waiting for file change.", {
            arrow: true
          });
          Console.enableProgressDisplay(false);
        }
        ...
        } else {// explanation should already have been logged
        }

        if (self.watchForChanges) {
          runLog.log("Your application is crashing. " + "Waiting for file change.", {
            arrow: true
          });
          Console.enableProgressDisplay(false);
        }
      ...
      if (self.watchForChanges) {
        ...
        self.proxy.setMode("errorpage"); // If onChange wasn't called synchronously (clearing watchPromise), wait
        // on it.

        self.watchPromise && self.watchPromise.await(); // While we were waiting, did somebody stop() us?
    ...
    closeAllWatchers(); // Giving up for good.
