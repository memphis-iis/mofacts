...
var debugBuild = !!process.env.METEOR_DEBUG_BUILD; // A job is something like "building package foo". It contains the set
...
var Job = function (options) {
  var self = this;
  self.messages = []; // Should be something like "building package 'foo'"
  // Should look good in "While $title:\n[messages]"

  self.title = options.title;
  self.rootPath = options.rootPath; // Array of Job (jobs created inside this job)
...
function capture(options, f) {
  ...
  const resetFns = [currentProgress.set(progress), currentMessageSet.set(messageSet)];
  let job = null;
...
function enterJob(options, f) {
  ...
  var progress;
  {
    var progressOptions = {}; // XXX: Just pass all the options?
  ...
  const resetFns = [currentProgress.set(progress)];
  ...
  if ('useMyCaller' in info) {
    if (info.useMyCaller) {
      const {
        insideFiber,
        outsideFiber
      } = parseStack.parse(new Error()); // Concatenate and get rid of lines about Future and buildmessage
...
var exception = function (error) {
  if (!currentJob.get()) {
    ...
    if (error instanceof files.FancySyntaxError) {
      error = new Error("Syntax error: " + error.message + " at " + error.file + ":" + error.line + ":" + error.column);
    }
  ...
  if (error instanceof files.FancySyntaxError) {
    ...
    var parsed = parseStack.parse(error); // If there is a part inside the fiber, that's the one we want. Otherwise,
    // use the one outside.
...
var forkJoin = function (options, iterable, fn) {
  ...
  return enterJobAsync(options).then(() => {
    ...
    let results = _.map(iterable, function () {
      ...
      }).then(() => fn(...args)) // Collect any errors thrown (and later re-throw the first one),
