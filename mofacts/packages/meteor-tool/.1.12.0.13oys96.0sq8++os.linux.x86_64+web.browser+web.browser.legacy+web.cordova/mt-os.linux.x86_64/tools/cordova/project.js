...
let cordova_lib, cordova_events, CordovaError;
module.link("cordova-lib", {
  cordova(v) {
    cordova_lib = v;
  },
...
let cordova_util;
module.link("cordova-lib/src/cordova/util.js", {
  default(v) {
    cordova_util = v;
  }

}, 13);
let PluginInfoProvider;
module.link("cordova-common/src/PluginInfo/PluginInfoProvider.js", {
  default(v) {
    PluginInfoProvider = v;
  }

}, 14);
let CORDOVA_PLATFORMS, CORDOVA_PLATFORM_VERSIONS, displayNameForPlatform, displayNamesForPlatforms, newPluginId, convertPluginVersions, convertToGitUrl;
...
function log() {
  ...
  Console.rawInfo("%% ".concat(util.format.apply(null, args), "\n"));
...
const pinnedPlatformVersions = CORDOVA_PLATFORM_VERSIONS; // We pin plugin versions to make sure we do not install versions that are
...
const getCommandOptionsForPlugin = function (plugin) {
  let commandOptions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  const cli_variables = commandOptions && commandOptions.cli_variables && commandOptions.cli_variables[plugin] || {};
  return _objectSpread(_objectSpread({}, commandOptions), {}, {
    cli_variables
  });
};
...
class CordovaProject {
  constructor(projectContext) {
    ...
    this.buildJsonPath = files.convertToOSPath(files.pathJoin(this.projectRoot, 'build.json'));
    this.createIfNeeded();
  }

  createIfNeeded() {
    buildmessage.assertInJob(); // Check if we have an existing Cordova project directory with outdated
    ...
    if (files.exists(this.projectRoot)) {
      ...
      const outdated = _.some(pinnedPlatformVersions, (pinnedVersion, platform) => {
        ...
        const installedVersion = this.installedVersionForPlatform(platform); // If we cannot establish the installed version, we consider it outdated
    ...
    if (!files.exists(this.projectRoot)) {
      ...
      const templatePath = files.mkdtemp('cordova-template-'); // If we don't create an empty hooks directory, cordova-lib will attempt
      // to install one from a hardcoded path to cordova-app-hello-world

      files.mkdir_p(files.pathJoin(templatePath, 'hooks')); // If we don't create an empty www directory, cordova-lib will get
      // confused
      ...
      builder.writeConfigXmlAndCopyResources(false); // Create the Cordova project root directory
      ...
      this.runCommands('creating Cordova project', () => Promise.asyncApply(() => {
        // No need to pass in appName and appId because these are set from
        // the generated config.xml
        Promise.await(cordova_lib.create(files.convertToOSPath(this.projectRoot), undefined, undefined, config));
      }), undefined, null);
    }
  ...
  writeBuildJson() {
    ...
    const iosCommonOptions = {
      ...
      buildFlag: ["-UseModernBuildSystem=0", ...(Console.verbose ? [] : ["-quiet"])]
    };
    files.writeFile(this.buildJsonPath, JSON.stringify({
      ios: {
  ...
  prepareFromAppBundle(bundlePath, pluginVersions) {
    ...
    this.ensurePluginsAreSynchronized(pluginVersions, builder.pluginsConfiguration); // Temporary workaround for Cordova iOS bug until
    // https://issues.apache.org/jira/browse/CB-10885 is fixed

    const iosBuildExtrasPath = files.pathJoin(this.projectRoot, 'platforms/ios/cordova/build-extras.xcconfig');
  ...
  prepareForPlatform(platform) {
    assert(platform); // Temporary workaround for Cordova iOS bug until
    ...
    const commandOptions = _objectSpread(_objectSpread({}, this.defaultOptions), {}, {
      platforms: [platform]
    });

    this.runCommands("preparing Cordova project for platform ".concat(displayNameForPlatform(platform)), () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.prepare(commandOptions));
    }));
  ...
  buildForPlatform(platform) {
    let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let extraPaths = arguments.length > 2 ? arguments[2] : undefined;
    assert(platform);
    ...
    this.runCommands("building Cordova app for platform ".concat(displayNameForPlatform(platform)), () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.build(commandOptions));
    }));
  } // Running


  run(platform, isDevice) {
    return Promise.asyncApply(() => {
      let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
      let extraPaths = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
      ...
      this.runCommands("running Cordova app for platform ".concat(displayNameForPlatform(platform), " with options ").concat(options), () => Promise.asyncApply(() => {
        Promise.await(cordova_lib.run(commandOptions));
      }));
    });
  } // Platforms
...
on an OS X system.");
    ...
    if (!inProject) {
      Console.warn("Please add the ".concat(displayNameForPlatform(platform), " platform to your project first."));
      Console.info("Run: ".concat(Console.command("meteor add-platform ".concat(platform))));
      return false;
    }

    const allRequirements = this.runCommands("checking Cordova requirements for platform ".concat(displayNameForPlatform(platform)), () => Promise.asyncApply(() => {
      return Promise.await(cordova_lib.requirements([platform], this.defaultOptions));
    }));
    let requirements = allRequirements && allRequirements[platform];

    if (!requirements) {
      Console.error("Failed to check requirements for platform ".concat(displayNameForPlatform(platform)));
      return false;
    } else if (requirements instanceof CordovaError) {
      Console.error("Cordova error: ".concat(requirements.message));
      return false;
    ...
    if (!satisfied) {
      Console.info();
      Console.info("Your system does not yet seem to fulfill all requirements to build apps for ".concat(displayNameForPlatform(platform), "."));
      Console.info();
      ...
      for (const requirement of requirements) {
        ...
        if (requirement.installed) {
          ...
          if (reason) {
            Console.failInfo("".concat(name, ": ").concat(reason));
          } else {
  ...
  installedVersionForPlatform(platform) {
    const command = files.convertToOSPath(files.pathJoin(this.projectRoot, 'platforms', platform, 'cordova', 'version')); // Make sure the command exists before trying to execute it

    if (files.exists(command)) {
      return this.runCommands("getting installed version for platform ".concat(platform, " in Cordova project"), execFileSync(command, {
  ...
  updatePlatforms() {
    let platforms = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.listInstalledPlatforms();
    this.runCommands("updating Cordova project for platforms ".concat(displayNamesForPlatforms(platforms)), () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.platform('update', platforms, this.defaultOptions));
    }));
  }

  addPlatform(platform) {
    this.runCommands("adding platform ".concat(displayNameForPlatform(platform), " to Cordova project"), () => Promise.asyncApply(() => {
      let version = pinnedPlatformVersions[platform];
      let platformSpec = version ? "".concat(platform, "@").concat(version) : platform;
      Promise.await(cordova_lib.platform('add', platformSpec, this.defaultOptions));
    }));
  }

  removePlatform(platform) {
    this.runCommands("removing platform ".concat(displayNameForPlatform(platform), " from Cordova project"), () => Promise.asyncApply(() => {
      Promise.await(cordova_lib.platform('rm', platform, this.defaultOptions));
    }));
  }
  ...
  ensurePlatformsAreSynchronized() {
    let platforms = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.cordovaPlatformsInApp;
  ...
  listInstalledPluginVersions() {
    let usePluginInfoId = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    const pluginInfoProvider = new PluginInfoProvider();
    const installedPluginVersions = pluginInfoProvider.getAllWithinSearchPath(files.convertToOSPath(this.pluginsDir));
  ...
  listFetchedPluginVersions() {
    ...
    const fetchedPluginsMetadata = JSON.parse(files.readFile(fetchJsonPath, 'utf8'));
    return _.object(_.map(fetchedPluginsMetadata, (metadata, name) => {
      ...
      const id = "".concat(scoped ? '@' : '').concat(idWithVersion.split('@')[scoped ? 1 : 0]);
      let version;

      if (source.type === 'registry') {
        version = idWithVersion.split('@')[scoped ? 2 : 1];
      } else if (source.type === 'git') {
        version = "".concat(source.url).concat('ref' in source ? "#".concat(source.ref) : '');
      } else if (source.type === 'local') {
        version = "file://".concat(source.path);
      }
  ...
  targetForPlugin(id, version) {
    ...
    if (utils.isUrlWithSha(version)) {
      return usePluginName ? convertToGitUrl(version) : "".concat(id, "@").concat(convertToGitUrl(version));
    } else if (utils.isUrlWithFileScheme(version)) {
      // Strip file:// and resolve the path relative to the cordova-build
      // directory
      const pluginPath = this.resolveLocalPluginPath(version); // We need to check if the directory exists ourselves because Cordova
      ...
      if (!(stat && stat.isDirectory())) {
        buildmessage.error("Couldn't find local directory '".concat(files.convertToOSPath(pluginPath), "' (while attempting to install plugin ").concat(id, ")."));
        return null;
      }

      return files.convertToOSPath(pluginPath);
    } else {
      return "".concat(id, "@").concat(version);
    }
  ...
  addPlugin(id, version) {
    ...
    if (target) {
      const commandOptions = _.extend(this.defaultOptions, {
        ...
        link: utils.isUrlWithFileScheme(version)
      });

      try {
        this.runCommands("adding plugin ".concat(target, " to Cordova project"), cordova_lib.plugin.bind(undefined, 'add', [target], commandOptions));
      } catch (error) {
        if (retry && utils.isUrlWithSha(version)) {
          Console.warn("Cordova plugin add for ".concat(id, " failed with plugin id \n          in the URL with hash, retrying now with plugin name. If this works you\n          can ignore the error above or you can update your plugin declaration\n          to use the id from config.xml instead of the name from package.json"));
          this.addPlugin(id, version, config, _objectSpread(_objectSpread({}, options), {}, {
            usePluginName: true,
            retry: false
          }));
          return;
  ...
  removePlugins(plugins) {
    ...
    plugins.forEach(plugin => {
      const commandOptionsPlugin = getCommandOptionsForPlugin(plugin, commandOptions);
      this.runCommands("removing plugin ".concat(plugin, "   from Cordova project"), cordova_lib.plugin.bind(undefined, 'rm --force', [plugin], commandOptionsPlugin));
    });
  } // Ensures that the Cordova plugins are synchronized with the app-level
  // plugins.


  ensurePluginsAreSynchronized(pluginVersions) {
    let pluginsConfiguration = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    assert(pluginVersions);
    buildmessage.assertInCapture();
    buildmessage.enterJob({
      ...
      // SHA reference.
      pluginVersions = convertPluginVersions(pluginVersions); // To ensure we do not attempt to install plugin versions incompatible
      ...
      pluginsConfiguration = _.object(_.map(pluginsConfiguration, (config, id) => {
        ...
        if (newId) {
          Console.warn();
          Console.labelWarn("Cordova plugin ".concat(id, " has been renamed to ").concat(newId, " as part of moving to npm. Please change the App.configurePlugin call in mobile-config.js accordingly."));
          return [newId, config];
        } else {
          return [id, config];
        }
      }));
      const installedPluginVersions = convertPluginVersions(this.listInstalledPluginVersions()); // Due to the dependency structure of Cordova plugins, it is impossible to
      ...
      let shouldReinstallAllPlugins = false; // Iterate through all of the plugins and find if any of them have a new
      ...
      _.each(pluginVersions, (version, id) => {
        ...
        if (isPluginFromLocalPath) {
          ...
          if (!_.has(installedPluginVersions, id) || installedPluginVersions[id] !== version) {
            ...
            Console.debug("Plugin ".concat(id, " version have changed or it was added, will perform cordova plugins reinstall"));
          }
        }
      });

      const installedPluginsByName = Object.keys(this.listInstalledPluginVersions(true)); // Check to see if we have any installed plugins that are not in the
      ...
      if (!shouldReinstallAllPlugins) {
        ...
        const androidJson = files.exists(androidJsonPath) ? JSON.parse(files.readFile(androidJsonPath, 'utf8')) : {
          installed_plugins: {}
        };
        const iosJson = files.exists(iosJsonPath) ? JSON.parse(files.readFile(iosJsonPath, 'utf8')) : {
          installed_plugins: {}
        };

        let previouslyInstalledPlugins = _.union(Object.keys(androidJson.installed_plugins), Object.keys(iosJson.installed_plugins)); // Now the problem is we have a list of names the plugins (name defined in the plugin.xml)
        ...
        previouslyInstalledPlugins.forEach(id => {
          if (!_.has(pluginVersions, id)) {
            Console.debug("Plugin ".concat(id, " was removed, will perform cordova plugins reinstall"));
            shouldReinstallAllPlugins = true;
          }
        });
      }
      ...
      if (shouldReinstallAllPlugins || !_.isEmpty(pluginsFromLocalPath)) {
        ...
        let pluginVersionsToInstall; // Now install the necessary plugins.
  ...
  ensurePluginsWereInstalled(requiredPlugins, pluginsConfiguration, retryInstall) {
    ...
    Object.keys(requiredPlugins).filter(plugin => {
      if (!installedPluginsNames.includes(plugin)) {
        Console.debug("Plugin ".concat(plugin, " was not installed."));

        if (retryInstall) {
          Console.debug("Retrying to install ".concat(plugin, "."));
          this.addPlugin(plugin, requiredPlugins[plugin], pluginsConfiguration[plugin]);
        }
    ...
    if (retryInstall) {
      ...
      throw new Error("Some Cordova plugins installation failed: (".concat(Object.keys(missingPlugins).join(', '), ")."));
    }
  }
  ...
  ensurePinnedPluginVersions(pluginVersions) {
    ...
    _.each(pluginVersions, (version, id) => {
      ...
      if (pinnedVersion && semver.lt(version, pinnedVersion)) {
        Console.labelWarn("Attempting to install plugin ".concat(id, "@").concat(version, ", but it should have a minimum version of ").concat(pinnedVersion, " to ensure compatibility with the current platform versions. Installing the minimum version for convenience, but you should adjust your dependencies."));
  ...
  defaultEnvWithPathsAdded() {
    let paths = this.defaultPaths || [];
    paths.unshift(...arguments);
    const env = files.currentEnvWithPathsAdded(...paths);
    return env;
  }

  get defaultPaths() {
    const nodeBinDir = files.getCurrentNodeBinDir(); // Add the ios-sim bin path so Cordova can find it

    const iosSimBinPath = files.pathJoin(files.getDevBundle(), 'lib/node_modules/ios-sim/bin');
    return [nodeBinDir, iosSimBinPath];
  }

  runCommands(title, promiseOrAsyncFunction) {
    let env = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : this.defaultEnvWithPathsAdded();
    let cwd = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : this.projectRoot;
    ...
    try {
      ...
      Console.error();
      const consoleOptions = Console.options({
        indent: 3
      });
      Console.error("While ".concat(title, ":"), consoleOptions);
      ...
      if (error instanceof CordovaError) {
        ...
        const errorMessage = Console.verbose ? error.stack || error.message : error.message;
        Console.error("Cordova error: ".concat(errorMessage), consoleOptions);
...
["prepareFromAppBundle", "prepareForPlatform", "buildForPlatform"].forEach(name => {
  CPp[name] = Profile(platform => {
    const prefix = "CordovaProject#".concat(name);
    return name.endsWith("ForPlatform") ? "".concat(prefix, " for ").concat(displayNameForPlatform(platform)) : prefix;
  }, CPp[name]);
});
