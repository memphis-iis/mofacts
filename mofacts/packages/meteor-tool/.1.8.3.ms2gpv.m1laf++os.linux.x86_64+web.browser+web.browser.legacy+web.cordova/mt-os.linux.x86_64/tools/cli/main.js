module.export({
  meteorCommandsJsonPath: () => meteorCommandsJsonPath,
  dumpMeteorCommands: () => dumpMeteorCommands
});
let isEmacs;
...
require('./flush-buffers-on-exit-in-windows.js'); // node (v8) defaults to only recording 10 lines of stack trace. This
...
function Command(options) {
  ...
  _.each(this.options, function (value, key) {
    ...
    if (_.has(value, 'default') && _.has(value, 'required')) {
      throw new Error(options.name + ": " + key + " can't be both optional " + "and required");
    }
...
var commands = {}; // Exception to throw from a command to bail out and show command
// usage information.
...
main.SpringboardToLatestRelease = function SpringboardToLatestRelease(track) {
...
main.SpringboardToSpecificRelease = function SpringboardToSpecificRelease(fullReleaseName, msg) {
...
main.registerCommand = function (options, func) {
  ...
  if (!options.catalogRefresh) {
    throw Error("Command does not select a catalogRefresh strategy: " + options.name);
  }
...
const meteorCommandsJsonPath = files.pathJoin(files.getDevBundle(), "bin", ".meteor-commands.json");

function dumpMeteorCommands() {
...
var longHelp = exports.longHelp = function (commandName) {
  ...
  var help = loadHelp(); // can use to see if there is help text for a particular command
  ...
  if (!(node instanceof Command)) {
    ...
    _.each(node, function (n, shortName) {
      var fullName = commandName + (commandName.length > 0 ? " " : "") + shortName; // Use helpDict to only include commands that have help text, otherwise
      // there is nothing to display.
      // For now, there's no way to mark commands with subcommands (eg 'admin')
      // as hidden.

      if (!n.hidden && helpDict[fullName]) {
        commandsWanted[fullName] = {
          name: shortName
        };
      }
    });
    ...
    _.each(help, function (helpEntry) {
      if (_.has(commandsWanted, helpEntry.name)) {
        ...
        commandList += "   " + shortName + new Array(maxNameLength + 1).join(' ').substr(shortName.length) + "   " + helpEntry.body.split('\n')[0] + "\n";
      }
...
var springboard = function (rel, options) {
  ...
  const serverArchitectures = catalog.official.filterArchesWithBuilds(toolsPkg, toolsVersion, archinfo.acceptableMeteorToolArches());
  ...
  if (serverArchitectures.length === 0) {
    ...
    Console.error("This project uses " + rel.getDisplayName() + ", which isn't", "available on this platform. To work with this app on all supported", "platforms, use", Console.command("meteor update --release " + releaseName), "to pin this app to the newest compatible release.");
    process.exit(1);
  }
  ...
  Console.withProgressDisplayVisible(function () {
    ...
    if (messages.hasMessages()) {
      ...
      if (options.fromApp) {
        Console.error("Sorry, this project uses " + rel.getDisplayName() + ", which is", "not installed and could not be downloaded. Please check to make", "sure that you are online.");
      } else {
        Console.error("Sorry, " + rel.getDisplayName() + " is not installed and could not", "be downloaded. Please check to make sure that you are online.");
      }
  ...
  serverArchitectures.some(arch => {
    return toolRecord = _.findWhere(toolIsopack.toolsOnDisk, {
      arch
    });
  });
  ...
  if (files.realpath(newToolsDir) === files.realpath(files.getCurrentToolsDir())) {
  ...
  const executable = files.pathJoin(newToolsDir, "meteor"); // Strip off the "node" and "meteor.js" from argv and replace it with the
  ...
  for (var i = 2; i < argc; ++i) {
    ...
    if (arg === "--unsafe-perm" || arg === "--allow-superuser") {
...
var oldSpringboard = function (toolsVersion) {
  ...
  var cmd = files.pathJoin(warehouse.getToolsDir(toolsVersion), 'bin', 'meteor'); // Release our connection to the sqlite catalog database for the current
  // process, so that the springboarded process can reestablish it.

  catalog.official.closePermanently(); // Now exec; we're not coming back.
...
Fiber(function () {
  ...
  if (require('semver').lt(process.version, MIN_NODE_VERSION)) {
    Console.error('Meteor requires Node ' + MIN_NODE_VERSION + ' or later.');
    process.exit(1);
          ...
          _.each(names, function (name) {
            ...
            if (_.has(isBoolean, name)) {
              if (isBoolean[name] !== optionIsBoolean) {
                throw new Error("conflict: option '" + name + "' is used " + "both as a boolean and as another type for " + "command " + key);
              }
            } else {
  ...
  walkCommands(commands); // This is for things like '--arch' and '--version' which look like
  ...
  for (var i = 0; i < argv.length; i++) {
    var term = argv[i]; // --: stop-parsing marker
    ...
    if (term.match(/^--/) || term.match(/^-.$/)) {
      var value = undefined; // Split the term (once only!) on an equal sign.
    ...
    if (term.match(/^-/)) {
      ...
      for (var j = 1; j < term.length; j++) {
        ...
        if (isBoolean[subterm] === false) {
        ...
        } else if (isBoolean[subterm] && j + 1 < term.length && term.charAt(j + 1) === '=') {
          ...
          rawOptions[subterm].push(false); // Don't process the '=' on the next pass.
  ...
  if (_.has(rawOptions, "--allow-superuser") || _.has(rawOptions, "--unsafe-perm")) {
    process.env.METEOR_ALLOW_SUPERUSER = "true";
  ...
  if (process.getuid && process.getuid() === 0) {
    const allowSuperuser = !!(process.env.METEOR_ALLOW_SUPERUSER && JSON.parse(process.env.METEOR_ALLOW_SUPERUSER));

    if (!allowSuperuser) {
      // Meteor is running as root without METEOR_ALLOW_SUPERUSER, notice and stop.
      Console.error("");
      Console.error("You are attempting to run Meteor as the 'root' superuser.", "If you are developing, this is almost certainly *not* what you want to do and will likely result in incorrect file permissions.", "However, if you are running this command in a build process (CI, etc.), or you are absolutely sure you know what you are doing,", "set the METEOR_ALLOW_SUPERUSER environment variable or pass --allow-superuser to proceed.");
    }

    Console.info("");
    Console.info("Even with METEOR_ALLOW_SUPERUSER or --allow-superuser, permissions in your app directory will be incorrect if you ever attempt to perform any Meteor tasks as a normal user.", "If you need to fix your permissions, run the following command from the root of your project:");
    Console.info("");
  ...
  require('../tool-env/isopackets.js').ensureIsopacketsLoadable(); // Initialize the server catalog. Among other things, this is where we get
  ...
  if (_.has(rawOptions, '--release')) {
    if (rawOptions['--release'].length > 1) {
      Console.error("--release should only be passed once. " + "Try 'meteor help' for help.");
      process.exit(1);
    }
    ...
    if (!releaseOverride) {
      Console.error("The --release option needs a value. " + "Try 'meteor help' for help.");
      process.exit(1);
    }
  ...
  if (appDir) {
    ...
    if (appReleaseFile.noReleaseSpecified()) {
      Console.error("Problem! This project has a .meteor/release file which is empty.", "The file should either contain the release of Meteor that you want", "to use, or the word 'none' if you will only use the project with", "unreleased checkouts of Meteor. Please edit the .meteor/release", "file in the project and change it to a valid Meteor release or", "'none'.");
      process.exit(1);
    } else if (appReleaseFile.fileMissing()) {
      Console.error("Problem! This project does not have a .meteor/release file.", "The file should either contain the release of Meteor that you", "want to use, or the word 'none' if you will only use the project", "with unreleased checkouts of Meteor. Please edit the", ".meteor/release file in the project and change it to a valid Meteor", "release or 'none'.");
      process.exit(1);
    }
  }
  ...
  if (!files.usesWarehouse()) {
    ...
    if (releaseOverride) {
      Console.error("Can't specify a release when running Meteor from a checkout.");
      process.exit(1);
    }
    ...
    if (releaseOverride) {
      ...
      if (appReleaseFile.isCheckout()) {// Looks like we don't have a release. Leave release.current === null.
      } else {
      ...
      if (!releaseName) {
        if (catalog.refreshFailed) {
          Console.error("The package catalog has no information about any Meteor", "releases, and we had trouble connecting to the package server.");
        } else {
          Console.error("The package catalog has no information about", "any Meteor releases.");
        }
  ...
  if (releaseName !== undefined) {
    ...
    if (process.env.METEOR_TEST_FAIL_RELEASE_DOWNLOAD !== 'not-found') {
      ...
      if (!rel) {
        ...
        if (warehouse.realReleaseExistsInWarehouse(releaseName)) {
          var manifest = warehouse.ensureReleaseExistsAndReturnManifest(releaseName);
      ...
      if (!rel && process.platform !== "win32") {
        ...
        try {
          manifest = warehouse.ensureReleaseExistsAndReturnManifest(releaseName);
        } catch (e) {
          // Note: this is WAREHOUSE's NoSuchReleaseError, not RELEASE's
          if (e instanceof warehouse.NoSuchReleaseError) {// pass ...
            ...
            if (!catalog.refreshFailed) {
              ...
              Console.warn();
              Console.warn("If you are using Meteor behind a proxy, set HTTP_PROXY and HTTPS_PROXY environment variables or see this page for more details: ", Console.url("https://github.com/meteor/meteor/wiki/Using-Meteor-behind-a-proxy"));
    ...
    if (!rel) {
      ...
      var displayRelease = catalogUtils.displayRelease(trackAndVersion[0], trackAndVersion[1]); // Now, let's process this.

      if (releaseOverride) {
        if (process.platform === "win32") {
          // Give a good warning if this release exists, but only in the super old
          // warehouse.
          var result = httpHelpers.request("http://warehouse.meteor.com/releases/" + releaseName + ".release.json");

          if (result.response.statusCode === 200) {
            Console.error("Meteor on Windows does not support running any releases", "before Meteor 1.1. Please use a newer release.");
            process.exit(1);
          }
        }
        ...
        if (catalog.refreshFailed) {
          Console.error("This project says that it uses " + displayRelease + ", but", "you don't have that version of Meteor installed, and we were", "unable to contact Meteor's update servers to find out about it.", "Please edit the .meteor/release file in the project and change", "it to a valid Meteor release, or go online.");
        } else {
          Console.error("This project says that it uses " + displayRelease + ", but you", "don't have that version of Meteor installed and the Meteor", "update servers don't have it either. Please edit the", ".meteor/release file in the project and change it to a valid", "Meteor release.");
        }
      } else {
  ...
  if (release.current && release.current.isProperRelease()) {
    if (files.getToolsVersion() !== release.current.getToolsPackageAtVersion()) {
  ...
  var commandName = '';
  var command = null; // Check for a command like '--arch' or '--version'. Make sure
    ...
    _.each(commands['--'] || {}, function (value, key) {
      ...
      if (rawOptions[fullName]) {
        if (rawOptions[fullName].length > 1) {
          Console.error("It doesn't make sense to pass " + fullName + " more than once.");
          process.exit(1);
        }

        if (_.size(rawOptions) > 1 || rawArgs.length !== 0 || command) {
          Console.error("Can't pass anything else along with " + value.name + ".");
          process.exit(1);
        }
  ...
  if (!command) {
    if (rawArgs.length === 0) {
      ...
      for (var i = 0; i < rawArgs.length; i++) {
        var word = rawArgs[i]; // Support "meteor help", "meteor help deploy", "meteor help admin",
        ...
        if (!_.has(walk, word)) {
          Console.error(Console.command("'" + commandName + "'") + " is not a Meteor command. See " + Console.command("'meteor --help'") + ".");
          process.exit(1);
        }
  ...
  if (!command && !showHelp) {
    ...
    Console.error("Try " + Console.command("'meteor " + commandName + " help'") + " " + "for available commands.");
    process.exit(1);
  ...
  _.each(command.options, function (optionInfo, optionName) {
    ...
    var presentShort = _.has(optionInfo, 'short') && _.has(rawOptions, "-" + optionInfo.short);

    var tryHelpMessage = "Try " + Console.command("'meteor help " + commandName + "'") + " " + "for help.";

    if (presentShort && presentLong) {
      // this would get caught below, but give a clearer error message
      Console.error(commandName + ": can't pass both -" + optionInfo.short + " and --" + optionName + ". " + tryHelpMessage);
      process.exit(1);
    }

    var helpfulOptionName = "--" + optionName + (presentShort ? " (-" + optionInfo.short + ")" : ""); // Collect all values we've received for this option, across the
    ...
    if (values.length > 1) {
      ...
      Console.error(Console.command(commandName) + ": can only take one " + Console.command(helpfulOptionName) + " option.");
      ...
      if (value === null) {
        ...
        Console.error(Console.command(commandName) + ": the " + Console.command(helpfulOptionName) + " option needs a value.");
        Console.error(tryHelpMessage);
        process.exit(1);
      } else if (optionInfo.type === Number) {
        if (!value.match(/^[0-9]+$/)) {
          Console.error(Console.command(commandName) + ": " + Console.command(helpfulOptionName) + " must be a number.");
          Console.error(tryHelpMessage);
          process.exit(1);
        }
        ...
        if (!value) {
          Console.error(Console.command(commandName) + ": the " + Console.command(helpfulOptionName) + " " + "option does not need a value.");
          Console.error(tryHelpMessage);
          process.exit(1);
        }

        value = true;
      } else if (optionInfo.type === String) {// nothing to do, 'value' needs no parsing or validation
      } else {
        throw new Error("unknown option type?");
      }

      options[optionName] = value; // Remove from the list of input arguments so that later we can
      ...
      if (_.has(optionInfo, 'default')) {
        ...
        Console.error(Console.command(commandName) + ": the --" + Console.command(optionName) + " option is required.");
  ...
  if (_.keys(rawOptions).length > 0 && !command.allowUnrecognizedOptions) {
    Console.error(Console.command(_.keys(rawOptions)[0]) + ": unknown option.");
    Console.rawError(longHelp(commandName));
    process.exit(1);
  ...
  if (options.args.length < command.minArgs) {
    Console.error(Console.command(commandName) + ": not enough arguments.");
    Console.rawError(longHelp(commandName));
    process.exit(1);
  }

  if (options.args.length > command.maxArgs) {
    Console.error(Console.command(commandName) + ": too many arguments.");
    Console.rawError(longHelp(commandName));
    process.exit(1);
  ...
  if (requiresApp && !options.appDir) {
    ...
    Console.error(Console.command(commandName) + ": You're not in a Meteor project directory.");
    Console.error();
    Console.error("To create a new Meteor project:");
    Console.error(Console.command("meteor create <project name>"), Console.options({
      indent: 2
    }));
    Console.error("For example:");
    Console.error(Console.command("meteor create myapp"), Console.options({
      indent: 2
    }));
    Console.error();
    Console.error("For more help, see " + Console.command("'meteor --help'") + ".");
    process.exit(1);
  ...
  var requiresAppOrPackage = command.evaluateOption('requiresAppOrPackage', options); // Some commands have different results when run from a package dir, but don't
  ...
  if (requiresPackage && !options.packageDir) {
    Console.error(Console.command(commandName) + ": You're not in a Meteor package directory.");
    process.exit(1);
  }

  if (requiresAppOrPackage && !options.packageDir && !options.appDir) {
    Console.error(Console.command(commandName) + ": You're not in a Meteor app directory or a Meteor package directory.");
    process.exit(1);
  }

  if (command.requiresRelease && !release.current) {
    Console.error("You must specify a Meteor version with --release when you work with", "this project. It was created from an unreleased Meteor checkout and", "doesn't have a version associated with it.");
    Console.error("You can permanently set a release for this project with " + Console.command("'meteor update'") + ".");
    process.exit(1);
  }

  if (command.requiresApp && release.current.isCheckout() && appReleaseFile && !appReleaseFile.isCheckout()) {
    ...
    Console.arrowWarn("Running Meteor from a checkout -- overrides project version " + Console.noWrap("(" + appReleaseFile.displayReleaseName + ")"));
  ...
  try {
    ...
    if (!catalog.triedToRefreshRecently && catalogRefreshStrategy.beforeCommand) {
      buildmessage.enterJob({
        title: 'updating package catalog'
      }, function () {
        catalogRefreshStrategy.beforeCommand();
      });
    }

    var ret = Promise.resolve(command.func(options, {
      rawOptions
    })).await();
  } catch (e) {
    Console.enableProgressDisplay(false);

    if (e === main.ShowUsage || e === main.WaitForExit || e === main.SpringboardToLatestRelease || e === main.SpringboardToSpecificRelease || e === main.WaitForExit) {
      throw new Error("you meant 'throw new main.Foo', not 'throw main.Foo'");
      ...
      springboard(latestRelease, {
        releaseOverride: latestRelease.name
      }); // (does not return)
      ...
      springboard(nextRelease, {
        releaseOverride: e.fullReleaseName
      }); // (does not return)
  ...
  Console.enableProgressDisplay(false); // Exit. (We will not get here if the command threw an exception
