...
var maybePrintNoticeHeader = function () {
  if (printedNoticeHeaderThisProcess) return;
  Console.info();
  Console.info("-- Notice --");
  Console.info();
...
var upgradersByName = {
  ...
  "0.9.4-platform-file": function (projectContext) {
    var oldPlatformsPath = files.pathJoin(projectContext.projectDir, ".meteor", "cordova-platforms");
    ...
    var oldPlatforms = _.compact(_.map(files.splitBufferToLines(oldPlatformsFile), files.trimSpaceAndComments)); // This method will automatically add "server" and "browser" and sort, etc.
  ...
  "notices-for-facebook-graph-api-2": function (projectContext) {
    ...
    if (projectContext.packageMap.getInfo('facebook')) {
      maybePrintNoticeHeader();
      Console.info("This version of Meteor now uses version 2.2 of the Facebook API", "for authentication, instead of 1.0. If you use additional Facebook", "API methods beyond login, you may need to request new", "permissions.\n\n", "Facebook will automatically switch all apps to API", "version 2.0 on April 30th, 2015. Please make sure to update your", "application's permissions and API calls by that date.\n\n", "For more details, see", "https://github.com/meteor/meteor/wiki/Facebook-Graph-API-Upgrade", Console.options({
        bulletPoint: "1.0.5: "
      }));
    }
  },
  "1.2.0-standard-minifiers-package": function (projectContext) {
    ...
    projectContext.projectConstraintsFile.addPackages(['standard-minifiers']);
    projectContext.projectConstraintsFile.writeIfModified();
  },
  "1.2.0-meteor-platform-split": function (projectContext) {
    const packagesFile = projectContext.projectConstraintsFile; // meteor-platform is split into a series of smaller umbrella packages
    ...
    if (packagesFile.getConstraint('meteor-platform')) {
      ...
      packagesFile.addPackages([// These packages replace meteor-platform in newly created apps
      'meteor-base', 'mobile-experience', 'mongo', 'blaze-html-templates', 'session', 'jquery', 'tracker', // These packages are not in newly created apps, but were in
      // meteor-platform so we need to add them just in case
      'logging', 'reload', 'random', 'ejson', 'spacebars', 'check']);
  ...
  "1.2.0-cordova-changes": function (projectContext) {
    ...
    files.rm_recursive(projectContext.getProjectLocalDirectory('cordova-build')); // Cordova plugin IDs have changed as part of moving to npm, so we convert
    ...
    maybePrintNoticeHeader(); // Print error messages generated during plugin conversion, if any
  ...
  "1.2.0-breaking-changes": function () {
    maybePrintNoticeHeader();
    Console.info("Meteor 1.2 includes many changes and improvements to the build system, some of which might require small changes to apps and packages. Please read the guide about breaking changes here:", Console.url("https://github.com/meteor/meteor/wiki/Breaking-changes-in-Meteor-1.2"), Console.options({
      bulletPoint: "1.2: "
    }));
  },
  "1.3.0-split-minifiers-package": function (projectContext) {
    const packagesFile = projectContext.projectConstraintsFile; // Minifiers are extracted into a new package called "standard-minifiers"

    if (packagesFile.getConstraint('standard-minifiers')) {
      packagesFile.removePackages(['standard-minifiers']);
      packagesFile.addPackages([// These packages replace meteor-platform in newly created apps
      'standard-minifier-css', 'standard-minifier-js']);
    }

    packagesFile.writeIfModified();
  },
  "1.4.0-remove-old-dev-bundle-link": function (projectContext) {
    const oldDevBundleLink = files.pathJoin(projectContext.projectDir, ".meteor", "dev_bundle");
  ...
  "1.4.3-split-account-service-packages": function (projectContext) {
    maybePrintNoticeHeader();
    Console.info("The account packages for different services (e.g. facebook, twitter, google, etc.) have been split and now make it possible to avoid bundling Blaze in the client bundle for projects not using Blaze. If found in the .meteor/packages file, these packages have been split automatically. Assuming there are no other Blaze dependencies, the \"<service>-config-ui\" package can be removed if there is no need for the Blaze configuration interface.", Console.options({
      bulletPoint: "1.4.3: "
    }));
    const packagesFile = projectContext.projectConstraintsFile; // The transformations to be made here are:
    ...
    const servicesAffected = ['facebook', 'github', 'google', 'meetup', 'meteor-developer', 'twitter', 'weibo'];
    servicesAffected.forEach(service => {
      // If `accounts-<service>`, we just add the configuration UI
      if (packagesFile.getConstraint("accounts-".concat(service))) {
        packagesFile.addPackages(["".concat(service, "-config-ui")]);
      } // If `<service>`, it changes to `<service>-oauth`


      if (packagesFile.getConstraint(service)) {
        packagesFile.removePackages([service]);
        packagesFile.addPackages(["".concat(service, "-oauth")]);
      }
    });
    packagesFile.writeIfModified();
  },
  ...
  '1.7-split-underscore-from-meteor-base': function (projectContext) {
    ...
    if (!packagesFile.getConstraint("underscore") && packagesFile.getConstraint("meteor-base")) {
      maybePrintNoticeHeader();
      Console.info("The underscore package has been removed as a dependency of all packages in meteor-base. Since some apps may have been using underscore through this dependency without having it listed in their .meteor/packages files, it has been added automatically. If your app is not using underscore, then you can safely remove it using 'meteor remove underscore'.", Console.options({
        bulletPoint: "1.7: "
      }));
  ...
  '1.8.3-split-jquery-from-blaze': function (projectContext) {
    ...
    if (!packagesFile.getConstraint("jquery") && packagesFile.getConstraint("blaze-html-templates")) {
      maybePrintNoticeHeader();
      Console.info("The jquery package has become a weak dependency of the blaze package. Since you will most likely need jquery as the commonly used blaze backend and it was not listed in your .meteor/packages files, it has been added automatically. If your app is not using jquery, then you can safely remove it using 'meteor remove jquery'.", Console.options({
        bulletPoint: "1.8.3: "
      }));
...
exports.runUpgrader = function (projectContext, upgraderName) {
  ...
  if (!_.has(upgradersByName, upgraderName)) throw new Error("Unknown upgrader: " + upgraderName);
  upgradersByName[upgraderName](projectContext);
};

exports.upgradersToRun = function (projectContext) {
  var ret = [];
  var finishedUpgraders = projectContext.finishedUpgraders.readUpgraders(); // This relies on the fact that Node guarantees object iteration ordering.
...
//# sourceMappingURL=upgraders.js.map