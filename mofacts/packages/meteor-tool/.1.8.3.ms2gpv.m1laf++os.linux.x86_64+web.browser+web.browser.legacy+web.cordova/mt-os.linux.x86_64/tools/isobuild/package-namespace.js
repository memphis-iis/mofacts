...
let packageVersionParser;
module.link("../packaging/package-version-parser.js", {
  default(v) {
    packageVersionParser = v;
  }
...
class PackageNamespace {
    ...
    each(options, function (value, key) {
      if (key === "summary" || key === "git") {
        ...
        if (typeof value !== "string") {
          buildmessage.error("The package version (specified with " + "Package.describe) must be a string."); // Recover by pretending that version was not set.
        } else {
          var goodVersion = true;

          try {
            var parsedVersion = packageVersionParser.getValidServerVersion(value);
          } catch (e) {
            if (!e.versionParserError) {
              throw e;
            }

            buildmessage.error("The package version " + value + " (specified with Package.describe) " + "is not a valid Meteor package version.\n" + "Valid package versions are semver (see http://semver.org/), " + "optionally followed by '_' and an integer greater or equal to 1.");
            goodVersion = false;
        ...
        if (goodVersion && parsedVersion !== value) {
          buildmessage.error("The package version (specified with Package.describe) may not " + "contain a plus-separated build ID."); // Recover by pretending that the version was not set.
      ...
      } else {// Do nothing. We might want to add some keys later, and we should err
        // on the side of backwards compatibility.
      }

      if (size(compact([source.debugOnly, source.prodOnly, source.testOnly])) > 1) {
        buildmessage.error("Package can't have more than one of: debugOnly, prodOnly, testOnly.");
      }
    });
  }
  /**
  ...
  onUse(f) {
    if (!this._packageSource.isTest) {
      if (this._fileAndDepLoader) {
        buildmessage.error("duplicate onUse handler; a package may have " + "only one", {
          useMyCaller: true
  ...
  onTest(f) {
    const isTest = this._packageSource.isTest; // If we are not initializing the test package, then we are initializing
    ...
    if (isTest) {
      if (this._fileAndDepLoader) {
        buildmessage.error("duplicate onTest handler; a package may have " + "only one", {
          useMyCaller: true
        }); // Recover by ignoring the duplicate
  ...
  registerBuildPlugin(options) {
    const isTest = this._packageSource.isTest; // Tests don't have plugins; plugins initialized in the control file
    ...
    if (options.name in pluginInfo) {
      buildmessage.error("this package already has a plugin named '" + options.name + "'", {
        useMyCaller: true
      }); // recover by ignoring plugin
    ...
    if (options.name.match(/\.\./) || options.name.match(/[\\\/]/)) {
      buildmessage.error("bad plugin name", {
        useMyCaller: true
      }); // recover by ignoring plugin
  ...
  includeTool() {
    ...
    if (!inCheckout()) {
      buildmessage.error("Package.includeTool() can only be used with a " + "checkout of meteor");
