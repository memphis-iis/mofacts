...
class Unibuild {
  constructor(isopack, _ref) {
    let {
      kind,
      // required (main/plugin/app)
      arch,
      // required
      uses,
      implies,
    ...
    this.arch = arch;
    this.uses = uses;
    this.implies = implies || []; // This WatchSet will end up having the watch items from the
    ...
    this.watchSet = watchSet || new WatchSet(); // Each Unibuild is given a unique id when it's loaded (it is not
    ...
    this.id = this.pkg.name + "." + this.kind + "@" + this.arch + "#" + nextBuildId++; // 'declaredExports' are the variables which are exported from this
    // package.  A list of objects with keys 'name' (required) and
    // 'testOnly' (boolean, defaults to false).

    this.declaredExports = declaredExports; // All of the data provided for eventual inclusion in the bundle,
    ...
    this.resources = resources; // Map from absolute paths of node_modules directories to
    // NodeModulesDirectory objects.

    this.nodeModulesDirectories = nodeModulesDirectories; // Provided for backwards compatibility; please use
  ...
  static fromJSON(unibuildJson, _ref2) {
    let {
      isopack,
    ...
    if (unibuildJson.format !== "unipackage-unibuild-pre1" && unibuildJson.format !== "isopack-2-unibuild") {
    ...
    const unibuildHasPrelink = unibuildJson.format === "unipackage-unibuild-pre1";
    const resources = [];

    _.each(unibuildJson.resources, function (resource) {
      rejectBadPath(resource.file);
      const data = files.readBufferWithLengthAndOffset(files.pathJoin(unibuildBasePath, resource.file), resource.length, resource.offset);

      if (resource.type === "prelink") {
        if (!unibuildHasPrelink) {
          throw Error("Unexpected prelink resource in " + unibuildJson.format + " at " + unibuildBasePath);
        ...
        if (resource.sourceMap) {
          ...
          prelinkResource.legacyPrelink.sourceMap = files.readFile(files.pathJoin(unibuildBasePath, resource.sourceMap), 'utf8');
        }
      ...
      } else if (_.contains(["head", "body", "css", "js", "asset"], resource.type)) {
        resources.push({
          type: resource.type,
          data: data,
        ...
        throw new Error("bad resource type in isopack: " + JSON.stringify(resource.type));
      }
    });
    ...
    const nodeModulesDirectories = NodeModulesDirectory.readDirsFromJSON(unibuildJson.node_modules, {
  ...
  toJSON(_ref3) {
    ...
    _.each(unibuild.nodeModulesDirectories, nmd => {
      const bundlePath = _.has(npmDirsToCopy, nmd.sourcePath) // We already have this npm directory from another unibuild.
      ? npmDirsToCopy[nmd.sourcePath] : npmDirsToCopy[nmd.sourcePath] = nmd.getPreferredBundlePath("isopack");
    ...
    _.each(unibuild.resources, function (resource) {
      if (_.contains(["head", "body"], resource.type)) {
        ...
        if (!usesModules && resource.fileOptions && resource.fileOptions.lazy) {
    ...
    _.each(unibuild.resources, function (resource) {
      ...
      const generatedFilename = builder.writeToGeneratedFilename(files.pathJoin(unibuildDir, resource.servePath || resource.path), {
        data: resource.data
      });
      ...
      unibuildJson.resources.push({
        ...
        length: resource.data.length,
        offset: 0,
        usesDefaultSourceProcessor: resource.usesDefaultSourceProcessor || undefined,
        servePath: resource.servePath || undefined,
        path: resource.path || undefined,
  ...
  getLegacyJsResources() {
    ...
    this.resources.forEach(resource => {
      if (resource.type === "source" && resource.extension === "js") {
...
//# sourceMappingURL=unibuild.js.map