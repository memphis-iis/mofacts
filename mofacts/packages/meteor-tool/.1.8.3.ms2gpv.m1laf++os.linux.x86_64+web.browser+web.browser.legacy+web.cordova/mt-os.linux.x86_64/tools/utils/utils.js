...
var utils = exports; // Parses <protocol>://<host>:<port> into an object { protocol: *, host:
...
exports.parseUrl = function (str, defaults) {
  ...
  var parsed = url.parse(str); // for consistency remove colon at the end of protocol
...
exports.ipAddress = function () {
  let defaultRoute; // netroute is not available on Windows

  if (process.platform !== "win32") {
    const netroute = require('netroute');

    const info = netroute.getInfo();
    defaultRoute = _.findWhere(info.IPv4 || [], {
      destination: "0.0.0.0"
    });
  }
  ...
  if (defaultRoute) {
    ...
    const addressEntry = _.findWhere(interfaces[iface], {
      family: "IPv4"
    });

    if (!addressEntry) {
      throw new Error("Network interface '".concat(iface, "', which seems to be the default route,\nnot found in interface list, or does not have an IPv4 address."));
    }
    ...
    let addressEntries = _.chain(interfaces).values().flatten().where({
      family: "IPv4",
      internal: false
    }).value();
...
exports.getHost = function () {
  ...
  if (archinfo.matches(archinfo.host(), 'os.osx') || archinfo.matches(archinfo.host(), 'os.linux')) {
...
exports.getAgentInfo = function () {
  ...
  ret.agent = "Meteor";
  ret.agentVersion = files.inCheckout() ? "checkout" : files.getToolsVersion();
...
exports.randomIdentifier = function () {
  const firstLetter = String.fromCharCode("a".charCodeAt(0) + Math.floor(Math.random() * 26));
...
exports.parsePackageAndVersion = function (packageAtVersionString, options) {
  var error = null;
  var separatorPos = Math.max(packageAtVersionString.lastIndexOf(' '), packageAtVersionString.lastIndexOf('@'));

  if (separatorPos < 0) {
    error = new Error("Malformed package version: " + JSON.stringify(packageAtVersionString));
  } else {
    var packageName = packageAtVersionString.slice(0, separatorPos);
    var version = packageAtVersionString.slice(separatorPos + 1);

    try {
      packageVersionParser.validatePackageName(packageName); // validate the version, ignoring the parsed result:
...
exports.validatePackageNameOrExit = function (packageName, options) {
  try {
    ...
    Console.error(e.message, Console.options({
      bulletPoint: "Error: "
...
exports.timeoutScaleFactor = timeoutScaleFactor; // If the given version matches a template (essentially, semver-style, but with
...
exports.generateSubsetsOfIncreasingSize = function (total, cb) {
  ...
  var Done = function () {}; // Generates all subsets of size subsetSize which contain the indices already
  ...
  var generateSubsetsOfFixedSize = function (goalSize, chosenIndices) {
    ...
    var firstIndexToConsider = chosenIndices.length ? chosenIndices[chosenIndices.length - 1] + 1 : 0;
...
exports.isNpmUrl = function (x) {
  ...
  return exports.isUrlWithSha(x) || /^(git|git\+ssh|git\+http|git\+https|https|http)?:\/\//.test(x);
};
...
exports.ensureOnlyValidVersions = function (dependencies, _ref) {
  let {
    forCordova
  } = _ref;
  ...
  _.each(dependencies, function (version, name) {
    ...
    if (!exports.isValidVersion(version, {
      ...
      throw new Error("Must declare valid version of dependency: " + name + '@' + version);
    }
  });
};

exports.isValidVersion = function (version, _ref2) {
  let {
    forCordova
  } = _ref2;
  return semver.valid(version) || exports.isUrlWithFileScheme(version) || (forCordova ? exports.isUrlWithSha(version) : exports.isNpmUrl(version));
};
...
exports.runGitInCheckout = function () {
  for (var _len2 = arguments.length, args = new Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
    args[_key2] = arguments[_key2];
  }

  args.unshift('--git-dir=' + files.convertToOSPath(files.pathJoin(files.getCurrentToolsDir(), '.git')));
...
exports.maxDateLength = "September 24th, 2014".length; // Returns a sha256 hash of a given string.
...
exports.sourceMapLength = function (sm) {
  ...
  return sm.mappings.length + (sm.sourcesContent || []).reduce((soFar, current) => {
...
function architecture() {
  const supportedArchitectures = {
    ...
    Windows_NT: {
      ia32: process.env.hasOwnProperty('PROCESSOR_ARCHITEW6432') ? 'x86_64' : 'i386',
      x64: 'x86_64'
    }
  };
  ...
  if (!supportedArchitectures[osType][osArch]) {
    throw new Error("Unsupported architecture ".concat(osArch));
  }
