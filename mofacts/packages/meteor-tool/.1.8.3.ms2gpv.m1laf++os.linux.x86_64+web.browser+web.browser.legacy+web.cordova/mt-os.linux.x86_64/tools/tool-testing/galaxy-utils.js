...
var auth = require('../meteor-services/auth.js'); // Run curl with the given specifications. Return an instance of Run.
...
var GALAXY_MOCK_MODE = process.env.GALAXY_MOCK_MODE; // Fail if the test is obviously not set up for using Galaxy.
//
...
exports.sanityCheck = selftest.markStack(function () {
  ...
  if (!GALAXY_USERNAME || !GALAXY_PASSWORD) {
    selftest.fail("Can't use test account with Galaxy. " + "Please specify GALAXY_USERNAME and GALAXY_PASSWORD.\n");
  }

  if (!process.env.APP_MONGO) {
    selftest.fail("Please provide an APP_MONGO url to use for deployed apps.\n");
  }
...
exports.httpOK = "HTTP/1.1 200 OK"; // String we expect to hit when using SSL.

exports.httpRedirect = "HTTP/1.1 307 Temporary Redirect"; // We expect containers to take some time to startup.
//
...
exports.createAndDeployApp = selftest.markStack(function (sandbox, options) {
  ...
  var appName = options.appName || testUtils.randomAppName(); // The simple app contains standart app packages and some small bits of code
  // so that we can check that it is being served correctly. Let's use that as
  // our default.

  var templateApp = options.templateApp || 'simple-app'; // Create the new galaxy settings.
...
exports.cleanUpApp = selftest.markStack(function (sandbox, appName) {
  testUtils.cleanUpApp(sandbox, appName); // Galaxy might take a while to spin up an app, though it should be fairly
  // quick.
...
exports.loggedInGalaxyAPIConnection = selftest.markStack(function () {
  ...
  return authClient.loggedInConnection(galaxyUrl, galaxyDomain, "galaxy-api");
...
exports.callGalaxyAPI = function (conn) {
...
exports.getAppRecordByName = selftest.markStack(function (appName) {
  ...
  conn.connection.registerStore('app', {
    update: function (msg) {
      if (msg.msg === 'added' && msg.fields && msg.fields.hostname === appName) {
        appRecord = _.extend({
          _id: msg.id
        }, msg.fields);
      }
  ...
  conn.subscribeAndWait("/app", appName); // If we can't find the app, fail the test right now.
...
exports.getAppContainerStatuses = selftest.markStack(function (appId, appName) {
  ...
  conn.connection.registerStore(statuses, {
    update: function (msg) {
      if (msg.msg === 'added' && msg.fields && msg.fields.appId === appId) {
        containers.push(_.extend({
          _id: msg.id
        }, msg.fields));
      }
    }
