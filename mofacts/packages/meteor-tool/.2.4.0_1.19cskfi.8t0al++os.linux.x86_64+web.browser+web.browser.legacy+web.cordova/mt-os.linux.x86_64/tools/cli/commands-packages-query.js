...
var compiler = require('../isobuild/compiler.js'); // We want these queries to be relatively fast, so we will only refresh the
// catalog if it is > 15 minutes old


var DEFAULT_MAX_AGE_MS = 15 * 60 * 1000; // Maximum number of recent versions of a package or a release that we should
// return to the user, unless a more complete mode is requested.

var MAX_RECENT_VERSIONS = 5; // XXX: Remove this if/when we do a Troposphere migration to backfill release
...
var formatHiddenVersions = function (hiddenVersions, oldestShownVersion) {
  ...
  var reasons = []; // Use our information about hidden versions to figure what reasons we
  ...
  if (!oldestShownVersion) {
    ...
    // age takes priority).
    if (packageVersionParser.lessThan(hiddenVersions.oldestVersion, oldestShownVersion)) {
      reasons.push("older");
    ...
    if (hiddenVersions.lastPreRelease && packageVersionParser.lessThan(oldestShownVersion, hiddenVersions.lastPreRelease)) {
      reasons.push("pre-release");
    }

    if (hiddenVersions.lastUnmigrated && packageVersionParser.lessThan(oldestShownVersion, hiddenVersions.lastUnmigrated)) {
      reasons.push("unmigrated");
    }
...
var getTempContext = function (options) {
  var projectContext; // If we are running in an app, we will use it to create a
...
var PkgExports = function (pkgExports) {
  var self = this; // Process and save the export data.

  self.data = _.map(pkgExports, function (exp) {
    var arches = exp.architectures; // Replace 'os' (what we store) with 'server' (what you would put in a
...
Object.assign(PkgExports.prototype, {
  ...
  getConsoleStr: function () {
    ...
    var strExports = _.map(self.data, function (exp) {
      ...
      return Console.noWrap(exp.name + " (" + exp.architectures.join(", ") + ")");
    });
...
var PkgImplies = function (pkgDeps) {
  var self = this;
  self.data = []; // Go through all the package dependencies. If a dependency has any implied
  // references, add it to the list.

  _.each(pkgDeps, function (ref, name) {
    var architectures = []; // We want to select the references that are implied (instead of just used)
...
Object.assign(PkgImplies.prototype, {
  ...
  getConsoleStr: function () {
    ...
    var strImplies = _.map(self.data, function (ref) {
      ...
      return Console.noWrap(ref.name + " (" + ref.architectures.join(", ") + ")");
    });
...
var PkgDependencies = function (pkgDeps) {
  var self = this;
  self.data = _.map( // The dependency on 'meteor' was almost certainly added automatically, by
  // Isobuild. Returning this to the user will only cause confusion.
  _.omit(pkgDeps, "meteor"), function (dep, depName) {
...
var PackageQuery = function (options) {
  var self = this; // This is the record in the packages collection. It contains things like
  // maintainers, and the package homepage.

  self.metaRecord = options.metaRecord;
  self.name = options.metaRecord.name; // This argument is required -- we use it to look up data. If it has not been
  ...
  self.localCatalog = options.projectContext.localCatalog; // Processing per-version availability architectures & dependencies is
  // expensive, so we don't do it unless we are asked to.

  self.showArchitecturesOS = options.showArchitecturesOS;
  self.showDependencies = options.showDependencies; // We don't want to show pre-releases and un-migrated versions to the user
  // unless they explicitly ask us about it.

  self.showHiddenVersions = options.showHiddenVersions; // Collect the data for this package, including looking up any specific
  ...
  if (options.version) {
    ...
    self.data = versionRecord.local ? self._getLocalVersion(versionRecord) : self._getOfficialVersion(versionRecord);
  } else {
...
Object.assign(PackageQuery.prototype, {
  ...
  _getVersionRecord: function (version) {
    var self = this; // We allow local version to override remote versions in meteor show, so we
    // should start by checking if this is a local version first.

    var versionRecord = self.localCatalog.getLatestVersion(self.name); // If we asked for "local" as the version number, and found any local version
    // at all, we are done.

    if (version === "local") {
      return versionRecord && Object.assign(versionRecord, {
        local: true
      });
    ...
    if (versionRecord && versionRecord.version === version) {
      return Object.assign(versionRecord, {
        local: true
      });
  ...
  print: function (options) {
    var self = this; // If we are asking for an EJSON-style output, we will only print out the
    // relevant fields.

    if (options.ejson) {
      Console.rawInfo(formatEJSON(self.data.version ? self._generateVersionObject(self.data) : self._generatePackageObject(self.data)));
      return;
  ...
  _collectPackageData: function () {
    ...
    var serverVersionRecords = catalog.official.getSortedVersionRecords(self.name);
    var totalVersions = serverVersionRecords.length; // If we are not going to show hidden versions, then we shouldn't waste time
    ...
    if (!self.showHiddenVersions) {
      ...
      var lastUnmigrated = "";
      var lastPreRelease = "";
      var oldestVersion = serverVersionRecords[0] && serverVersionRecords[0].version;

      var filteredVersionRecords = _.filter(serverVersionRecords, function (vr) {
        if (vr.unmigrated) {
    ...
    data["totalVersions"] = totalVersions; // Some per-version information gets displayed with the rest of the package
  ...
  _getOfficialVersion: function (versionRecord) {
    ...
    var data = {
      ...
      publishedBy: versionRecord.publishedBy && versionRecord.publishedBy.username,
    ...
    data["implies"] = new PkgImplies(versionRecord.dependencies); // Processing and formatting architectures takes time, so we don't want to
    ...
    if (self.showArchitecturesOS) {
      ...
      var architectures = _.map(allBuilds, function (build) {
        ...
        var archOS = _.filter(build.buildArchitectures.split('+'), function (arch) {
  ...
  _getLocalVersion: function (localRecord) {
    ...
    data["directory"] = packageSource.sourceRoot; // Get the exports.

    data["exports"] = new PkgExports(packageSource.getExports());
    data["implies"] = new PkgImplies(localRecord.dependencies); // If the version was not explicitly set by the user, the catalog backfills
    // a placeholder version for the constraint solver. We don't want to show
    // that version to the user.

    data["version"] = packageSource.versionExplicitlyProvided ? localRecord.version : "local"; // Processing dependencies takes time, and we don't want to do it if we
    // don't have to.
    ...
    var readmeInfo;
    main.captureAndExit("=> Errors while reading local packages:", "reading " + data["directory"], function () {
  ...
  _displayVersion: function (data) {
    var self = this;
    Console.info(data.name + "@" + data.version, Console.options({
      bulletPoint: "Package: "
    }));
    ...
    if (data.exports && !data.exports.isEmpty()) {
      Console.info(data["exports"].getConsoleStr(), Console.options({
        bulletPoint: "Exports: "
      }));
    }

    if (data.implies && !data.implies.isEmpty()) {
      Console.info(data["implies"].getConsoleStr(), Console.options({
        bulletPoint: "Implies: "
      }));
    }

    if (data.git) {
      Console.info(Console.url(data.git), Console.options({
        bulletPoint: "Git: "
      }));
    ...
    if (data.dependencies && !data.dependencies.isEmpty()) {
      ...
      Console.info(data.dependencies.getConsoleStr(), Console.options({
        indent: 2
      }));
    ...
    if (data.local && catalog.official.getVersion(data.name, data.version)) {
      Console.info();
      Console.info("This package version is built locally from source.", "The same version of this package also exists on the package server.", "To view its metadata, run", Console.command("'meteor show " + data.name + "@" + data.version + "'"), "from outside the project.");
    }
  },
  ...
  _generateVersionObject: function (data) {
    var versionFields = ["name", "version", "description", "summary", "git", "directory", "publishedBy", "publishedOn", "installed", "local", "architecturesOS", "deprecated", "deprecatedMessage"];
    var processedData = {};
  ...
  _displayPackage: function (data) {
    var self = this;
    var defaultVersion = data.defaultVersion; // Every package has a name. Some packages have a homepage.

    var displayName = data.defaultVersion ? data.name + "@" + data.defaultVersion.version : data.name;
    Console.info(displayName, Console.options({
      bulletPoint: "Package: "
    }));
    ...
    if (data.homepage) {
      Console.info(Console.url(data.homepage), Console.options({
        bulletPoint: "Homepage: "
      }));
    } // Local packages might not have any maintainers.


    if (!_.isEmpty(data.maintainers)) {
      Console.info(data.maintainers.join(", "), Console.options({
        bulletPoint: "Maintainers: "
      }));
    } // Git is per-version, so we will print the latest one, if one exists.


    if (defaultVersion && defaultVersion.git) {
      Console.info(Console.url(defaultVersion.git), Console.options({
        bulletPoint: "Git: "
      }));
    } // Print the exports.


    if (defaultVersion && defaultVersion.exports && !defaultVersion.exports.isEmpty()) {
      Console.info(defaultVersion["exports"].getConsoleStr(), Console.options({
        bulletPoint: "Exports: "
      }));
    }

    if (defaultVersion && defaultVersion.implies && !defaultVersion.implies.isEmpty()) {
      Console.info(defaultVersion["implies"].getConsoleStr(), Console.options({
        bulletPoint: "Implies: "
      }));
    }

    Console.info(); // If we don't have a long-form description, we will use the summary. For a
    // local package, we might not have a summary, in which case we should be
    // careful not to print extra lines.

    var printDescription = defaultVersion && (defaultVersion.description || defaultVersion.summary);
    ...
    if (data.versions && !_.isEmpty(data.versions)) {
      var versionsHeader = self.showHiddenVersions ? "Versions:" : "Recent versions:";
      ...
      data.versions.forEach(function (v) {
        ...
        var publishDate = utils.longformDate(v.publishedOn); // If there is a status that we would like to report for this package,
        // figure it out now.
      ...
      Console.printTwoColumns(versionRows, {
        indent: 2,
        ignoreWidth: true
      });
    ...
    if (data.totalVersions > versionRows.length) {
      var oldestShownVersion = data["versions"][0] && data["versions"][0].version || ""; // A string explaining why those versions have been hidden.

      var hiddenVersions = formatHiddenVersions(data["hiddenVersions"], oldestShownVersion); // We will word things in the message in different ways, based on whether
      // multiple versions exist/have been hidden.

      var hiddenVersionsPluralizer = data.totalVersions - data.versions.length == 1 ? "One " + hiddenVersions + " version of " + self.name + " has" : hiddenVersions[0].toUpperCase() + hiddenVersions.slice(1) + " versions of " + self.name + " have";
      var allVersionsPluralizer = data.totalVersions === 1 ? "the hidden version" : "all " + data.totalVersions + " versions"; // Display the final message.

      Console.info(hiddenVersionsPluralizer, "been hidden.", "To see " + allVersionsPluralizer + ", run", Console.command("'meteor show --show-all " + self.name + "'") + ".");
    }
  },
  // Returns a user-friendly object from this PackageQuery to the caller.  Takes
  ...
  _generatePackageObject: function (data) {
    var packageFields = ["name", "homepage", "maintainers", "totalVersions"]; // Process the versions array. We only want some of the keys, and we want to
    // make sure to get the right exports object.

    var versions = data.versions.map(function (version) {
      var versionFields = ["name", "version", "description", "summary", "git", "publishedBy", "publishedOn", "installed", "local", "directory", "architecturesOS", "deprecated", "deprecatedMessage"];
      var processedData = {};
...
var ReleaseQuery = function (options) {
  var self = this; // This is the record in the Releases collection. Contains metadata, such as
  // maintainers.

  self.metaRecord = options.metaRecord;
  self.name = options.metaRecord.name; // We don't always want to show non-recommended release versions.

  self.showHiddenVersions = options.showHiddenVersions; // Aggregate the query data. If we are asking for a specific version, get data
  // for a specific version, otherwise aggregate the data about this release
  // track in general.

  self.data = options.version ? self._getVersionDetails(options.version) : self._getReleaseData();
};
...
Object.assign(ReleaseQuery.prototype, {
  ...
  //     object.
  print: function (options) {
    var self = this; // If we are asking for an EJSON-style output, print out the relevant fields.

    if (options.ejson) {
      var versionFields = ["track", "version", "description", "publishedBy", "publishedOn", "tool", "packages", "recommended"];
  ...
  _getVersionDetails: function (version) {
    var self = this;
    var versionRecord = catalog.official.getReleaseVersion(self.name, version);
  ...
  _getReleaseData: function () {
    ...
    data["defaultVersion"] = catalog.official.getDefaultReleaseVersionRecord(self.name); // Collect information about versions.
    ...
    var versionFields = ["version", "description", "recommended"]; // orderKey is important for dealing with experimental versions, but it is
  ...
  _displayVersion: function (data) {
    ...
    if (!_.isEmpty(data.packages)) {
      ...
      _.each(data.packages, function (version, packageName) {
        Console.info(packageName + ": " + version, Console.options({
          indent: 2
        }));
      });
    ...
    Console.info("Published by " + data.publishedBy + " on " + utils.longformDate(getReleaseVersionPublishedOn(data)));
  },
  ...
  _displayRelease: function (data) {
    var self = this;
    Console.info("Release:", data.track); // There is no such thing as a local release, which means all releases have
    // a maintainer.
    ...
    if (data.totalVersions > rows.length) {
      var versionsPluralizer = data.totalVersions > 1 ? "all " + data.totalVersions + " versions" : "the hidden version"; // We only hide release versions for one reason -- they are not
      ...
      var versionsHidden = data.totalVersions - rows.length > 1 ? "Non-recommended versions of " + self.name + " have been hidden." : "One non-recommended version of " + self.name + " has been hidden.";
      Console.info(versionsHidden, "To see " + versionsPluralizer + ", run", Console.command("'meteor show --show-all " + self.name + "'") + ".");
    }
  },
...
main.registerCommand({
  ...
  var fullName;
  var name;
  var version; // Because of the new projectContext interface, we need to initialize the
  // project context in order to load the local catalog. This is not ideal.

  var projectContext = getTempContext(options); // If the user specified a query, process it.
  ...
  if (!_.isEmpty(options.args)) {
    ...
    if (!options.packageDir) {
      ...
      // a query.
      Console.error("Please specify a package or release name to show information about it.");
      process.exit(1);
    } // Use the projectContext to get the name of the package.


    var currentVersion = projectContext.localCatalog.getVersionBySourceRoot(options.packageDir);
    name = currentVersion.packageName;
    version = "local";
    fullName = name + "@local";
  }

  var query = null; // First, we need to figure out if we are dealing with a package, or a
  ...
  var packageRecord = catalog.official.getPackage(name) || projectContext.localCatalog.getPackage(name);
...
main.registerCommand({
  ...
  usesPackage: true,
  minArgs: 0,
  // So we can provide specific help
  maxArgs: 1,
  options: {
    maintainer: {
      type: String
  ...
  var projectContext = getTempContext(options); // XXX We should push the queries into SQLite!

  var allPackages = _.union(catalog.official.getAllPackageNames(), projectContext.localCatalog.getAllPackageNames());
  ...
  var filterBroken = function (match, isRelease, name) {
    ...
    if (!options["show-all"]) {
      ...
      vr = projectContext.localCatalog.getLatestVersion(name) || catalog.official.getLatestMainlineVersion(name);
    } else {
  ...
  if (options.maintainer) {
    var username = options.maintainer; // In the future, we should consider checking this on the server, but I
    ...
    selector = function (name, isRelease) {
      var record; // XXX make sure search works while offline
      ...
      return filterBroken(name.match(search) && record && !!_.findWhere(record.maintainers, {
        username: username
      }), isRelease, name);
    };
  } else {
    selector = function (name, isRelease) {
      return filterBroken(name.match(search), isRelease, name);
    };
  }
  ...
  buildmessage.enterJob({
    ...
    _.each(allPackages, function (pack) {
      if (selector(pack, false)) {
        ...
        if (!options['show-all']) {
          vr = projectContext.localCatalog.getLatestVersion(pack) || catalog.official.getLatestMainlineVersion(pack);
        } else {
  ...
  if (!output) {
    ...
    Console.info("You can use", Console.command("'meteor show'"), "to get more information on a specific item.");
  }
});
//# sourceMappingURL=commands-packages-query.js.map