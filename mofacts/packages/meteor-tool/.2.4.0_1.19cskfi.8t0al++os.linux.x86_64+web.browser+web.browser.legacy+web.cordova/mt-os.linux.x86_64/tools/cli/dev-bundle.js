!function (module1) {
  ...
  function getDevBundleDir() {
    ...
    var releaseFile = find(process.cwd(), makeStatTest("isFile"), ".meteor", "release");
    ...
    var release = fs.readFileSync(releaseFile, "utf8").replace(/^\s+|\s+$/g, "");

    if (!/^METEOR@\d+/.test(release)) {
      return defaultDevBundlePromise;
    }

    return Promise.resolve(getDevBundleForRelease(release)).then(function (devBundleDir) {
      if (devBundleDir) {
  ...
  function getDevBundleForRelease(release) {
    ...
    var packageMetadataDir = find(rootDir, makeStatTest("isDirectory"), ".meteor", "package-metadata");

    if (!packageMetadataDir) {
      return null;
    }

    var meteorToolDir = path.resolve(packageMetadataDir, "..", "packages", "meteor-tool");
    ...
    var dbPath = path.join(packageMetadataDir, "v2.0.1", "packages.data.db");
    ...
    return new Promise(function (resolve, reject) {
      db.get("SELECT content FROM releaseVersions WHERE track=? AND version=?", [track, version], function (error, data) {
        error ? reject(error) : resolve(data);
      });
    }).then(function (data) {
      if (data) {
        var tool = JSON.parse(data.content).tool;
        var devBundleDir = path.join(meteorToolDir, tool.split("@").slice(1).join("@"), "mt-" + getHostArch(), "dev_bundle");
