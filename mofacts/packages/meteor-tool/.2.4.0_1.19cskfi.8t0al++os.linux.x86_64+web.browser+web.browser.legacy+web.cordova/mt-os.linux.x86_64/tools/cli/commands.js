...
module.export({
  parseServerOptionsForRunCommand: () => parseServerOptionsForRunCommand,
  parseRunTargets: () => parseRunTargets,
  AVAILABLE_SKELETONS: () => AVAILABLE_SKELETONS
});
...
let ensureDevBundleDependencies;
...
let CordovaRunner;
module.link("../cordova/runner.js", {
  CordovaRunner(v) {
    CordovaRunner = v;
  }

}, 2);
let iOSRunTarget, AndroidRunTarget;
module.link("../cordova/run-targets.js", {
  iOSRunTarget(v) {
    iOSRunTarget = v;
  },
...
let EXAMPLE_REPOSITORIES;
module.link("./example-repositories.js", {
  EXAMPLE_REPOSITORIES(v) {
    EXAMPLE_REPOSITORIES = v;
  }
...
const {
  Profile
} = require("../tool-env/profile");

// The architecture used by Meteor Software's hosted servers; it's the
// architecture used by 'meteor deploy'.
var DEPLOY_ARCH = 'os.linux.x86_64'; // The default port that the development server listens on.

var DEFAULT_PORT = '3000'; // __dirname - the location of the current executing file

var __dirnameConverted = files.convertToStandardPath(__dirname); // Given a site name passed on the command line (eg, 'mysite'), return
...
var showInvalidArchMsg = function (arch) {
  ...
  Object.keys(archinfo.VALID_ARCHITECTURES).forEach(function (va) {
    Console.info(Console.command(va), Console.options({
      indent: 2
    }));
  });
...
function parseServerOptionsForRunCommand(options, runTargets) {
  ...
  if (mobileServerOption) {
    ...
    const isRunOnDeviceRequested = _.any(runTargets, runTarget => runTarget.isDevice);
...
function parseMobileServerOption(mobileServerOption) {
  let optionName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'mobile-server';
  let parsedMobileServerUrl = utils.parseUrl(mobileServerOption, {
    protocol: 'http'
  });

  if (!parsedMobileServerUrl.hostname) {
    Console.error("--".concat(optionName, " must include a hostname."));
    throw new main.ExitWithCode(1);
  }
...
function parseRunTargets(targets) {
  return targets.map(target => {
    ...
    if (platform == 'ios') {
      ...
      Console.error("Unknown run target: ".concat(target));
      throw new main.ExitWithCode(1);
    }
  });
}
...
function filterWebArchs(webArchs, excludeArchsOption) {
  if (excludeArchsOption) {
    const excludeArchs = excludeArchsOption.trim().split(/\s*,\s*/).filter(arch => excludableWebArchs.includes(arch));
...
function normalizeInspectOptions(options) {
  ...
  if (_.has(options, "debug-port")) {
    console.log("The --debug-port option is deprecated; " + "please use --inspect-brk=<port> instead.");
  ...
  if (_.has(options, "inspect-brk")) {
    ...
    if (_.has(options, "inspect")) {
      console.log("Both --inspect and --inspect-brk provided; " + "ignoring --inspect.");
...
main.registerCommand(Object.assign({
  name: 'run'
}, runCommandOptions), doRunCommand);

function doRunCommand(options) {
  Console.setVerbose(!!options.verbose); // Additional args are interpreted as run targets
  ...
  if (options['app-port']) {
    ...
    appHost = appPortMatch[1] || null; // It's legit to specify `--app-port host:` and still let the port be
    // randomized.
  ...
  if (options.production) {
    Console.warn("Warning: The --production flag should only be used to simulate production " + "bundling for testing purposes. Use meteor build to create a bundle for " + "production deployment. See: https://guide.meteor.com/deployment.html");
  }
  ...
  return runAll.run(_objectSpread(_objectSpread({
...
main.registerCommand(Object.assign({
  name: 'debug'
}, runCommandOptions), function (options) {
...
main.registerCommand({
  ...
  if (!options.appDir) {
    ...
    require('../shell-client').connect(files.convertToOSPath(projectContext.getMeteorShellDirectory()));
...
const AVAILABLE_SKELETONS = ["apollo", "bare", "blaze", "full", "minimal", DEFAULT_SKELETON, "typescript", "vue", "svelte"];
main.registerCommand({
  name: 'create',
  maxArgs: 1,
  options: {
  ...
  disableNativeWatcher(); // Creating a package is much easier than creating an app, so if that's what
  ...
  if (options.package) {
    ...
    var transform = function (x) {
      var xn = x.replace(/~name~/g, packageName).replace(/~fs-name~/g, fsName); // If we are running from checkout, comment out the line sourcing packages
      ...
      if (release.current.isCheckout()) {
        xn = xn.replace(/~cc~/g, "//");
        var rel = catalog.official.getDefaultReleaseVersion(); // the no-release case should never happen except in tests.

        relString = rel ? rel.version : "no-release";
      } else {
        xn = xn.replace(/~cc~/g, "");
        relString = release.current.getDisplayName({
          noPrefix: true
        });
    ...
    var displayPackageDir = files.convertToOSPath(files.pathRelative(files.cwd(), packageDir)); // Since the directory can't have colons, the directory name will often not
  ...
  if (options.list) {
    ...
    _.each(EXAMPLE_REPOSITORIES, function (repoInfo, name) {
      const branchInfo = repoInfo.branch ? "/tree/".concat(repoInfo.branch) : '';
      Console.info(Console.command("".concat(name, ": ").concat(repoInfo.repo).concat(branchInfo)), Console.options({
        indent: 2
      }));
    });

    Console.info();
    Console.info("To create an example, simply", Console.command("git clone"), "the relevant repository and branch (run", Console.command("'meteor create --example <name>'"), " to see the full command).");
    return 0;
  }
  ...
  if (options.example) {
    ...
    if (!repoInfo) {
      Console.error("".concat(options.example, ": no such example."));
      Console.error("List available applications with", Console.command("'meteor create --list'") + ".");
      return 1;
    }

    const branchOption = repoInfo.branch ? " -b ".concat(repoInfo.branch) : '';
    const path = options.args.length === 1 ? " ".concat(options.args[0]) : '';
    Console.info("To create the ".concat(options.example, " example, please run:"));
    Console.info(Console.command("git clone ".concat(repoInfo.repo).concat(branchOption).concat(path)), Console.options({
      indent: 2
    }));
    return 0;
  }
  ...
  var destinationHasCodeFiles = false; // If the directory doesn't exist, it clearly doesn't have any source code
  // inside itself
  ...
  const skeletonExplicitOption = AVAILABLE_SKELETONS.find(skeleton => !!options[skeleton]);
  const skeleton = skeletonExplicitOption || DEFAULT_SKELETON;
  files.cp_r(files.pathJoin(__dirnameConverted, '..', 'static-assets', "skel-".concat(skeleton)), appPath, {
    transformFilename: function (f) {
      return transform(f);
    },
    transformContents: function (contents, f) {
  ...
  main.captureAndExit("=> Errors while creating your project", function () {
    ...
    if (!release.current.isCheckout()) {
      projectContext.projectConstraintsFile.updateReleaseConstraints(release.current._manifest);
  ...
  var appNameToDisplay = appPathAsEntered === "." ? "current directory" : "'".concat(appPathAsEntered, "'");
  var message = "Created a new Meteor app in ".concat(appNameToDisplay);
  message += ".";
  ...
  if (appPathAsEntered !== ".") {
    ...
    const appPathWithQuotesIfSpaces = appPathAsEntered.indexOf(' ') === -1 ? appPathAsEntered : "'".concat(appPathAsEntered, "'"); // Don't tell people to 'cd .'
  ...
  Console.info(Console.url("https://www.meteor.com/tutorials"), Console.options({
    indent: 2
  }));
  Console.info("");
  Console.info("When you’re ready to deploy and host your new Meteor application, check out Cloud:");
  Console.info(Console.url("https://www.meteor.com/cloud"), Console.options({
    indent: 2
  }));
...
main.registerCommand(_objectSpread({
  name: "build"
}, buildCommands), function (options) {
  return Promise.asyncApply(() => {
    disableNativeWatcher();
    return Profile.run("meteor build", () => Promise.await(buildCommand(options)));
  });
...
main.registerCommand(_objectSpread({
  ...
  return Promise.asyncApply(() => {
    ...
    Console.error();
    return Profile.run("meteor bundle", () => Promise.await(buildCommand(_objectSpread(_objectSpread({}, options), {}, {
      _bundleOnly: true
    }))));
  });
});
...
var buildCommand = function (options) {
  ...
  if (options.architecture && !_.has(archinfo.VALID_ARCHITECTURES, options.architecture)) {
  ...
  projectContext.packageMapDelta.displayOnConsole(); // _bundleOnly implies serverOnly

  const serverOnly = options._bundleOnly || !!options['server-only']; // options['mobile-settings'] is used to set the initial value of
  ...
  if (options.platforms) {
    ...
    platformsArray.forEach(plat => {
      if (![...excludableWebArchs, 'android', 'ios'].includes(plat)) {
        throw new Error("Not allowed platform on '--platforms' flag: ".concat(plat));
      }
    });
...
on an OS X system.");
  ...
  var relative = files.pathRelative(options.appDir, outputPath); // We would like the output path to be outside the app directory, which
  // means the first step to getting there is going up a level.

  if (relative.substr(0, 2) !== '..') {
    Console.warn();
    Console.labelWarn("The output directory is under your source tree.\nYour generated files may get interpreted as source code!\nConsider building into a different directory instead\n".concat(Console.command("meteor build ../output")), Console.options({
      indent: 2
    }));
    Console.warn();
  }

  var bundlePath = options.directory ? options._bundleOnly ? outputPath : files.pathJoin(outputPath, 'bundle') : files.pathJoin(buildDir, 'bundle');
  ...
  if (!options.directory) {
    main.captureAndExit('', 'creating server tarball', () => {
      try {
        var outputTar = options._bundleOnly ? outputPath : files.pathJoin(outputPath, appName + '.tar.gz');
  ...
  if (!_.isEmpty(cordovaPlatforms)) {
    let cordovaProject;
    main.captureAndExit('', () => {
      let pluginVersionsFromStarManifest, displayNameForPlatform;
      module.link("../cordova/index.js", {
        pluginVersionsFromStarManifest(v) {
          pluginVersionsFromStarManifest = v;
        },
      ...
      buildmessage.enterJob({
        ...
        if (buildmessage.jobHasMessages()) return;
        const pluginVersions = pluginVersionsFromStarManifest(bundleResult.starManifest);
        cordovaProject.prepareFromAppBundle(bundlePath, pluginVersions);
      });

      for (platform of cordovaPlatforms) {
        buildmessage.enterJob({
          title: "building Cordova app for ".concat(displayNameForPlatform(platform))
        }, () => {
          let buildOptions = {
            release: !options.debug
          };
          const buildPath = files.pathJoin(projectContext.getProjectLocalDirectory('cordova-build'), 'platforms', platform);
          const platformOutputPath = files.pathJoin(outputPath, platform); // Prepare the project once again to ensure that it is up to date
          ...
          if (platform === 'ios') {
            ...
            cordovaProject.buildForPlatform(platform, _objectSpread(_objectSpread({}, buildOptions), {}, {
              argv: ["--packageType", options.packageType || "bundle"]
            }));
          } // Once prepared, copy the bundle to the final location.


          files.cp_r(buildPath, files.pathJoin(platformOutputPath, 'project')); // Make some platform-specific adjustments to the resulting build.
          ...
          if (platform === 'ios') {
            ...
            const apkPath = files.pathJoin(buildPath, "app/build/outputs/".concat(packageType, "/").concat(options.debug ? 'debug' : 'release'), options.debug ? "app-debug.".concat(packageExtension) : "".concat(packageName, ".").concat(packageExtension));
            console.log(apkPath);

            if (files.exists(apkPath)) {
              files.copyFile(apkPath, files.pathJoin(platformOutputPath, options.debug ? "app-debug.".concat(packageExtension) : "".concat(packageName, ".").concat(packageExtension)));
            }
...
main.registerCommand({
  ...
  let projectContext = null; // if the goal is to lint the package, don't include the whole app
  ...
  if (packageDir) {
    ...
    main.captureAndExit("=> Errors while setting up package:", () => // Read metadata and initialize catalog.
    projectContext.initializeCatalog());
    const versionRecord = projectContext.localCatalog.getVersionBySourceRoot(packageDir);
  ...
  if (bundle.errors) {
    Console.error("=> Errors building your ".concat(displayName, ":\n\n").concat(bundle.errors.formatMessages()));
...
main.registerCommand({
  ...
  if (options.args.length === 0) {
    ...
    var mongoPort = findMongoPort(files.pathJoin(options.appDir, ".meteor", "local", "db")); // XXX detect the case where Meteor is running, but MONGO_URL was
    // specified?
    ...
    if (!mongoPort) {
      ...
      Console.info();
      Console.info("If you're trying to connect to the database of an app you deployed with ".concat(Console.command("'meteor deploy'"), ", specify your site's name as an argument to this command."));
      return 1;
    }
...
main.registerCommand({
  ...
  if (options.args.length !== 0) {
    ...
    Console.error(Console.command("meteor deploy --delete appname"), Console.options({
      indent: 2
    }));
    Console.error("followed by");
    Console.error(Console.command("meteor deploy appname"), Console.options({
      indent: 2
    }));
    return 1;
  }
  ...
  if (process.env.MONGO_URL) {
    Console.info("As a precaution, meteor reset only clears the local database that is " + "provided by meteor run for development. The database specified with " + "MONGO_URL will NOT be reset.");
  ...
  return files.rm_recursive_async(files.pathJoin(options.appDir, '.meteor', 'local')).then(() => {
...
main.registerCommand({
  ...
  options: {
    ...
    'override-architecture-with-local': {
      type: Boolean
    },
    'allow-incompatible-update': {
      type: Boolean
  ...
  return Promise.asyncApply(() => {
    ...
    return Profile.run("meteor deploy", () => Promise.await(deployCommand(...args)));
  });
});

function deployCommand(options, _ref) {
  let {
    rawOptions
  } = _ref;
  ...
  if (options.password) {
    Console.error("Setting passwords on apps is no longer supported. Now there are " + "user accounts and your apps are associated with your account so " + "that only you (and people you designate) can access them. See the " + Console.command("'meteor authorized'") + " command.");
    return 1;
  }
  ...
  if (!loggedIn && !options["deploy-token"]) {
    Console.error("You must be logged in to deploy, just enter your email address.");
    Console.error();
...
main.registerCommand({
  ...
  if (Object.keys(_.pick(options, 'add', 'remove', 'transfer', 'list')).length > 1) {
    Console.error("Sorry, you can only perform one authorization operation at a time.");
    return 1;
  }
...
function doTestCommand(options) {
  ...
  const runTargets = parseRunTargets(_.intersection(Object.keys(options), ['ios', 'ios-device', 'android', 'android-device']));
  const {
    parsedServerUrl,
    parsedMobileServerUrl,
    parsedCordovaServerPort
  } = parseServerOptionsForRunCommand(options, runTargets); // Make a temporary app dir (based on the test runner app). This will be
  ...
  if (testAppPath) {
    ...
    try {
      if (files.mkdir_p(absTestAppPath, 448)) {
        testRunnerAppDir = absTestAppPath;
      } else {
        Console.error('The specified --test-app-path directory could not be used, as ' + "\"".concat(testAppPath, "\" already exists and it is not a directory."));
        return 1;
      }
    } catch (error) {
      Console.error('Unable to create the specified --test-app-path directory of ' + "\"".concat(testAppPath, "\"."));
      throw error;
    }
  }
  ...
  if (options['driver-package']) {
    includePackages.push(global.testCommandMetadata.driverPackage = options['driver-package'].trim());
  } else if (options["test-packages"]) {
    includePackages.push(global.testCommandMetadata.driverPackage = "test-in-browser");
  }
  ...
  if (options["test-packages"]) {
    ...
    try {
      ...
      if (error.code === 'EACCES' && options['test-app-path']) {
        Console.error('The specified --test-app-path directory of ' + "\"".concat(testRunnerAppDir, "\" exists, but the current user does not have ") + "read/write permission in it.");
      }
    ...
    projectContextOptions.explicitlyAddedLocalPackageDirs = packagesByPath; // XXX Because every run uses a new app with its own IsopackCache directory,
    ...
    projectContext.releaseFile.write(release.current.isCheckout() ? "none" : release.current.name);
    var packagesToAdd = getTestPackageNames(projectContext, options.args); // filter out excluded packages
    ...
    packagesToAdd.unshift(global.testCommandMetadata.driverPackage); // Also, add `autoupdate` so that you don't have to manually refresh the tests
    ...
    projectContext.projectConstraintsFile.removeAllPackages();
    projectContext.projectConstraintsFile.addConstraints(constraintsToAdd); // Write these changes to disk now, so that if the first attempt to prepare
    ...
    projectContextOptions.projectLocalDir = files.pathJoin(testRunnerAppDir, '.meteor', 'local'); // Copy the existing build and isopacks to speed up the initial start

    function copyDirIntoTestRunnerApp(allowSymlink) {
  ...
  return runTestAppForPackages(projectContext, Object.assign(options, {
    mobileServerUrl: utils.formatUrl(parsedMobileServerUrl),
    cordovaServerPort: parsedCordovaServerPort,
    proxyPort: parsedServerUrl.port,
    proxyHost: parsedServerUrl.hostname
...
main.registerCommand({
  ...
  if (result.response.statusCode === 401 && body && body.error === "invalid_credential") {
    Console.error("You must be logged in to list your organizations."); // XXX It would be nice to do a username/password prompt here like
...
main.registerCommand({
  ...
  var selftest = require('../tool-testing/selftest.js'); // Auto-detect whether to skip 'net' tests, unless --force-online is passed.
...
main.registerCommand({
  ...
  Console.warn();
  Console.warn("The 'meteor admin get-machine' command has been disabled and", "the build farm has been discontinued.");
  Console.warn();
  Console.info("As of Meteor 1.4, packages with binary dependencies are", "automatically compiled when they are installed in an application,", "assuming the target machine has a basic compiler toolchain.");
  Console.info();
