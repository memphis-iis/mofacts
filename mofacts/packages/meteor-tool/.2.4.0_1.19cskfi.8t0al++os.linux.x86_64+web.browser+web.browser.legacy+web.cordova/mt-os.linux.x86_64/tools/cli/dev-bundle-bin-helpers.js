...
exports.getCommand = function (name, devBundleDir) {
  var result = null; // Strip leading and/or trailing whitespace.
...
function isValidCommand(name, devBundleDir) {
  ...
  var meteorCommandsJsonPath = path.join(devBundleDir, "bin", ".meteor-commands.json");
...
exports.getEnv = function (options) {
  ...
  return devBundlePromise.then(function (devBundleDir) {
    var paths = [// When npm looks for node, it must find dev_bundle/bin/node.
    path.join(devBundleDir, "bin"), // When npm looks for meteor, it should find dev_bundle/../meteor.
    path.dirname(devBundleDir), // Also make available any scripts installed by packages in
    // dev_bundle/lib/node_modules, such as node-gyp.
    path.join(devBundleDir, "lib", "node_modules", ".bin")];
    var env = Object.create(process.env); // Make sure notifications to update npm aren't presented to the user.

    env.NO_UPDATE_NOTIFIER = true; // Make sure `meteor npm install --global ...` installs into
...
function addWindowsVariables(devBundleDir, env) {
  ...
  env.PYTHON = env.PYTHON || path.join(devBundleDir, "python", "python.exe"); // While the original process.env object allows for case insensitive
  ...
  return new Promise(function (resolve) {
    var nodeGypPylibDir = path.join(devBundleDir, "lib", "node_modules", "node-gyp", "gyp", "pylib");

    var child = require("child_process").spawn(env.PYTHON, ["-c", ["from gyp.MSVSVersion import SelectVisualStudioVersion", "try:", "  print SelectVisualStudioVersion(allow_fallback=False).short_name", "except:", "  print 2015"].join("\n")], {
      cwd: nodeGypPylibDir,
      stdio: "pipe"
    });
    ...
    function finish(codeOrError) {
      if (codeOrError) {
        ...
        cachedMSVSVersion = Buffer.concat(chunks).toString("utf8").replace(/^\s+|\s+$/g, "");
      }
...
//# sourceMappingURL=dev-bundle-bin-helpers.js.map