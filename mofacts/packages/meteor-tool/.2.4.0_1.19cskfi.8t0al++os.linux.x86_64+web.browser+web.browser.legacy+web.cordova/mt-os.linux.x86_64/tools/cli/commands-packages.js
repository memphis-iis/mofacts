let ensureDevBundleDependencies, newPluginId, splitPluginsAndPackages;
module.link("../cordova/index.js", {
  ...
  splitPluginsAndPackages(v) {
    splitPluginsAndPackages = v;
  }

}, 0);
let disableNativeWatcher;
module.link("../fs/safe-watcher", {
  disableNativeWatcher(v) {
    disableNativeWatcher = v;
  }

}, 1);
let updateMeteorToolSymlink;
module.link("../packaging/updater.js", {
  updateMeteorToolSymlink(v) {
    updateMeteorToolSymlink = v;
  }
...
main.registerCommand({
  ...
  disableNativeWatcher(); // If we're in an app, make sure that we can build the current app. Otherwise
  ...
  var addPackages = function (packageNames) {
    projectContext.projectConstraintsFile.addConstraints(_.map(packageNames, function (p) {
...
var updatePackageMetadata = function (packageSource, conn) {
  ...
  var version = packageSource.version; // You can't change the metadata of a record that doesn't exist.

  var existingRecord = catalog.official.getVersion(name, version);

  if (!existingRecord) {
    Console.error("You can't call", Console.command("`meteor publish --update`"), "on version " + version + " of " + "package '" + name + "' without publishing it first.");
    return 1;
  } // Load in the user's documentation, and check that it isn't blank.


  var readmeInfo;
  main.captureAndExit("=> Errors while publishing:", "reading documentation", function () {
  ...
  if (readmeInfo && readmeInfo.hash === files.blankHash) {
    Console.error("Your documentation file is blank, so users may have trouble", "figuring out how to use your package. Please fill it out, or", "set 'documentation: null' in your Package.describe.");
    return 1;
  }

  ; // Finally, call to the server.

  main.captureAndExit("=> Errors while publishing:", "updating package metadata", function () {
  ...
  Console.info("Success. You can take a look at the new metadata by running", Console.command("'meteor show " + name + "@" + version + "'"), "outside the current project directory."); // Refresh, so that we actually learn about the thing we just published.
...
main.registerCommand({
  ...
  if (options.create && options['existing-version']) {
    // Make up your mind!
    Console.error("The --create and --existing-version options cannot", "both be specified.");
    return 1;
  }

  if (options.update && options.create) {
    Console.error("The --create and --update options cannot both be specified.");
    return 1;
  }

  if (options.update && options["existing-version"]) {
    Console.error("The --update option implies that the version already exists.", "You do not need to use the --existing-version flag with --update.");
    return 1;
  }
  ...
  if (!options.appDir) {
    ...
    projectContext = new projectContextModule.ProjectContext({
      ...
      // won't have a packages dir, that's OK
  ...
  var localVersionRecord = projectContext.localCatalog.getVersionBySourceRoot(options.packageDir);
  ...
  if (!localVersionRecord) {
    ...
    Console.error("The package you are in appears to be inside a Meteor app but is not " + "in its packages directory. You may only publish packages that are " + "entirely outside of a project or that are loaded by the project " + "that they are inside.");
    return 1;
  }
  ...
  if (!packageSource.versionExplicitlyProvided) {
    Console.error("A version must be specified for the package. Set it with " + "Package.describe.");
    return 1;
  ...
  if (options.create) {
    ...
    if (packageInfo) {
      Console.error("Package already exists. To create a new version of an existing " + "package, do not use the --create flag!");
      return 2;
    }

    if (!options['top-level'] && !packageName.match(/:/)) {
      Console.error("Only administrators can create top-level packages without an", "account prefix. (To confirm that you wish to create a top-level", "package with no account prefix, please run this command again", "with the --top-level option.)"); // You actually shouldn't be able to get here without being logged in, but
  ...
  _.each([packageName, packageSource.testName], function (name) {
    ...
    projectContext.projectConstraintsFile.addConstraints([utils.parsePackageConstraint(name)]);
  ...
  if (!options['no-lint']) {
    ...
    if (warnings && warnings.hasMessages()) {
      Console.arrowError("Errors linting your package; run with --no-lint to ignore.");
  ...
  if (process.env.METEOR_TEST_NO_PUBLISH) {
    Console.error('Would publish the package at this point, but since the ' + 'METEOR_TEST_NO_PUBLISH environment variable is set, just going ' + 'to finish here.');
    return 0;
  }
  ...
  main.captureAndExit("=> Errors while publishing:", "publishing the package", function () {
  ...
  Console.info('Published ' + packageName + '@' + localVersionRecord.version + '.'); // We are only publishing one package, so we should close the connection, and
  // then exit with the previous error code.

  conn.close(); // Warn the user if their package is not good for all architectures.
  ...
  if (binary && options['existing-version']) {
    ...
    Console.warn();
    Console.warn("This package contains binary code which must be", "compiled on the architecture it will eventually run on.");
    Console.warn();
    Console.info("Meteor 1.4 and higher will automatically compile packages", "with binary dependencies when they are installed, assuming", "the target machine has a basic compiler toolchain.");
    Console.info();
    Console.info("To see the requirements for this compilation step,", "consult the platform requirements for 'node-gyp':");
    Console.info(Console.url("https://github.com/nodejs/node-gyp"), Console.options({
      indent: 2
    }));
    Console.info();
...
main.registerCommand({
  ...
  if (all.length !== 2) {
    Console.error('Incorrect argument. Please use the form of <packageName>@<version>');
  ...
  if (!packageInfo) {
    Console.error("You can't call " + Console.command("`meteor publish-for-arch`") + "on package '" + name + "' without " + " publishing it first.");
    Console.error();
    Console.error("To publish the package, run " + Console.command("`meteor publish --create` ") + "from the package directory.");
    Console.error();
    return 1;
  }
  ...
  if (!pkgVersion) {
    Console.error("You can't call", Console.command("`meteor publish-for-arch`"), "on version " + versionString + " of " + "package '" + name + "' without publishing it first.");
    Console.error();
    Console.error("To publish the package, run " + Console.command("`meteor publish ` ") + "from the package directory.");
    Console.error();
    return 1;
  }

  if (!pkgVersion.source || !pkgVersion.source.url) {
    Console.error("There is no source uploaded for", name + '@' + versionString);
    return 1;
  ...
  if (!_.has(pkgVersion, 'releaseName')) {
    if (files.inCheckout()) {
      Console.error("This package was published from an old version of meteor, " + "but you are running from checkout! Consider running " + Console.command("`meteor --release 1.0`"), "so we can springboard correctly.");
      process.exit(1);
    }
  ...
  if (pkgVersion.releaseName === null) {
    if (!files.inCheckout()) {
      Console.error("This package was published from a checkout of meteor!", "The tool cannot replicate that environment and will not even try.", "Please check out meteor at the " + "corresponding git commit and try again.");
      process.exit(1);
    }
  } else if (files.inCheckout()) {
    Console.error("This package was published from a built version of meteor, " + "but you are running from checkout! Consider running from a " + "proper Meteor release with " + Console.command("`meteor --release " + pkgVersion.releaseName + "`"), "so we can springboard correctly.");
    process.exit(1);
  ...
  var tempProjectDir = files.mkdtemp('meteor-package-arch-build'); // Copy over a version lock file from the source tarball.

  var versionsFile = files.pathJoin(packageDir, '.versions');

  if (!files.exists(versionsFile)) {
    Console.error("This package has no valid version lock file: are you trying to use " + "publish-for-arch on a core package? Publish-for-arch cannot " + "guarantee safety. Please use", Console.command("'meteor publish --existing-version'"), "instead.");
    process.exit(1);
  }

  files.copyFile(files.pathJoin(packageDir, '.versions'), files.pathJoin(tempProjectDir, '.meteor', 'versions')); // Set up the project.
  ...
  projectContext.projectConstraintsFile.addConstraints([utils.parsePackageConstraint(name + "@=" + versionString)]);
  main.captureAndExit("=> Errors while initializing project:", function () {
    projectContext.prepareProjectForBuild();
  });
  projectContext.packageMapDelta.displayOnConsole({
  ...
  main.captureAndExit("=> Errors while publishing build:", "publishing package " + name + " for architecture " + isopk.buildArchitectures(), function () {
    packageClient.createAndPublishBuiltPackage(conn, isopk, projectContext.isopackCache);
  });
...
main.registerCommand({
  ...
  var relConf = {}; // Let's read the json release file. It should, at the very minimum contain
  ...
  main.captureAndExit("=> Errors in release schema:", "double-checking release schema", function () {
    ...
    if (!_.has(relConf, 'track')) {
      buildmessage.error("Configuration file must specify release track. (track).");
    }

    if (!_.has(relConf, 'version')) {
      buildmessage.error("Configuration file must specify release version. (version).");
    }

    if (!_.has(relConf, 'description')) {
      buildmessage.error("Configuration file must contain a description (description).");
    ...
    if (!options['from-checkout']) {
      if (!_.has(relConf, 'tool')) {
        buildmessage.error("Configuration file must specify a tool version (tool) unless in " + "--from-checkout mode.");
      }

      if (!_.has(relConf, 'packages')) {
        buildmessage.error("Configuration file must specify package versions (packages) " + "unless in --from-checkout mode.");
      }
    ...
    if (relConf.track === catalog.DEFAULT_TRACK) {
      ...
      if (start === "0.8." || start === "0.7." || start === "0.6." || start === "0.5.") {
        buildmessage.error("It looks like you are trying to publish a pre-package-server " + "meteor release. Doing this through the package server is going " + "to cause a lot of confusion. Please use the old release process.");
      }
    }
  ...
  if (!options['create-track']) {
    ...
    if (!trackRecord) {
      Console.error('There is no release track named ' + relConf.track + '. If you are creating a new track, use the --create-track flag.');
      return 1;
    ...
    if (!packageClient.amIAuthorized(relConf.track, conn, true)) {
      Console.error('You are not an authorized maintainer of ' + relConf.track + ".");
  ...
  if (options['from-checkout']) {
    ...
    if (relConf.packages || relConf.tool) {
      Console.error("Setting the --from-checkout option will use the tool and packages " + "in your meteor checkout. " + "Your release configuration file should not contain that information.");
      return 1;
    ...
    projectContext.projectConstraintsFile.addConstraints(_.map(allPackagesWithTests, function (p) {
      return utils.parsePackageConstraint(p);
    })); // Build!
    ...
    main.captureAndExit("=> Errors in release packages:", function () {
      _.each(allPackages, function (packageName) {
        buildmessage.enterJob("checking consistency of " + packageName, function () {
          ...
          if (!packageSource.versionExplicitlyProvided) {
            buildmessage.error("A version must be specified for the package. Set it with " + "Package.describe.");
            return;
          ...
          var oldVersionRecord = catalog.official.getVersion(packageName, packageSource.version); // Include this package in our release.

          relConf.packages[packageName] = packageSource.version; // If there is no old version, then we need to publish this package.
          ...
          if (!oldVersionRecord) {
            ...
            if (!relConf.official && newVersion.split("-").length < 2) {
              buildmessage.error("It looks like you are building an experimental release or " + "pre-release. Any packages we publish here should have an " + "pre-release identifier at the end (eg, 1.0.0-dev). If " + "this is an official release, please set official to true " + "in the release configuration file.");
              return;
            }
            ...
            const existingBuild = // First try with the non-simplified build architecture
            // list, which is likely to be something like
            // os+web.browser+web.browser.legacy+web.cordova:
            catalog.official.getBuildWithPreciseBuildArchitectures(oldVersionRecord, isopk.buildArchitectures()) || // If that fails, fall back to the simplified architecture
            ...
            catalog.official.getBuildWithPreciseBuildArchitectures(oldVersionRecord, isopk.buildArchitectures(true));
            ...
            } else if (!options["skip-tree-hashing"] || // Always check the treeHash of the meteor-tool
            ...
            packageName === "meteor-tool") {
              ...
              var bundleBuildResult = packageClient.bundleBuild(isopk, projectContext.isopackCache);
              somethingChanged = bundleBuildResult.treeHash !== existingBuild.build.treeHash;
            }

            if (somethingChanged) {
              buildmessage.error("Something changed in package " + packageName + "@" + isopk.version + ". Please upgrade its version number.");
            }
          }
        });
      });
    ...
    if (options['dry-run']) {
      main.captureAndExit("=> Dry run", function () {
        buildmessage.error("This is not an error but it was just a validation" + " and nothing was published. Remove --dry-run to publish.");
      });
    ...
    _.each(toPublish, function (packageName) {
      main.captureAndExit("=> Errors while publishing:", "publishing package " + packageName, function () {
        var isopk = projectContext.isopackCache.getIsopack(packageName);
        ...
        Console.info('Published ' + packageName + '@' + packageSource.version + '.');
  ...
  main.captureAndExit("=> Errors while publishing release:", "publishing release", function () {
    ...
    if (options['create-track']) {
      ...
      buildmessage.enterJob("creating a new release track", function () {
        packageClient.callPackageServerBM(conn, 'createReleaseTrack', {
          name: relConf.track
        });
      });
    ...
    buildmessage.enterJob("creating a new release version", function () {
      ...
      if (relConf.patchFrom) {
        packageClient.callPackageServerBM(conn, 'createPatchReleaseVersion', record, relConf.patchFrom);
      } else {
        packageClient.callPackageServerBM(conn, 'createReleaseVersion', record);
      }
    });
  }); // Learn about it.
  ...
  if (options['from-checkout']) {
    ...
    if (config.getPackageServerFilePrefix() !== 'packages') {
      ...
      try {
        Console.info("Pushing git tag (this should fail if you are not from Meteor Software)");
        utils.runGitInCheckout('push', 'git@github.com:meteor/meteor.git', 'refs/tags/' + gitTag);
      } catch (err) {
        Console.error("Failed to push git tag. Please push git tag manually!");
        fail = true;
      }
    ...
    if (!_.isEmpty(unfinishedBuilds)) {
      Console.warn();
      Console.labelWarn("Some packages contain binary dependencies.");
      Console.warn("Builds have not been published for the following packages:");
      ...
      Console.warn("Please publish the builds separately, from a proper build machine.");
    }
  }
...
main.registerCommand({
  ...
  if (showTree || showJson) {
    ...
    const showDetails = !!options['details']; // Load package details of all used packages (inc. dependencies)
    ...
    const dontExpand = new Set(topLevelSet.values()); // Recursive function that outputs each package

    const printPackage = function (_ref) {
      let {
        packageToPrint,
        isWeak,
        indent1,
      ...
      let deps = Object.keys(depsObj).sort(); // Ignore references to a meteor version or isobuild marker packages
      ...
      const shouldExpand = deps.length > 0 && !expandedAlready && !isWeak; // with normal tree display we send the current info to stdout
      ...
      if (showTree) {
        ...
        if (expandedAlready) {
          suffix += topLevelSet.has(packageName) ? " ".concat(suffixes.topLevel) : " ".concat(suffixes.expandedAbove);
        }
      ...
      if (showJson) {
        if (expandedAlready) {
          ...
          if (topLevelSet.has(packageName)) {
            parent[packageName] = "".concat(packageToPrint.version, "-").concat(suffixes.topLevel);
          } else {
            parent[packageName] = "".concat(packageToPrint.version, "-").concat(suffixes.expandedAbove);
          }
        } else {
          ...
          Object.entries(infoSource).forEach(_ref2 => {
            let [key, value] = _ref2;
      ...
      if (shouldExpand) {
        ...
        deps.forEach((dep, index) => {
          ...
          const child = packageDetails.get(dep); // with normal tree display we increase indentation
          ...
          if (showTree) {
            ...
            if (child) {
              ...
              Console.info("".concat(newIndent1, "\u2500 ").concat(dep, " ").concat(suffixes.weak, " ").concat(suffixes.skipped));
            } else {
              Console.info("".concat(newIndent1, "\u2500 ").concat(dep, " ").concat(suffixes.missing));
            }
          }
          ...
          if (showJson) {
            if (child) {
              ...
              parent[packageName].dependencies[dep] = "".concat(suffixes.weak, " ").concat(suffixes.skipped);
            } else {
  ...
  var anyBuiltLocally = false; // Iterate over packages that are used directly by this app (not indirect
  // dependencies).

  projectContext.projectConstraintsFile.eachConstraint(function (constraint) {
    var packageName = constraint.package; // Skip isobuild:* pseudo-packages.
    ...
    var versionRecord = projectContext.projectCatalog.getVersion(packageName, mapInfo.version);

    if (!versionRecord) {
      throw Error("no version record for " + packageName + "@" + mapInfo.version);
    }
  ...
  _.each(projectContext.cordovaPluginsFile.getPluginVersions(), function (version, name) {
    items.push({
      name: 'cordova:' + name,
      description: version
    });
  ...
  if (newVersionsAvailable) {
    Console.info();
    Console.info("New versions of these packages are available! Run", Console.command("'meteor update'"), "to try to update those", "packages to their latest versions. If your packages cannot be", "updated further, try typing", Console.command("`meteor add <package>@<newVersion>`"), "to see more information.", Console.options({
      bulletPoint: "* "
    }));
  }
  ...
  if (anyBuiltLocally) {
    Console.info();
    Console.info("These packages are built locally from source.", Console.options({
      bulletPoint: "+ "
    }));
  }
...
var getNewerVersion = function (packageName, curVersion, whichCatalog) {
  ...
  if (!latest) {
    ...
    throw Error("no latest record for package where we have a version? " + packageName);
  }

  var latestVersion = latest.version;

  if (curVersion !== latestVersion && // If we're currently running a prerelease, "latest" may be older than
...
var maybeUpdateRelease = function (options) {
  ...
  if (release.current && release.current.isCheckout()) {
    Console.error("You are running Meteor from a checkout, so we cannot update", "the Meteor release. Checking to see if we can update your packages.");
    return 0;
  ...
  var releaseTrack = release.current ? release.current.getReleaseTrack() : catalog.DEFAULT_TRACK; // Unless --release was passed (in which case we ought to already have
  ...
  if (!release.forced) {
    var latestRelease = release.latestKnown(releaseTrack); // Are we on some track without ANY recommended releases at all,
    // and the user ran 'meteor update' without specifying a release? We
    // really can't do much here.

    if (!latestRelease) {
      Console.error("There are no recommended releases on release track " + releaseTrack + ".");
      return 1;
    }

    if (release.current && !release.current.isRecommended() && options.appDir && !options.patch) {
      var releaseVersion = release.current.getReleaseVersion();
      var newerRecommendedReleases = getLaterReleaseVersions(releaseTrack, releaseVersion);
      ...
      if (!newerRecommendedReleases.length) {
        ...
        if (release.current.name === latestRelease) {
          Console.info("Already on the latest recommended release " + "(" + latestRelease + "). Not updating.");
        } else {
          Console.info("Not updating the release, because this app is at a " + "newer release (" + release.current.name + ") than " + "the latest recommended release " + "(" + latestRelease + ").");
        }
  ...
  updateMeteorToolSymlink(true); // If we're not in an app, then we're basically done. The only thing left to
  ...
  if (!options.appDir) {
    if (release.forced) {
      ...
      // help --release xyz'.
      Console.info("Installed. Run", Console.command("'meteor update --release " + release.current.getDisplayName({
        noPrefix: true
      }) + "'"), "inside of a particular project directory to update that project to", release.current.getDisplayName() + ".");
    } else {
      // We get here if the user ran 'meteor update' and we didn't
      // find a new version.
      Console.info("The latest version of Meteor, " + release.current.getReleaseVersion() + ", is already installed on this computer. Run " + Console.command("'meteor update'") + " inside of a particular " + "project directory to update that project to " + release.current.getDisplayName());
    }
  ...
  if (projectContext.releaseFile.fullReleaseName === release.current.name) {
    ...
    Console.info("This project is already at " + release.current.getDisplayName() + maybeTheLatestRelease + ".");
    return 0;
  ...
  if (options.patch) {
    ...
    if (!projectContext.releaseFile.normalReleaseSpecified()) {
      Console.error("Cannot patch update unless a release is set.");
      return 1;
    }

    var record = catalog.official.getReleaseVersion(projectContext.releaseFile.releaseTrack, projectContext.releaseFile.releaseVersion);

    if (!record) {
      Console.error("Cannot update to a patch release from an old release.");
      return 1;
    }
    ...
    var patchRecord = catalog.official.getReleaseVersion(projectContext.releaseFile.releaseTrack, updateTo); // It looks like you are not at the latest patch version,
    ...
    // than the app's.
    releaseVersion = getLaterReleaseVersions(projectContext.releaseFile.releaseTrack, projectContext.releaseFile.releaseVersion)[0];

    if (!releaseVersion) {
      // We could not find any releases newer than the one that we are on, on
      // that track, so we are done.
      Console.info("This project is already at " + Console.noWrap(projectContext.releaseFile.displayReleaseName) + ", which is newer than the latest release.");
      return 0;
    }
  }

  var releaseName = "".concat(releaseTrack, "@").concat(releaseVersion); // We could at this point springboard to solutionRelease (which is no newer
  ...
  var upgradersToRun = upgraders.upgradersToRun(projectContext); // Update every package in .meteor/packages to be (semver)>= the version
  ...
  projectContext.projectConstraintsFile.updateReleaseConstraints(releaseRecord); // Download and build packages and write the new versions to .meteor/versions.
  ...
  projectContext.packageMapDelta.displayOnConsole({
    title: "Changes to your project's package version selections from " + "updating the release:"
  });
  Console.info(files.pathBasename(options.appDir) + ": updated to " + projectContext.releaseFile.displayReleaseName + "."); // Now run the upgraders.
...
function getLaterReleaseVersions(releaseTrack, releaseVersion) {
  var releaseInfo = catalog.official.getReleaseVersion(releaseTrack, releaseVersion);
  var orderKey = releaseInfo && releaseInfo.orderKey || null;
  return catalog.official.getSortedRecommendedReleaseVersions(releaseTrack, orderKey);
}
...
main.registerCommand({
  ...
  disableNativeWatcher(); // If you are specifying packages individually, you probably don't want to
  // update the release.
  ...
  if (options["packages-only"] && options["patch"]) {
    Console.error("The --patch option only applies to the release, not packages.");
    return 1;
  }
  ...
  var releaseUpdateStatus = maybeUpdateRelease(options); // If we encountered an error and cannot proceed, return.
  ...
  var upgradePackageNames = []; // If no packages have been specified (`meteor update` with no positional
  ...
  if (options.args.length === 0) {
    ...
    if (options["all-packages"]) {
      upgradePackageNames = _.filter(Object.keys(projectContext.packageMapFile.getCachedVersions()), packageName => !compiler.isIsobuildFeaturePackage(packageName));
    }
    ...
    if (options["all-packages"]) {
      Console.error("You cannot both specify a list of packages to" + " update and pass --all-packages.");
      exit(1);
    }
  ...
  if (!files.inCheckout() && projectContext.releaseFile.normalReleaseSpecified()) {
    releaseRecordForConstraints = catalog.official.getReleaseVersion(projectContext.releaseFile.releaseTrack, projectContext.releaseFile.releaseVersion);

    if (!releaseRecordForConstraints) {
      throw Error("unknown release " + projectContext.releaseFile.displayReleaseName);
    }
  }

  const upgradePackagesWithoutCordova = upgradePackageNames.filter(name => name.split(':')[0] !== 'cordova');
  ...
  if (!_.isEqual(upgradePackagesWithoutCordova, upgradePackageNames)) {
    ...
    Console.warn();
    Console.warn(Console.command("meteor add cordova:PLUGIN-NAME@x.y.z"), Console.options({
      indent: 2
    }));
    Console.warn();
    Console.warn("The 'PLUGIN-NAME' should be an official plugin name", "(e.g. cordova-plugin-media) and the 'x.y.z' should be an available version of", "the plugin. The latest version can be found with the following command:");
    Console.warn();
    Console.warn(Console.command("meteor npm view PLUGIN-NAME version"), Console.options({
      indent: 2
    }));
    ...
    Console.warn(); // Exclude cordova packages
  ...
  main.captureAndExit("=> Errors while upgrading packages:", "upgrading packages", function () {
  ...
  if (projectContext.packageMapDelta.hasChanges()) {
    projectContext.packageMapDelta.displayOnConsole({
      title: "Changes to your project's package version selections from " + "updating package versions:"
    });
  } else if (options.args.length) {
    Console.info("The specified packages are at their latest compatible versions.");
  } else {
    Console.info("Your top-level dependencies are at their latest compatible versions.");
  }
  ...
  if (!options.args.length) {
    ...
    var printItem = function (rec) {
      Console.info(" * " + rec.name + " " + rec.selectedVersion + " (" + rec.latestVersion + " is available)");
    };
    ...
    if (nonlatestIndirectDeps.length) {
      ...
      Console.info(["These versions may not be compatible with your project.", "To update one or more of these packages to their latest", "compatible versions, pass their names to `meteor update`,", "or just run `meteor update --all-packages`.", "If the packages do not upgrade after this, this could mean", "that there is a newer version of Meteor which the package", "requires, but it not yet recommended or that some package", "dependencies are not up to date and don't allow you to get", "the latest package version."].join("\n"));
    }
  }
}); ///////////////////////////////////////////////////////////////////////////////
// admin run-upgrader
...
main.registerCommand({
  ...
  console.log("%s: running upgrader %s.", files.pathBasename(options.appDir), upgrader);
  upgraders.runUpgrader(projectContext, upgrader);
}); ///////////////////////////////////////////////////////////////////////////////
...
main.registerCommand({
  ...
  var allPackages = projectContext.localCatalog.getAllNonTestPackageNames();
  Console.info("Listing packages where the checkout version doesn't match the", "latest version on the package server.");
...
main.registerCommand({
  ...
  let exitCode = 0; // Split arguments into Cordova plugins and packages
    ...
    function cordovaPluginAdd() {
      ...
      for (target of pluginsToAdd) {
        ...
        if (!(version && utils.isValidVersion(version, {
          ...
          Console.error("".concat(id, ": Meteor requires either an exact version   (e.g. ").concat(id, "@1.0.0), a Git URL with a SHA reference, or a local path."));
          exitCode = 1;
        } else if (newId) {
          plugins[newId] = version;
          Console.info("Added Cordova plugin ".concat(newId, "@").concat(version, "   (plugin has been renamed as part of moving to npm)."));
          changed = true;
        } else {
          plugins[id] = version;
          Console.info("Added Cordova plugin ".concat(id, "@").concat(version, "."));
          changed = true;
        }
      }
  ...
  var infoMessages = [];
  var constraintsToAdd = []; // For every package name specified, add it to our list of package
  ...
  var messages = buildmessage.capture(function () {
    _.each(packagesToAdd, function (packageReq) {
      buildmessage.enterJob("adding package " + packageReq, function () {
        ...
        var packageRecord = projectContext.projectCatalog.getPackage(constraint.package);
        ...
        _.each(constraint.versionConstraint.alternatives, function (subConstr) {
          ...
          var versionRecord = projectContext.localCatalog.getVersion(constraint.package, subConstr.versionString);

          if (!versionRecord) {
            // XXX #2846 here's an example of something that might require a
            // refresh
            versionRecord = catalog.official.getVersion(constraint.package, subConstr.versionString);
          }

          if (!versionRecord) {
            buildmessage.error("no such version " + constraint.package + "@" + subConstr.versionString);
          }
        });
        ...
        var current = projectContext.projectConstraintsFile.getConstraint(constraint.package); // Check that the constraint is new. If we are already using the package
        ...
        if (!current) {
          ...
          infoMessages.push(constraint.package + " without a version constraint has already been added.");
        } else if (current.constraintString === constraint.constraintString) {
          infoMessages.push(constraint.package + " with version constraint " + constraint.constraintString + " has already been added.");
        } else {
          // We are changing an existing constraint.
          if (current.constraintString) {
            infoMessages.push("Currently using " + constraint.package + " with version constraint " + current.constraintString + ".");
          } else {
            infoMessages.push("Currently using " + constraint.package + " without any version constraint.");
          }

          if (constraint.constraintString) {
            infoMessages.push("The version constraint will be changed to " + constraint.constraintString + ".");
          } else {
  ...
  projectContext.projectConstraintsFile.addConstraints(constraintsToAdd); // Run the constraint solver, download packages, etc.
  ...
  projectContext.packageMapDelta.displayOnConsole(); // Show descriptions of directly added packages.

  Console.info();

  _.each(constraintsToAdd, function (constraint) {
    var version = projectContext.packageMap.getInfo(constraint.package).version;
    var versionRecord = projectContext.projectCatalog.getVersion(constraint.package, version);
    var deprecatedMessage = "";

    if (versionRecord.deprecated) {
      if (versionRecord.deprecatedMessage) {
        deprecatedMessage = " - DEPRECATED: ".concat(versionRecord.deprecatedMessage);
      } else {
        deprecatedMessage = ' - DEPRECATED';
      }
    }

    Console.info(constraint.package + (versionRecord.description ? ": " + versionRecord.description : "") + deprecatedMessage);
  });
...
main.registerCommand({
  ...
  if (!_.isEmpty(pluginsToRemove)) {
    function cordovaPluginRemove() {
      ...
      for (id of pluginsToRemove) {
        ...
        if (/@/.test(id)) {
          Console.error("".concat(id, ": do not specify version constraints."));
          exitCode = 1;
        } else if (_.has(plugins, id)) {
          delete plugins[id];
          Console.info("Removed Cordova plugin ".concat(id, "."));
          changed = true;
        } else if (newId && _.has(plugins, newId)) {
          delete plugins[newId];
          Console.info("Removed Cordova plugin ".concat(newId, "   (plugin has been renamed as part of moving to npm)."));
          changed = true;
        } else {
          Console.error("Cordova plugin ".concat(id, " is not in this project."));
          exitCode = 1;
        }
      }
  ...
  projectContext.projectConstraintsFile.removePackages(packagesToRemove); // Run the constraint solver, rebuild local packages, etc. This will write
  // our changes to .meteor/packages if it succeeds.

  main.captureAndExit("=> Errors after removing packages", function () {
    projectContext.prepareProjectForBuild();
  });
  projectContext.packageMapDelta.displayOnConsole(); // Log that we removed the constraints. It is possible that there are
...
main.registerCommand({
  ...
  var name = options.args[0]; // Yay, checking that options are correct.
  ...
  if (!options.list) {
    ...
    try {
      if (options.add) {
        ...
        if (fullRecord.release) {
          packageClient.callPackageServer(conn, 'addReleaseMaintainer', name, options.add);
        } else {
          packageClient.callPackageServer(conn, 'addMaintainer', name, options.add);
        }
      } else if (options.remove) {
        Console.info("Removing a maintainer from " + name + "...");

        if (fullRecord.release) {
          packageClient.callPackageServer(conn, 'removeReleaseMaintainer', name, options.remove);
        } else {
          packageClient.callPackageServer(conn, 'removeMaintainer', name, options.remove);
        }
    ...
    conn.close(); // Update the catalog so that we have this information, and find the record
  ...
  if (!record) {
    Console.info("Could not get list of maintainers:", "package " + name + " does not exist.");
    return 1;
  }
...
main.registerCommand({
  ...
  try {
    packageClient.callPackageServer(conn, 'setBannersOnReleases', bannersData.track, bannersData.banners);
  } catch (e) {
...
main.registerCommand({
  ...
  var url = options.args[1]; // Now let's get down to business! Fetching the thing.
  ...
  try {
    Console.rawInfo("Changing homepage on " + name + " to " + url + "...\n");
    packageClient.callPackageServer(conn, '_changePackageHomepage', name, url);
    Console.info(" done");
  } catch (err) {
    packageClient.handlePackageServerConnectionError(err);
    return 1;
...
main.registerCommand({
  ...
  try {
    var status = options.success ? "successfully" : "unsuccessfully"; // XXX: This should probably use progress bars instead.

    _.each(versions, function (version) {
      Console.rawInfo("Setting " + name + "@" + version + " as " + status + " migrated ...\n");
      packageClient.callPackageServer(conn, '_changeVersionMigrationStatus', name, version, !options.success);
      Console.info("done.");
    });
  } catch (err) {
    packageClient.handlePackageServerConnectionError(err);
...
//# sourceMappingURL=commands-packages.js.map