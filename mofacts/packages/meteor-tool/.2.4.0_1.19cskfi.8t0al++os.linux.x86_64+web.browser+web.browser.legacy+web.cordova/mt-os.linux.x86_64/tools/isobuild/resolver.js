!function (module1) {
  ...
  let pathJoin, pathRelative, pathNormalize, pathDirname, pathBasename, convertToOSPath, convertToPosixPath;
  ...
  let optimisticStatOrNull, optimisticReadJsonOrNull;
  ...
  class Resolver {
    constructor(_ref) {
      ...
      _defineProperty(this, "targetArch", void 0);
      ...
      _defineProperty(this, "statOrNull", optimisticStatOrNull);
      ...
      this.findPkgJsonSubsetForPath = wrap(path => findPkgJsonSubsetForPath.call(this, path));
      this.getPkgJsonSubsetForDir = wrap(path => getPkgJsonSubsetForDir.call(this, path));
    ...
    resolve(id, absParentPath, _seenDirPaths) {
      let resolved = this.resolveAbsolute(id, absParentPath) || this.resolveRelative(id, absParentPath) || this.resolveNodeModule(id, absParentPath);
      ...
      while (resolved && resolved.stat && resolved.stat.isDirectory()) {
        ...
        _seenDirPaths = _seenDirPaths || new Set(); // If the "main" field of a package.json file resolves to a
        ...
        if (!_seenDirPaths.has(dirPath)) {
          ...
          const foundPkgJsonMain = found && this.mainFields.some(name => {
            ...
            if (isString(value)) {
              ...
              resolved = this.joinAndStat(dirPath, value) || this.resolve(value, found.path, _seenDirPaths);
          ...
          if (foundPkgJsonMain && found) {
            ...
            resolved.packageJsonMap[found.path] = found.pkg; // The resolution above may have returned a directory, so we
      ...
      if (resolved) {
        ...
        if (pkgJsonInfo && isObject(pkgJsonInfo.pkg.browser)) {
          if (!resolved.packageJsonMap) {
            resolved.packageJsonMap = Object.create(null);
          }
    ...
    resolveAbsolute(id, _absParentPath) {
      return id.charAt(0) === "/" && this.joinAndStat(this.sourceRoot, id.slice(1)) || null;
    }
    ...
    getPkgJsonSubsetForDir(dirPath) {
  ...
  _defineProperty(Resolver, "getOrCreate", wrap(function (options) {
  ...
  ;
  const Rp = Resolver.prototype;
  Object.keys(Rp).forEach(key => {
    if (key === "constructor") return;
    Rp[key] = Profile("Resolver#".concat(key), Rp[key]);
  });
}.call(this, module);
