...
class IsopackCache {
  constructor(options) {
    var self = this;
    options = options || {}; // cacheDir may be null; in this case, we just don't ever save things to disk.

    self.cacheDir = options.cacheDir; // Root directory for caches used by build plugins.  Can be null, in which
    ...
    self._pluginCacheDirRoot = options.pluginCacheDirRoot; // This is a bit of a hack, but basically: we really don't want to spend time
    ...
    self._includeCordovaUnibuild = !!options.includeCordovaUnibuild; // Defines the versions of packages that we build. Must be set.

    self._packageMap = options.packageMap; // tropohouse may be null; in this case, we can't load versioned packages.
    // eg, for building isopackets.

    self._tropohouse = options.tropohouse; // If provided, this is another IsopackCache for the same cache dir; when
    ...
    if (self._previousIsopackCache && self._previousIsopackCache.cacheDir !== self.cacheDir) {
  ...
  getSourceRoot(name, arch) {
    ...
    if (packageInfo) {
      ...
      if (packageInfo.kind === "versioned") {
        const isopackPath = this._tropohouse.packagePath(name, packageInfo.version);
  ...
  uses(isopack, name, arch) {
    ...
    return _.some(unibuild.uses, use => {
      return this.implies(this._isopacks[use.package], name, arch);
    });
  }
  ...
  implies(isopack, name, arch) {
    ...
    return _.some(unibuild.implies, imp => {
      return this.implies(this._isopacks[imp.package], name, arch);
    });
  }
  ...
  _ensurePackageLoaded(name, onStack) {
    ...
    var ensureLoaded = function (depName) {
      if (_.has(onStack, depName)) {
        buildmessage.error("circular dependency between packages " + name + " and " + depName); // recover by not enforcing one of the dependencies
    ...
    if (self._previousIsopackCache && _.has(self._previousIsopackCache._isopacks, name)) {
      var previousInfo = self._previousIsopackCache._packageMap.getInfo(name);

      if (packageInfo.kind === 'versioned' && previousInfo.kind === 'versioned' && packageInfo.version === previousInfo.version || packageInfo.kind === 'local' && previousInfo.kind === 'local' && packageInfo.packageSource.sourceRoot === previousInfo.packageSource.sourceRoot) {
        previousIsopack = self._previousIsopackCache._isopacks[name];
      }
    }

    if (packageInfo.kind === 'local') {
      var packageNames = packageInfo.packageSource.getPackagesToLoadFirst(self._packageMap);
      ...
      Profile.time('IsopackCache Load local isopack', () => {
        ...
        if (!isopack) {
          ...
          buildmessage.enterJob("loading package " + name + "@" + packageInfo.version, function () {
            var pluginCacheDir;

            if (self._pluginCacheDirRoot) {
              pluginCacheDir = self._pluginCacheDirForVersion(name, packageInfo.version);
              files.mkdir_p(pluginCacheDir);
            }

            var isopackPath = self._tropohouse.packagePath(name, packageInfo.version);
      ...
      self._isopacks[name] = isopack; // Also load its dependencies. This is so that if this package is being
  ...
  _loadLocalPackage(name, packageInfo, previousIsopack) {
    ...
    buildmessage.enterJob("building package " + name, function () {
      ...
      if (previousIsopack && self._checkUpToDatePreloaded(previousIsopack)) {
        isopack = previousIsopack; // We don't need to call self._lintLocalPackage here, because
        ...
        var isopackBuildInfoJson = self.cacheDir && files.readJSONOrNull(self._isopackBuildInfoPath(name));
        ...
        if (upToDate) {
          ...
          isopack.setPluginProviderPackageMap(self._packageMap.makeSubsetMap(Object.keys(isopackBuildInfoJson.pluginProviderPackageMap))); // Because we don't save linter messages to disk, we have to relint
          // this package.
  ...
  _checkUpToDate(isopackBuildInfoJson) {
    var self = this; // If there isn't an isopack-buildinfo.json file, then we definitely aren't
    // up to date!
    ...
    if (self._includeCordovaUnibuild !== isopackBuildInfoJson.includeCordovaUnibuild) {
      return false;
    ...
    if (!self._packageMap.isSupersetOfJSON(isopackBuildInfoJson.pluginProviderPackageMap)) {
      return false;
    } // Merge in the watchsets for all unibuilds and plugins in the package, then
    // check it once.


    var watchSet = watch.WatchSet.fromJSON(isopackBuildInfoJson.pluginDependencies);
  ...
  _checkUpToDatePreloaded(previousIsopack) {
    var self = this; // If we include Cordova but this Isopack doesn't, or via versa, then we're
    // not up to date.
    ...
    if (!self._packageMap.isSupersetOfJSON(previousIsopack.pluginProviderPackageMap.toJSON())) {
      return false;
    } // Merge in the watchsets for all unibuilds and plugins in the package, then
    // check it once.
  ...
  _pluginCacheDirForPackage(packageName) {
    var self = this;
    return files.pathJoin(self._pluginCacheDirRoot, colonConverter.convert(packageName));
  }

  _pluginCacheDirForVersion(packageName, version) {
    var self = this;
    return files.pathJoin(self._pluginCacheDirForPackage(packageName), version);
  }

  _pluginCacheDirForLocal(packageName) {
    var self = this; // assumes that `local` is not a valid package version.

    return files.pathJoin(self._pluginCacheDirForPackage(packageName), 'local');
  }

  _isopackBuildInfoPath(packageName) {
    var self = this;
    return files.pathJoin(self._isopackDir(packageName), 'isopack-buildinfo.json');
  }
...
"_ensurePackageLoaded"].forEach(method => {
  ICp[method] = Profile(packageName => method + "(" + packageName + ")", ICp[method]);
});
//# sourceMappingURL=isopack-cache.js.map