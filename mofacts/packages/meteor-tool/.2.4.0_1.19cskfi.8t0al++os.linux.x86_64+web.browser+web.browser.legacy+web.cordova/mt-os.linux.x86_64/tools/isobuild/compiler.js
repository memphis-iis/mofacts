!function (module1) {
  ...
  module1.export({
    getActivePluginPackages: () => getActivePluginPackages,
    isIsobuildFeaturePackage: () => isIsobuildFeaturePackage
  });
  let SourceProcessorSet;
  module1.link("./build-plugin.js", {
    SourceProcessorSet(v) {
      SourceProcessorSet = v;
    }

  }, 0);
  let NodeModulesDirectory, buildJsImage;
  module1.link("./bundler.js", {
    NodeModulesDirectory(v) {
      NodeModulesDirectory = v;
    },
  ...
  var compiler = exports; // Whenever you change anything about the code that generates isopacks, bump
  ...
  compiler.BUILT_BY = 'meteor/34'; // This is a list of all possible architectures that a build can target. (Client
  ...
  compiler.compile = Profile(function (packageSource, options) {
    return "compiler.compile(".concat(packageSource.name || 'the app', ")");
    ...
    var plugins = {};
    var pluginProviderPackageNames = {}; // Build plugins
    ...
    _.each(packageSource.architectures, function (architecture) {
      ...
      files.withCache(() => {
        ...
        Object.assign(pluginProviderPackageNames, unibuildResult.pluginProviderPackageNames);
      });
    });

    if (options.includePluginProviderPackageMap) {
      isopk.setPluginProviderPackageMap(packageMap.makeSubsetMap(Object.keys(pluginProviderPackageNames)));
    }
  ...
  compiler.lint = Profile(function (packageSource, options) {
    return "compiler.lint(".concat(packageSource.name || 'the app', ")");
  }, function (packageSource, options) {
  ...
  compiler.getMinifiers = function (packageSource, options) {
    ...
    minifiers = _.uniq(minifiers); // check for extension-wise uniqness
  ...
  function getLinterSourceProcessorSet(_ref) {
    ...
    buildmessage.assertInJob();
    const sourceProcessorSet = new SourceProcessorSet(isopack.displayName, {
      allowConflicts: true
    });
  ...
  var lintUnibuild = function (_ref2) {
    let {
      isopack,
      isopackCache,
      sourceArch
    ...
    const sourceProcessorSet = getLinterSourceProcessorSet({
      isopack,
      activePluginPackages
    ...
    const unibuild = _.find(isopack.unibuilds, unibuild => archinfo.matches(unibuild.arch, sourceArch.arch));

    if (!unibuild) {
      throw Error("No ".concat(sourceArch.arch, " unibuild for ").concat(isopack.name, "!"));
    }

    const {
      sources
    } = sourceArch.getFiles(sourceProcessorSet, unibuild.watchSet);
  ...
  var compileUnibuild = Profile(function (options) {
    return "compileUnibuild (".concat(options.isopack.name || 'the app', ")");
  }, function (options) {
    buildmessage.assertInCapture();
    ...
    const watchSet = inputSourceArch.watchSet.clone(); // *** Determine and load active plugins
    ...
    buildmessage.enterJob("determining active plugins", () => {
      sourceProcessorSet = new SourceProcessorSet(isopk.displayName(), {
        hardcodeJs: true
      });
      activePluginPackages.forEach(otherPkg => {
        otherPkg.ensurePluginsInitialized(); // Note that this may log a buildmessage if there are conflicts.
    ...
    const sourceProcessorFiles = sourceProcessorSet ? inputSourceArch.getFiles(sourceProcessorSet, watchSet) : {};
    ...
    _.each(inputSourceArch.localNodeModulesDirs, (info, dir) => {
      addNodeModulesDirectory(_objectSpread({
    ...
    if (nodeModulesPath) {
      ...
      const shrinkwrapPath = nodeModulesPath.replace(/node_modules$/, 'npm-shrinkwrap.json');
    ...
    _.values(sources).forEach(source => {
      ...
      const absPath = files.pathResolve(inputSourceArch.sourceRoot, relPath);
      const filename = files.pathBasename(relPath); // Find the handler for source files with this extension
      ...
      if (classification.type === 'unmatched') {
        ...
        const linterClassification = linterSourceProcessorSet.classifyFilename(filename, inputSourceArch.arch);
        ...
        buildmessage.error("No plugin known to handle file '".concat(relPath, "'. If you want this file to be a static asset, use addAssets instead of addFiles; eg, api.addAssets('").concat(relPath, "', 'client').")); // recover by ignoring
      ...
      try {
        Profile.time("legacy handler (.".concat(classification.extension, ")"), () => {
          handler(compileStep);
        });
      } catch (e) {
        e.message = e.message + " (compiling " + relPath + ")";
        buildmessage.exception(e); // Recover by ignoring this source file (as best we can -- the
    ...
    if (!process.env.METEOR_FORCE_PORTABLE) {
      ...
      _.each(nodeModulesDirectories, nmd => {
        if (nmd.local) {// Meteor never attempts to modify the contents of local
  ...
  function runLinters(_ref3) {
    ...
    const whichArch = inputSourceArch.arch === 'os' ? archinfo.host() : inputSourceArch.arch; // For linters, figure out what are the global imports from other packages
    ...
    _.values(sources).forEach(sourceItem => {
      ...
      const classification = sourceProcessorSet.classifyFilename(files.pathBasename(relPath), inputSourceArch.arch); // If we don't have a linter for this file (or we do but it's only on
      ...
      if (classification.type !== "meteor-ignore" && !classification.sourceProcessors) {
        throw Error("Unexpected classification for ".concat(relPath, ": ").concat(classification.type));
      ...
      const {
        hash,
        contents
      } = watch.readAndWatchFileWithHash(watchSet, files.pathResolve(inputSourceArch.sourceRoot, relPath));
    ...
    _.each(sourceItemsForLinter, _ref4 => {
      let {
        sourceProcessor,
        sources
      } = _ref4;
      const sourcesToLint = sources.map(wrappedSource => new linterPluginModule.LintingFile(wrappedSource));
      const markedLinter = buildmessage.markBoundary(sourceProcessor.userPlugin.processFilesForPackage, sourceProcessor.userPlugin);
  ...
  function getActivePluginPackages(isopk, _ref5) {
    ...
    compiler.eachUsedUnibuild({
      ...
      arch: archinfo.host(),
      isopackCache: isopackCache,
      skipUnordered: true // implicitly skip weak deps by not specifying acceptableWeakPackages option
  ...
  compiler.eachUsedUnibuild = function (options, callback) {
    ...
    while (!_.isEmpty(usesToProcess)) {
      var use = usesToProcess.shift(); // We only care about real packages, not isobuild:* psuedo-packages.

      if (isIsobuildFeaturePackage(use.package)) {
        continue;
      }

      var usedPackage = isopackCache.getIsopack(use.package); // Ignore this package if we were told to skip debug-only packages and it is
      // debug-only.
  ...
  class SourceResource {
    constructor(_ref6) {
      ...
      _defineProperty(this, "type", "source");

      this.type = "source";
      this.extension = extension || null;
      this.usesDefaultSourceProcessor = usesDefaultSourceProcessor;
      this.path = path;
      this.fileOptions = fileOptions; // Is set to true if the resource's hash or data is accessed, which can be
