!function (module1) {
  ...
  module1.link("@babel/runtime/helpers/defineProperty", {
    default(v) {
      _defineProperty = v;
    }
  ...
  module1.link("./package-namespace.js", {
    PackageNamespace(v) {
      PackageNamespace = v;
    }
  ...
  let PackageCordova;
  module1.link("./package-cordova.js", {
    PackageCordova(v) {
      PackageCordova = v;
    }
  ...
  let PackageAPI;
  module1.link("./package-api.js", {
    PackageAPI(v) {
      PackageAPI = v;
    }

  }, 5);
  let TEST_FILENAME_REGEXPS, APP_TEST_FILENAME_REGEXPS, isTestFilePath;
  module1.link("./test-files", {
    TEST_FILENAME_REGEXPS(v) {
      TEST_FILENAME_REGEXPS = v;
    },

    APP_TEST_FILENAME_REGEXPS(v) {
      APP_TEST_FILENAME_REGEXPS = v;
    },
  ...
  let convertColonsInPath;
  module1.link("../utils/colon-converter.js", {
    convert(v) {
      convertColonsInPath = v;
    }

  }, 7);
  let optimisticReadFile, optimisticHashOrNull, optimisticStatOrNull, optimisticReadMeteorIgnore, optimisticLookupPackageJsonArray;
  module1.link("../fs/optimistic", {
    ...
    optimisticReadMeteorIgnore(v) {
      optimisticReadMeteorIgnore = v;
    },

    optimisticLookupPackageJsonArray(v) {
      optimisticLookupPackageJsonArray = v;
    }
  ...
  var loadOrderSort = function (sourceProcessorSet, arch) {
    const isTemplate = _.memoize(filename => {
      ...
      switch (classification.type) {
        ...
        default:
          throw Error("Surprising type ".concat(classification.type, " for ").concat(filename));
      }
    });

    return function (a, b) {
      const aBasename = files.pathBasename(a);
      const bBasename = files.pathBasename(b); // XXX MODERATELY SIZED HACK --
      ...
      var b_parts = b.split(files.pathSep); // deeper paths loaded first.
  ...
  var getExcerptFromReadme = function (text) {
    ...
    var parsed = reader.parse(text); // Commonmark will parse the Markdown into an array of nodes. These are the
    // nodes that represent the text between the first and second heading.

    var relevantNodes = []; // Go through the document until we get the nodes that we are looking for,
    // then stop.

    _.any(parsed.children, function (child) {
      var isHeader = child.t === "Header"; // Don't excerpt anything before the first header.
    ...
    var stop = _.last(relevantNodes).end_line; // XXX: There is a bug in commonmark that happens when processing the last
  ...
  class SymlinkLoopChecker {
    constructor(sourceRoot) {
      _defineProperty(this, "_realpath", Profile('_realpath', function (relDir) {
    ...
    check(relDir) {
      let quietly = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  var PackageSource = function () {
    var self = this; // The name of the package, or null for an app pseudo-package or
    ...
    self.name = null; // The path relative to which all source file paths are interpreted
    // in this package. Also used to compute the location of the
    // package's .npm directory (npm shrinkwrap state).

    self.sourceRoot = null; // Path that will be prepended to the URLs of all resources emitted
    ...
    self.serveRoot = null; // Package metadata. Keys are 'summary', 'git' and 'documentation'. Currently
    // all of these are optional.

    self.metadata = {};
    self.docsExplicitlyProvided = false; // Package version as a meteor-version string. Optional; not all packages
    ...
    self.version = null;
    self.versionExplicitlyProvided = false; // Available architectures of this package. Array of SourceArch.

    self.architectures = []; // The information necessary to build the plugins in this
    // package. Map from plugin name to object with keys 'name', 'use',
    // 'sources', and 'npmDependencies'.

    self.pluginInfo = {}; // Analogous to watchSet in SourceArch but for plugins. At this
    // stage will typically contain just 'package.js'.

    self.pluginWatchSet = new watch.WatchSet(); // npm packages used by this package (on os.* architectures only).
    // Map from npm package name to the required version of the package
    // as a string.

    self.npmDependencies = {}; // Files to be stripped from the installed NPM dependency tree. See the
    // Npm.strip comment below for further usage information.

    self.npmDiscards = null; // Absolute path to a directory on disk that serves as a cache for
    ...
    self.npmCacheDirectory = null; // cordova plugins used by this package (on os.* architectures only).
    // Map from cordova plugin name to the required version of the package
    // as a string.

    self.cordovaDependencies = {}; // If this package has a corresponding test package (for example,
    // underscore-test), defined in the same package.js file, store its value
    // here.

    self.testName = null; // Test packages are dealt with differently in the linker (and not published
    // to the catalog), so we need to keep track of them.

    self.isTest = false; // Some packages belong to a test framework and should never be bundled into
    // production. A package with this flag should not be picked up by the bundler
    // for production builds.

    self.debugOnly = false; // A package marked prodOnly is ONLY picked up by the bundler for production
    // builds.

    self.prodOnly = false; // A package marked testOnly is ONLY picked up by the bundler as
    // part of the `meteor test` command.

    self.testOnly = false; // If this is set, we will take the currently running git checkout and bundle
    // the meteor tool from it inside this package as a tool. We will include
    // built copies of all known isopackets.

    self.includeTool = false; // Is this a core package? Core packages don't record version files, because
    ...
    self.isCore = false; // Flags for Atmosphere and developers to mark if deprecated packages
  ...
  Object.assign(PackageSource.prototype, {
    ...
    initFromOptions: function (name, options) {
      ...
      self.sourceRoot = options.sourceRoot || files.pathSep; // serveRoot is actually a part of a url path, root here is a forward slash

      self.serveRoot = options.serveRoot || '/';
      utils.ensureOnlyValidVersions(options.npmDependencies, {
        forCordova: false
      });
      self.npmDependencies = options.npmDependencies; // If options.npmDir is a string, make sure it contains no colons.

      self.npmCacheDirectory = _.isString(options.npmDir) ? convertColonsInPath(options.npmDir) : options.npmDir;
      utils.ensureOnlyValidVersions(options.cordovaDependencies, {
        forCordova: true
      });
      ...
      if (options.localNodeModulesDirs) {
        Object.assign(sourceArch.localNodeModulesDirs, options.localNodeModulesDirs);
      }
    ...
    initFromPackageDir: Profile((dir, options) => {
      return "PackageSource#initFromPackageDir for ".concat((options === null || options === void 0 ? void 0 : options.name) || dir.split(files.pathSep).pop());
    }, function (dir, options) {
      var self = this;
      buildmessage.assertInCapture();
      var isPortable = true;
      options = options || {};
      var initFromPackageDirOptions = options; // If we know what package we are initializing, we pass in a
      ...
      self.version = "0.0.0"; // To make the transition to using README.md files in Isobuild easier, we
      // initialize the documentation directory to README.md by default.

      self.metadata.documentation = "README.md";
      self.sourceRoot = dir; // If we are running from checkout we may be looking at a core package. If
      ...
      try {
        ...
        buildmessage.exception(e); // Could be a syntax error or an exception. Recover by
      ...
      try {
        ...
        buildmessage.error(e.message); // recover by ignoring
      ...
      if ((self.debugOnly || self.prodOnly || self.testOnly) && !_.isEmpty(self.pluginInfo)) {
        buildmessage.error("can't register build plugins in debugOnly, prodOnly or testOnly packages"); // recover by ignoring
      ...
      if (Package._fileAndDepLoader) {
        try {
          ...
          buildmessage.exception(e); // Recover by ignoring all of the source files in the
      ...
      if (!self._checkCrossUnibuildVersionConstraints()) {// A build error was written. Recover by ignoring the
      ...
      self.cordovaDependencies = Cordova._dependencies; // Create source architectures, one for the server and one for each web
      // arch.
      ...
      _.each(compiler.ALL_ARCHES, function (arch) {
        ...
        self.architectures.push(new SourceArch(self, {
          ...
          getFiles(sourceProcessorSet, watchSet) {
            ...
            const sources = result.sources; // Files explicitly passed to api.addFiles remain at the
            ...
            self._findSources({
              ...
              if (source) {
                const fileOptions = source.fileOptions || (source.fileOptions = Object.create(null)); // Since this file was explicitly added with api.addFiles or
                ...
                const fileOptions = Object.create(null); // Since this file was not explicitly added with
      ...
      self.serveRoot = files.pathJoin('/packages/', self.name); // Name of the test.
    ...
    _readAndWatchDirectory(relDir, watchSet, _ref) {
      let {
        include,
        exclude,
        names
    ...
    initFromAppDir: Profile("initFromAppDir", function (projectContext, ignoreFiles) {
      ...
      self.serveRoot = '/'; // Determine used packages. Note that these are the same for all arches,
      ...
      const mainModulesByArch = projectContext.meteorConfig.getMainModulesByArch();
      const testModulesByArch = projectContext.meteorConfig.getTestModulesByArch();
      const nodeModulesToRecompileByArch = projectContext.meteorConfig.getNodeModulesToRecompileByArch();
      ...
      _.each(compiler.ALL_ARCHES, function (arch) {
        ...
        const mainModule = projectContext.meteorConfig.getMainModule(arch, mainModulesByArch);
        const testModule = projectContext.meteorConfig.getTestModule(arch, testModulesByArch);
        const nodeModulesToRecompile = projectContext.meteorConfig.getNodeModulesToRecompile(arch, nodeModulesToRecompileByArch); // XXX what about /web.browser/* etc, these directories could also
        ...
        var sourceArch = new SourceArch(self, {
          ...
          getFiles(sourceProcessorSet, watchSet) {
            ...
            const sources = self._findSources(findOptions).sort(loadOrderSort(sourceProcessorSet, arch)).map(relPath => {
            ...
            if (missingMainModule) {
              buildmessage.error(["Could not find mainModule for '" + arch + "' architecture: " + mainModule, 'Check the "meteor" section of your package.json file?'].join("\n"));
            }
        ...
        const origNodeModulesDir = origAppDir && files.pathJoin(origAppDir, "node_modules");
        const origNodeModulesStat = origNodeModulesDir && files.statOrNull(origNodeModulesDir);
        ...
        self.architectures.push(sourceArch); // sourceArch's WatchSet should include all the project metadata files
    ...
    _inferAppFileOptions(relPath, _ref2) {
      let {
        arch,
        mainModule,
        testModule
      ...
      if (isTest && !isTestFile) {
        fileOptions.lazy = true; // Ignore any meteor.mainModule that was specified, since we're
      ...
      for (var i = 0; i < dirs.length; ++i) {
        ...
        if (dir === "node_modules") {
          fileOptions.lazy = true; // We used to disable transpilation for modules within node_modules,
        ...
        if (i > 0 && dirs[i - 1] === "client" && dir === "compatibility" && archinfo.matches(arch, "web") && relPath.endsWith(".js")) {
    ...
    _findSources: Profile(_ref3 => {
      let {
        sourceArch
      } = _ref3;
      return "PackageSource#_findSources for ".concat(sourceArch.arch);
    }, function (_ref4) {
      let {
        sourceProcessorSet,
        watchSet,
      ...
      const sourceReadOptions = sourceProcessorSet.appReadDirectoryOptions(arch); // Adding, removing, or modifying a .meteorignore file should trigger
      // a rebuild with the new rules applied.

      sourceReadOptions.names.push(".meteorignore"); // Ignore files starting with dot (unless they are explicitly in
      // sourceReadOptions.names, e.g. .meteorignore, added above).

      sourceReadOptions.exclude.push(/^\./); // Ignore the usual ignorable files.

      sourceReadOptions.exclude.push(...ignoreFiles); // Read top-level source files, excluding control files that were not
      // explicitly included.
      ...
      if (testModule || !isApp) {// If we have a meteor.testModule from package.json, then we don't
        ...
        if (isTest || isAppTest) {
          ...
          sourceReadOptions.exclude.push(...APP_TEST_FILENAME_REGEXPS, ...TEST_FILENAME_REGEXPS);
        }
      }
      ...
      if (isApp) {
        ...
        anyLevelExcludes.push(archinfo.matches(arch, "os") ? /^client\/$/ : /^server\/$/);
      }

      anyLevelExcludes.push(...sourceReadOptions.exclude);
      const topLevelExcludes = isApp ? [...anyLevelExcludes, /^packages\/$/, /^programs\/$/, /^public\/$/, /^private\/$/, /^cordova-build-override\/$/, /^acceptance-tests\/$/] : anyLevelExcludes;

      const nodeModulesReadOptions = _objectSpread(_objectSpread({}, sourceReadOptions), {}, {
      ...
      function find(dir, depth) {
        ...
        dir = dir.replace(/\/$/, ""); // If we're in a node_modules directory, cache the results of the
        ...
        if (inNodeModules) {
          ...
          const pkgJsonArray = optimisticLookupPackageJsonArray(self.sourceRoot, dir); // If a package.json file with a "name" property is found, it will
          // always be the first in the array.

          const pkgJson = pkgJsonArray[0];

          if (pkgJson && pkgJson.name && nodeModulesToRecompile.has(pkgJson.name)) {
        ...
        const sources = _.difference(self._readAndWatchDirectory(dir, inNodeModules ? null : watchSet, readOptions), depth > 0 ? [] : controlFiles);

        const subdirectories = self._readAndWatchDirectory(dir, inNodeModules ? null : watchSet, {
          include: [/\/$/],
        ...
        subdirectories.forEach(subdir => {
          if (/(^|\/)node_modules\/$/.test(subdir)) {
            ...
            nodeModulesDir = subdir; // A "local" node_modules directory is one that's managed by the
        ...
        if (nodeModulesDir && (!inNodeModules || sources.length > 0)) {
          ...
          // the app (#6037).
          sources.push(...find(nodeModulesDir, depth + 1, {
            inNodeModules: true,
            cache: !inNodeModules
          }));
        }
    ...
    _findAssets(_ref5) {
      ...
      if (!_.isEmpty(assetDirs)) {
        ...
        while (!_.isEmpty(assetDirs)) {
          var dir = assetDirs.shift(); // remove trailing slash
    ...
    getExports: function () {
      var self = this;
      var ret = {}; // Go over all of the architectures, and aggregate the exports together.
    ...
    processReadme: function () {
      ...
      var ret = {};
      ret.path = files.pathJoin(self.sourceRoot, self.metadata.documentation); // Read in the text of the Readme.
      ...
      try {
        ...
        buildmessage.error(errorMessage); // Recover by returning null
    ...
    _checkCrossUnibuildVersionConstraints: function () {
      var self = this;
      return !!self._computeDependencyMetadata({
        logError: true
      });
    },
      ...
      if (failed && options.logError) {
        _.each(allConstraints, function (constraints, name) {
          ...
          if (constraints.length > 1) {
            buildmessage.error("The version constraint for a dependency must be the same " + "at every place it is mentioned in a package. " + "'" + name + "' is constrained both as " + constraints.join(' and ') + ". Change them to match."); // recover by returning false (the caller had better detect
