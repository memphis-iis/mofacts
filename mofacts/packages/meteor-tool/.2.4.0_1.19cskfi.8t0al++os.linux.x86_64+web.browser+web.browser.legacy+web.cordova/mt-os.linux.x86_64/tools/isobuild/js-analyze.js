!function (module1) {
  module1.export({
    findImportedModuleIdentifiers: () => findImportedModuleIdentifiers,
    findAssignedGlobals: () => findAssignedGlobals
  });
  let parse;
  module1.link("@meteorjs/babel", {
    parse(v) {
      parse = v;
    }
  ...
  let Visitor;
  module1.link("reify/lib/visitor.js", {
    default(v) {
      Visitor = v;
    }
  ...
  module1.link("reify/lib/utils.js", {
  ...
  function findImportedModuleIdentifiers(source, hash) {
    const possibleIndexes = findPossibleIndexes(source, ["require", "import", "export", "dynamicImport", "link"]);
  ...
  const importedIdentifierVisitor = new class extends Visitor {
    reset(rootPath, code, possibleIndexes) {
      ...
      this.identifiers = Object.create(null); // Defining this.possibleIndexes causes the Visitor to ignore any
    ...
    addIdentifier(id, type, dynamic) {
      const entry = hasOwn.call(this.identifiers, id) ? this.identifiers[id] : this.identifiers[id] = {
      ...
      if (type === "require") {
        ...
        entry.possiblySpurious = entry.possiblySpurious && this.requireIsBound;
      } else {
    ...
    visitCallExpression(path) {
      ...
      } else if (node.callee.type === "Import" || isIdWithName(node.callee, "import")) {
        this.addIdentifier(firstArg.value, "import", true);
      } else if (node.callee.type === "MemberExpression" && // The Reify compiler sometimes renames references to the
      ...
      isIdWithName(node.callee.object, /^module\d*$/)) {
        const propertyName = isPropertyWithName(node.callee.property, "link") || isPropertyWithName(node.callee.property, "dynamicImport");

        if (propertyName) {
          this.addIdentifier(firstArg.value, "import", propertyName === "dynamicImport");
        }
      }
    }
    ...
    _importExportSourceHelper(path) {
      const node = path.getValue(); // The .source of an ImportDeclaration or Export{Named,All}Declaration
  ...
  function isPropertyWithName(node, name) {
    if (isIdWithName(node, name) || isStringLiteral(node) && node.value === name) {
      return name;
    }
  ...
  function findAssignedGlobals(source, hash) {
    if (hash && globalsCache.has(hash)) {
      return globalsCache.get(hash);
    }

    const ast = tryToParse(source, hash); // We have to pass ignoreEval; otherwise, the existence of a direct eval call
    ...
    const assignedGlobals = {}; // Passing {sourceType: "module"} to analyzeScope leaves this list
    ...
    programScope.implicit.left.forEach(entry => {
      if (entry.identifier && entry.identifier.type === "Identifier" && // Only consider identifers that are assigned a value.
      entry.writeExpr) {
