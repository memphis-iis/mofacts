...
let OutputLog;
module.link("./output-log.js", {
  default(v) {
    OutputLog = v;
  }

}, 5);
let randomPort, timeoutScaleFactor, sleepMs;
...
class Run {
  constructor(execPath, options) {
    ...
    this.cwd = options.cwd || files.convertToStandardPath(process.cwd()); // default env variables

    this.env = Object.assign({
      SELFTEST: "t",
      METEOR_NO_WORDWRAP: "t"
    }, options.env);
    this._args = [];
    this.proc = null;
    this.baseTimeout = 20;
  ...
  args() {
    ...
    args.forEach(a => {
      if (typeof a !== "object") {
        this._args.push("".concat(a));
      } else {
        Object.keys(a).forEach(key => {
          const value = a[key];

          this._args.push("--".concat(key));

          this._args.push("".concat(value));
        });
      }
    });
  }
  ...
  _endMatchers() {
    this.matcherEndPromise = this.matcherEndPromise || Promise.all([this.stdoutMatcher.endAsync(), this.stderrMatcher.endAsync()]);
  ...
  _ensureStarted() {
    ...
    this.proc = spawn(files.convertToOSPath(this.execPath), this._args, {
  ...
  expectExit(code) {
    ...
    if (this.exitStatus === undefined) {
      ...
      this.extraTime = 0;
      let timer;
      const failure = new TestFailure('exit-timeout', {
        run: this
      });
      const promise = new Promise((resolve, reject) => {
        this.exitPromiseResolvers.push(resolve);
        timer = setTimeout(() => {
          this.exitPromiseResolvers = this.exitPromiseResolvers.filter(r => r !== resolve);
          reject(failure);
        }, timeout * 1000);
      });
  ...
  tellMongo(command) {
    ...
    this._ensureStarted(); // If it's the first time we've called tellMongo on this sandbox,
    ...
    if (!this.fakeMongoConnection) {
      ...
      for (let attempts = 0; !this.fakeMongoConnection && attempts < 600; attempts++) {
  ...
  static runTest(testList, test, testRunner) {
    ...
    try {
      runningTest = test;
      startTime = +new Date(); // ensure we mark the bottom of the stack each time we start a new test
    ...
    if (failure) {
      ...
      Console.error("... fail! (".concat(test.durationMs, " ms)"), Console.options({
        bulletPoint: "".concat(checkmark, " ")
      }));
      ...
      if (failure instanceof TestFailure) {
        ...
        frames.some(frame => {
          ...
          if (files.exists(absPath)) {
            ...
            if (parts[0] === "tools" && parts[1] === "tool-testing") {
              // Ignore frames inside the /tools/tool-testing directory,
              // like run.js and selftest.js.
              return false;
            }

            pathWithLineNumber = "".concat(relPath, ":").concat(frame.line);
            return true;
          ...
          pathWithLineNumber = "".concat(frame.file, ":").concat(frame.line);
          return true;
        });
        Console.rawError(" => Failure Reason: \"".concat(failure.reason, "\" at \"").concat(pathWithLineNumber, "\"\n"));

        if (failure.reason === 'no-match' || failure.reason === 'junk-before' || failure.reason === 'match-timeout') {
          Console.arrowError("Pattern: \"".concat(failure.details.pattern, "\""), 2);
        }

        if (failure.reason === "wrong-exit-code") {
          const s = status => "".concat(status.signal || status.code || "???");

          Console.rawError("  => Expected: \"".concat(s(failure.details.expected), "\"") + "; actual: \"".concat(s(failure.details.actual), "\"\n"));
        }

        if (failure.reason === 'expected-exception') {}

        if (failure.reason === 'not-equal') {
          Console.rawError("  => Expected: \"".concat(JSON.stringify(failure.details.expected), "\"; \n            actual: \"").concat(JSON.stringify(failure.details.actual), "\""));
        }
        ...
        if (failure.details.run) {
          ...
          if (!lines.length) {
            ...
            Console.arrowError("Last ".concat(historyLines, " lines:"), 2);
        ...
        Console.rawError("  => Test threw exception: ".concat(failure.stack, "\n"));
      }

      if (options.retries > 0) {
        Console.error("... retrying (" + options.retries + (options.retries === 1 ? " try" : " tries") + " remaining) ...", Console.options({
          indent: 2
        }));
        options.retries--;
      ...
      Console.success("... ok! (".concat(test.durationMs, " ms)"));
    }
  }
