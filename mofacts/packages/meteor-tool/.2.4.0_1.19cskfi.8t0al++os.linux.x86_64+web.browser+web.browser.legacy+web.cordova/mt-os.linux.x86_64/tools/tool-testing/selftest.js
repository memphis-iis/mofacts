...
let parseStackMarkBottom, parseStackMarkTop, parseStackParse;
module.link("../utils/parse-stack", {
  markBottom(v) {
    parseStackMarkBottom = v;
  },
...
module.link("./sandbox.js", {
  default: "Sandbox"
}, 9);
module.link("../tool-env/install-runtime.js");
const fail = parseStackMarkTop(function (reason) {
  throw new TestFailure(reason);
});
const expectEqual = parseStackMarkTop(function (actual, expected) {
...
const expectTrue = parseStackMarkTop(function (actual) {
  if (!actual) {
    throw new TestFailure('not-true');
  }
});
const expectFalse = parseStackMarkTop(function (actual) {
  if (actual) {
    throw new TestFailure('not-false');
  }
});
const expectThrows = parseStackMarkTop(function (f) {
  let threw = false;
...
const getAllTests = () => {
  ...
  allTests = []; // Load all files in the 'tests' directory that end in .js. They
...
function define(name, tagsList, f) {
...
function getFilteredTests(options) {
  ...
  if (allTests.length) {
    testState = readTestState(); // Add pseudo-tags 'non-matching', 'unchanged', 'non-galaxy' and 'in other
    ...
    allTests = allTests.map(test => {
      ...
      } else if (options.onlyChanged && test.fileHash === testState.lastPassedHashes[test.file]) {
        newTags.push('unchanged');
      } else if (options.excludeRegexp && options.excludeRegexp.test(test.name)) {
      ...
      return Object.assign(Object.create(Object.getPrototypeOf(test)), test, {
...
class TestList {
  ...
  notifyFailed(test, failureObject) {
    ...
    this.failedTests.push(test); // Mark that the specific test failed.

    test.failed = true; // If there is a failure object, store that for potential output.
  ...
  saveJUnitOutput(path) {
    const grouped = groupTestsByFile(this.filteredTests); // We'll form an collection of "testsuites"

    const testSuites = [];

    const attrSafe = attr => (attr || "").replace('"', "&quot;");

    const durationForOutput = durationMs => durationMs / 1000; // Each file is a testsuite.


    Object.keys(grouped).forEach(file => {
      const testCases = [];
      let countError = 0;
      let countFailure = 0; // Each test is a "testcase".

      grouped[file].forEach(test => {
        const testCaseAttrs = ["name=\"".concat(attrSafe(test.name), "\"")];

        if (test.durationMs) {
          testCaseAttrs.push("time=\"".concat(durationForOutput(test.durationMs), "\""));
        }
        ...
        if (test.failed) {
          ...
          if (test.failureObject instanceof TestFailure) {
            countFailure++;
            failureElement = ["<error type=\"".concat(test.failureObject.reason, "\">"), '<![CDATA[', inspect(test.failureObject.details, {
              depth: 4
            }), ']]>', '</error>'].join('\n');
          } else if (test.failureObject && test.failureObject.stack) {
            countError++;
            failureElement = ['<failure>', '<![CDATA[', test.failureObject.stack, ']]>', '</failure>'].join('\n');
          } else {
            countError++;
            failureElement = '<failure />';
          }

          testCases.push(["<testcase ".concat(testCaseAttrsString, ">"), failureElement, '</testcase>'].join('\n'));
        } else {
          testCases.push("<testcase ".concat(testCaseAttrsString, "/>"));
        }
      });
      const testSuiteAttrs = ["name=\"".concat(file, "\""), "tests=\"".concat(testCases.length, "\""), "failures=\"".concat(countFailure, "\""), "errors=\"".concat(countError, "\""), "time=\"".concat(durationForOutput(this.durationMs), "\"")];
      const testSuiteAttrsString = testSuiteAttrs.join(' ');
      testSuites.push(["<testsuite ".concat(testSuiteAttrsString, ">"), testCases.join('\n'), '</testsuite>'].join('\n'));
    });
    const xmlHeader = '<?xml version="1.0" encoding="UTF-8"?>';
    const testSuitesString = testSuites.join('\n');
    files.writeFile(path, [xmlHeader, "<testsuites>", testSuitesString, "</testsuites>"].join('\n'), 'utf8');
  ...
  generateSkipReport() {
    ...
    this.skippedTags.forEach(tag => {
      ...
      if (count) {
        ...
        const nounPhrase = / /.test(tag) ? noun + " " + tag : tag + " " + noun; // " (foo)" or ""

        const parenthetical = tagDescriptions[tag] ? " (" + tagDescriptions[tag] + ")" : '';
...
function listTests(options) {
  ...
  Object.keys(grouped).forEach(file => {
    Console.rawInfo("".concat(file, ".js\n"));
    grouped[file].forEach(test => {
      Console.rawInfo('  - test:' + test.name + (test.tags.length ? ' [' + test.tags.join(' ') + ']' : '') + '\n');
    });
  });
  Console.error();
...
const shouldSkipCurrentTest = _ref => {
  let {
    currentTestIndex,
    options: {
      skip,
...
function runTests(options) {
  ...
  testList.filteredTests.forEach((test, index) => {
    ...
    const skipMessage = shouldSkip ? options.preview ? 'will skip' : 'skipped' : options.preview ? 'will run' : 'running';
    const countMessage = "(".concat(index + 1, "/").concat(testList.filteredTests.length, ")");
    const testMessage = "".concat(test.file, ".js test:").concat(test.name, " ...");
    Console.info("".concat(skipMessage, " ").concat(countMessage, " ").concat(testMessage));

    if (shouldSkip || options.preview) {
      return;
    }

    Run.runTest(testList, test, parseStackMarkBottom(() => {
      test.f(options);
    }), {
  ...
  if (!totalRun) {
    ...
    testList.failedTests.forEach(test => {
      Console.rawError("  - ".concat(test.file, ".js: test:").concat(test.name, "\n"));
    });
    return 1;
  }
}
