!function (module1) {
  ...
  let optimisticReadJsonOrNull, optimisticHashOrNull;
  module1.link("./fs/optimistic", {
    optimisticReadJsonOrNull(v) {
      optimisticReadJsonOrNull = v;
    },
  ...
  let Resolver;
  module1.link("./isobuild/resolver", {
    default(v) {
      Resolver = v;
    }
  ...
  var Profile = require('./tool-env/profile').Profile; // This variable was duplicated due to an issue on importing it.
  ...
  const CAN_DELAY_LEGACY_BUILD = !JSON.parse(process.env.METEOR_DISALLOW_DELAYED_LEGACY_BUILD || "false"); // The ProjectContext represents all the context associated with an app:
  ...
  function ProjectContext(options) {
    ...
    if (!options.projectDir) throw Error("missing projectDir!");
    self.originalOptions = options;
    self.reset();
  }

  exports.ProjectContext = ProjectContext; // The value is the name of the method to call to continue.
  ...
  Object.assign(ProjectContext.prototype, {
    reset: function (moreOptions, resetOptions) {
      var self = this; // Allow overriding some options until the next call to reset;

      var options = Object.assign({}, self.originalOptions, moreOptions); // This is options that are actually directed at reset itself.
      ...
      self._packageMapFilename = options.packageMapFilename || files.pathJoin(self.projectDir, '.meteor', 'versions');
      self._serverArchitectures = options.serverArchitectures || []; // We always need to download host versions of packages, at least for
      // plugins.

      self._serverArchitectures.push(archinfo.host());

      self._serverArchitectures = _.uniq(self._serverArchitectures); // test-packages overrides this to load local packages from your real app
      // instead of from test-runner-app.

      self._projectDirForLocalPackages = options.projectDirForLocalPackages || options.projectDir;
      self._explicitlyAddedLocalPackageDirs = options.explicitlyAddedLocalPackageDirs; // Used to override the directory that Meteor's build process
      ...
      self.projectLocalDir = process.env.METEOR_LOCAL_DIR ? files.pathResolve(options.projectDir, files.convertToStandardPath(process.env.METEOR_LOCAL_DIR)) : options.projectLocalDir || files.pathJoin(self.projectDir, '.meteor', 'local'); // Used by 'meteor rebuild'; true to rebuild all packages, or a list of
      // package names.  Deletes the isopacks and their plugin caches.

      self._forceRebuildPackages = options.forceRebuildPackages; // Set in a few cases where we really want to only get packages from
      // checkout.

      self._ignorePackageDirsEnvVar = options.ignorePackageDirsEnvVar; // Set by some tests where we want to pretend that we don't have packages in
      // the git checkout (because they're using a fake warehouse).

      self._ignoreCheckoutPackages = options.ignoreCheckoutPackages; // Set by some tests to override the official catalog.

      self._officialCatalog = options.officialCatalog || catalog.official;
      if (options.alwaysWritePackageMap && options.neverWritePackageMap) throw Error("always or never?"); // Set by 'meteor create' and 'meteor update' to ensure that
      // .meteor/versions is always written even if release.current does not match
      // the project's release.

      self._alwaysWritePackageMap = options.alwaysWritePackageMap; // Set by a few special-case commands that call
      ...
      self._neverWriteProjectConstraintsFile = options.neverWriteProjectConstraintsFile;
      self._neverWritePackageMap = options.neverWritePackageMap; // Set by 'meteor update' to specify which packages may be updated. Array of
      // package names.

      self._upgradePackageNames = options.upgradePackageNames; // Set by 'meteor update' to mean that we should upgrade the
      // "patch" (and wrapNum, etc.) parts of indirect dependencies.

      self._upgradeIndirectDepPatchVersions = options.upgradeIndirectDepPatchVersions; // Set by publishing commands to ensure that published packages always have
      ...
      self._forceIncludeCordovaUnibuild = options.forceIncludeCordovaUnibuild; // If explicitly specified as null, use no release for constraints.
      ...
      if (resetOptions.preservePackageMap && self.packageMap) {
        self._cachedVersionsBeforeReset = self.packageMap.toVersionMap(); // packageMapFile should always exist if packageMap does
      ...
      self._allowIncompatibleUpdate = options.allowIncompatibleUpdate; // If set, we run the linter on the app and local packages.  Set by 'meteor
      // lint', and the runner commands (run/test-packages/debug) when --no-lint
      // is not passed.

      self.lintAppAndLocalPackages = options.lintAppAndLocalPackages; // If set, we run the linter on just one local package, with this
      // source root. Set by 'meteor lint' in a package, and 'meteor publish'.

      self._lintPackageWithSourceRoot = options.lintPackageWithSourceRoot; // Initialized by readProjectMetadata.
      ...
      self.finishedUpgraders = null; // Initialized by initializeCatalog.

      self.projectCatalog = null;
      self.localCatalog = null; // Once the catalog is read and the names of the "explicitly
      ...
      self.explicitlyAddedPackageNames = null; // Initialized by _resolveConstraints.
      ...
      self._completedStage = STAGE.INITIAL; // The resolverResultCache is used by the constraint solver; to
    ...
    _completeStagesThrough: function (targetStage) {
      ...
      buildmessage.enterJob('preparing project', function () {
        while (self._completedStage !== targetStage) {
          ...
          if (self.completedStage === STAGE.SAVE_CHANGED_METADATA) throw Error("can't find requested stage " + targetStage); // The actual value of STAGE.FOO is the name of the method that takes
    ...
    _readProjectMetadata: Profile('_readProjectMetadata', function () {
      ...
      buildmessage.enterJob('reading project metadata', function () {
        ...
        if (buildmessage.jobHasMessages()) return; // Read .meteor/release.

        self.releaseFile = new exports.ReleaseFile({
          projectDir: self.projectDir,
          catalog: self._officialCatalog
        });
        if (buildmessage.jobHasMessages()) return; // Read .meteor/packages.

        self.projectConstraintsFile = new exports.ProjectConstraintsFile({
          projectDir: self.projectDir,
          includePackages: self._includePackages
        });
        if (buildmessage.jobHasMessages()) return; // Read .meteor/versions.

        self.packageMapFile = new exports.PackageMapFile({
          filename: self._packageMapFilename
        });
        if (buildmessage.jobHasMessages()) return; // Read .meteor/cordova-plugins.

        self.cordovaPluginsFile = new exports.CordovaPluginsFile({
          projectDir: self.projectDir
        });
        if (buildmessage.jobHasMessages()) return; // Read .meteor/platforms, creating it if necessary.

        self.platformList = new exports.PlatformList({
          projectDir: self.projectDir
        });
        if (buildmessage.jobHasMessages()) return; // Read .meteor/.id, creating it if necessary.

        self._ensureAppIdentifier();

        if (buildmessage.jobHasMessages()) return; // Set up an object that knows how to read and write
    ...
    _ensureProjectDir: function () {
      var self = this;
      files.mkdir_p(files.pathJoin(self.projectDir, '.meteor')); // This file existing is what makes a project directory a project directory,
    ...
    getProjectWatchSet: function () {
      ...
      [self.releaseFile, self.projectConstraintsFile, self.packageMapFile, self.platformList, self.cordovaPluginsFile].forEach(function (metadataFile) {
    ...
    getProjectAndLocalPackagesWatchSet: function () {
      var self = this;
      var watchSet = self.getProjectWatchSet(); // Include the loaded local packages (ie, the non-metadata files) but only
    ...
    _ensureAppIdentifier: function () {
      var self = this;
      var identifierFile = files.pathJoin(self.projectDir, '.meteor', '.id'); // Find the first non-empty line, ignoring comments. We intentionally don't
      ...
      var appId = _.find(_.map(lines, files.trimSpaceAndComments), _.identity); // If the file doesn't exist or has no non-empty lines, regenerate the
      // token.


      if (!appId) {
        appId = [utils.randomIdentifier(), utils.randomIdentifier()].join(".");
        var comment = "# This file contains a token that is unique to your project.\n" + "# Check it into your repository along with the rest of this directory.\n" + "# It can be used for purposes such as:\n" + "#   - ensuring you don't accidentally deploy one app on top of another\n" + "#   - providing package authors with aggregated statistics\n" + "\n";
    ...
    _resolveConstraints: Profile('_resolveConstraints', function () {
      ...
      var depsAndConstraints = self._getRootDepsAndConstraints(); // If this is in the runner and we have reset this ProjectContext for a
      ...
      if (self._cachedVersionsBeforeReset && self._oldPackageMapFileHash === self.packageMapFile.fileHash) {
      ...
      var anticipatedPrereleases = self._getAnticipatedPrereleases(depsAndConstraints.constraints, cachedVersions);
      ...
      var resolverRunCount = 0; // Nothing before this point looked in the official or project catalog!
      ...
      catalog.runAndRetryWithRefreshIfHelpful(function (canRetry) {
        buildmessage.enterJob("selecting package versions", function () {
          ...
          try {
            Profile.time("Select Package Versions" + (resolverRunCount > 1 ? " (Try " + resolverRunCount + ")" : ""), function () {
              solution = resolver.resolve(depsAndConstraints.deps, depsAndConstraints.constraints, resolveOptions);
            });
          } catch (e) {
            if (!e.constraintSolverError && !e.versionParserError) throw e; // If the contraint solver gave us an error, refreshing
            ...
            buildmessage.error(e.message, {
              tags: {
                refreshCouldHelp: !!e.constraintSolverError
              }
            });
          }
          ...
          self.packageMapDelta = new packageMapModule.PackageMapDelta({
            ...
            neededToUseUnanticipatedPrereleases: solution.neededToUseUnanticipatedPrereleases,
            anticipatedPrereleases: anticipatedPrereleases
          });
    ...
    _readResolverResultCache() {
      if (!this._resolverResultCache) {
        try {
          this._resolverResultCache = JSON.parse(files.readFile(files.pathJoin(this.projectLocalDir, "resolver-result-cache.json")));
        } catch (e) {
    ...
    _saveResolverResultCache() {
      files.writeFileAtomically(files.pathJoin(this.projectLocalDir, "resolver-result-cache.json"), JSON.stringify(this._resolverResultCache) + "\n");
    },

    getBuildCache() {
      try {
        return JSON.parse(files.readFile(files.pathJoin(this.projectLocalDir, "build-cache.json")));
      } catch (e) {
        return null;
      }
    },
    ...
    saveBuildCache(buildCache) {
      files.writeFileAtomically(files.pathJoin(this.projectLocalDir, "build-cache.json"), JSON.stringify(buildCache) + "\n");
    },
    ...
    _localPackageSearchDirs: function () {
      const self = this;
      let searchDirs = [files.pathJoin(self._projectDirForLocalPackages, 'packages')]; // User can provide additional package directories to search in
      ...
      if (process.env.PACKAGE_DIRS) {
        Console.warn('For compatibility, the PACKAGE_DIRS environment variable', 'is deprecated and will be removed in a future Meteor release.');
        Console.warn('Developers should now use METEOR_PACKAGE_DIRS and', 'Windows projects should now use a semi-colon (;) to separate paths.');
      }

      function packageDirsFromEnvVar(envVar) {
        let delimiter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : files.pathOsDelimiter;
        return process.env[envVar] && process.env[envVar].split(delimiter) || [];
      }

      const envPackageDirs = [// METEOR_PACKAGE_DIRS should use the arch-specific delimiter
      ...packageDirsFromEnvVar('METEOR_PACKAGE_DIRS'), // PACKAGE_DIRS (deprecated) always used ':' separator (yes, even Windows)
      ...
      if (!self._ignoreCheckoutPackages && files.inCheckout()) {
        ...
        searchDirs.push( // Include packages like packages/ecmascript.
        packagesDir, // Include packages like packages/non-core/coffeescript.
        files.pathJoin(packagesDir, "non-core"), // Include packages like packages/non-core/blaze/packages/blaze.
    ...
    _initializeCatalog: Profile('_initializeCatalog', function () {
      ...
      catalog.runAndRetryWithRefreshIfHelpful(function () {
        buildmessage.enterJob("scanning local packages", function () {
          ...
          _.each(self._explicitlyAddedLocalPackageDirs, function (dir) {
            var localVersionRecord = self.localCatalog.getVersionBySourceRoot(dir);
    ...
    _addLocalPackageConstraints: function (depsAndConstraints) {
      ...
      _.each(self.localCatalog.getAllPackageNames(), function (packageName) {
        ...
        var constraint = utils.parsePackageConstraint(packageName + "@=" + versionRecord.version); // Add a constraint ("this is the only version available") but no
    ...
    _addReleaseConstraints: function (depsAndConstraints) {
      var self = this;
      if (!self._releaseForConstraints) return;

      _.each(self._releaseForConstraints.packages, function (version, packageName) {
        var constraint = utils.parsePackageConstraint( // Note that this used to be an exact name@=version constraint,
        ...
        packageName + "@~" + version); // Add a constraint but no dependency (we don't automatically use
        // all local packages!):
    ...
    _buildResolver: function () {
      const {
        ConstraintSolver
      } = loadIsopackage('constraint-solver');
    ...
    _downloadMissingPackages: Profile('_downloadMissingPackages', function () {
      var self = this;
      buildmessage.assertInJob();
      if (!self.packageMap) throw Error("which packages to download?");
      catalog.runAndRetryWithRefreshIfHelpful(function () {
    ...
    _buildLocalPackages: Profile('_buildLocalPackages', function () {
      ...
      self.isopackCache = new isopackCacheModule.IsopackCache({
        packageMap: self.packageMap,
        includeCordovaUnibuild: self._forceIncludeCordovaUnibuild || self.platformList.usesCordova(),
      ...
      if (self._forceRebuildPackages) {
        self.isopackCache.wipeCachedPackages(self._forceRebuildPackages === true ? null : self._forceRebuildPackages);
      }
    ...
    _saveChangedMetadata: Profile('_saveChangedMetadata', function () {
      var self = this; // Save any changes to .meteor/packages.

      if (!self._neverWriteProjectConstraintsFile) self.projectConstraintsFile.writeIfModified(); // Write .meteor/versions if the command always wants to (create/update),
      // or if the release of the app matches the release of the process.

      if (!self._neverWritePackageMap && (self._alwaysWritePackageMap || release.current.isCheckout() && self.releaseFile.isCheckout() || !release.current.isCheckout() && release.current.name === self.releaseFile.fullReleaseName)) {
  ...
  exports.ProjectConstraintsFile = function (options) {
    ...
    self.watchSet = null; // List of packages that should be included if not provided in .meteor/packages

    self._includePackages = options.includePackages || []; // Have we modified the in-memory representation since reading from disk?

    self._modified = null; // List of each line in the file; object with keys:
    ...
    self._constraintLines = null; // Maps from package name to entry in _constraintLines.
  ...
  Object.assign(exports.ProjectConstraintsFile.prototype, {
    _readFile: function () {
      ...
      var contents = watch.readAndWatchFile(self.watchSet, self.filename); // No .meteor/packages? This isn't a very good project directory. In fact,
      // that's the definition of a project directory! (And that should have been
      // fixed by _ensureProjectDir!)

      if (contents === null) throw Error("packages file missing: " + self.filename);
      ...
      var lines = files.splitBufferToLines(contents); // Don't keep a record for the space at the end of the file.

      if (lines.length && _.last(lines) === '') lines.pop();
      ...
      _.each(lines, function (line) {
        ...
        self._constraintLines.push(lineRecord); // Strip comment.
        ...
        line = match[1];
        lineRecord.trailingSpaceAndComment = match[2] + lineRecord.trailingSpaceAndComment; // Strip leading space.

        match = line.match(/^(\s*)((?:\S.*)?)$/);
        lineRecord.leadingSpace = match[1];
        line = match[2]; // No constraint? Leave lineRecord.constraint null and continue.
        ...
        if (!lineRecord.constraint) return; // recover by ignoring
        // Mark as not iterable if already included in self._includePackages

        if (_.has(extraConstraintMap, lineRecord.constraint.package)) lineRecord.skipOnRead = true;

        if (_.has(self._constraintMap, lineRecord.constraint.package)) {
          buildmessage.error("Package name appears twice: " + lineRecord.constraint.package, {
    ...
    _write: function () {
      ...
      var lines = _.map(self._constraintLines, function (lineRecord) {
        ...
        if (lineRecord.skipOnWrite) return;
      ...
      if (messages.hasMessages()) throw Error("wrote bad .meteor/packages: " + messages.formatMessages());
    },
    ...
    eachConstraint: function (iterator) {
      ...
      _.each(self._constraintLines, function (lineRecord) {
        if (!lineRecord.skipOnRead && lineRecord.constraint) iterator(lineRecord.constraint);
      });
    },
    ...
    getConstraint: function (name) {
      var self = this;
      if (_.has(self._constraintMap, name)) return self._constraintMap[name].constraint;
      return null;
    },
    ...
    addConstraints: function (constraintsToAdd) {
      ...
      _.each(constraintsToAdd, function (constraintToAdd) {
        ...
        if (_.isEqual(constraintToAdd, lineRecord.constraint)) return; // nothing changed
    ...
    updateReleaseConstraints: function (releaseRecord) {
      this.addConstraints(_.compact(_.map(releaseRecord.packages, (version, packageName) => {
    ...
    removePackages: function (packagesToRemove) {
      var self = this;
      self._constraintLines = self._constraintLines.filter(function (lineRecord) {
        return !(lineRecord.constraint && packagesToRemove.includes(lineRecord.constraint.package));
      });
  ...
  Object.assign(exports.PackageMapFile.prototype, {
    _readFile: function () {
      ...
      self.fileHash = fileInfo.hash; // No .meteor/versions? That's OK, you just get to start your calculation
      // from scratch.
      ...
      _.each(lines, function (line) {
        ...
        if (!packageVersion) return; // recover by ignoring
        ...
        if (_.has(self._versions, packageVersion.package)) return;
    ...
    write: function (packageMap) {
      var self = this;
      var newVersions = packageMap.toVersionMap(); // Only write the file if some version changed. (We don't need to do no-op
      // writes, even if they fix sorting in the file.)

      if (_.isEqual(self._versions, newVersions)) return;
      ...
      files.writeFileAtomically(self.filename, fileContents); // Replace our watchSet with one for the new contents of the file.
  ...
  Object.assign(exports.PlatformList.prototype, {
    _readFile: function () {
      var self = this; // Reset the WatchSet.

      self.watchSet = new watch.WatchSet();
      var contents = watch.readAndWatchFile(self.watchSet, self.filename);
      var platforms = contents ? files.splitBufferToLines(contents) : []; // We don't allow comments here, since it's cruel to allow comments in a
      // file when you're going to overwrite them anyway.

      platforms = _.uniq(_.compact(_.map(platforms, files.trimSpace)));
      platforms.sort(); // Missing some of the default platforms (or the whole file)? Add them and
      // try again.

      if (_.difference(exports.PlatformList.DEFAULT_PLATFORMS, platforms).length) {
    ...
    write: function (platforms) {
      var self = this;
      self._platforms = null;
      platforms = _.uniq(platforms.concat(exports.PlatformList.DEFAULT_PLATFORMS));
      platforms.sort();
    ...
    getCordovaPlatforms: function () {
      var self = this;
      return _.difference(self._platforms, exports.PlatformList.DEFAULT_PLATFORMS);
    },
    ...
    getWebArchs() {
      var self = this;
      var archs = ["web.browser", "web.browser.legacy"];
    ...
    canDelayBuildingArch(arch) {
      return CAN_DELAY_LEGACY_BUILD && arch === "web.browser.legacy";
    }
  ...
  exports.CordovaPluginsFile = function (options) {
    ...
    self.watchSet = null; // Map from plugin name to version.
  ...
  Object.assign(exports.CordovaPluginsFile.prototype, {
    _readFile: function () {
      ...
      self.watchSet = new watch.WatchSet();
      self._plugins = {};
      var contents = watch.readAndWatchFile(self.watchSet, self.filename); // No file?  No plugins.
      ...
      _.each(lines, function (line) {
        line = files.trimSpace(line);
        if (line === '') return; // We just do a standard split here, not utils.parsePackageConstraint,
  ...
  exports.ReleaseFile = function (options) {
    ...
    self.watchSet = null; // The release name actually written in the file.  Null if no fill.  Empty if
    // the file is empty.

    self.unnormalizedReleaseName = null; // The full release name (with METEOR@ if it's missing in
    // unnormalizedReleaseName).

    self.fullReleaseName = null; // FOO@bar unless FOO === "METEOR" in which case "Meteor bar".

    self.displayReleaseName = null; // Just the track.
  ...
  Object.assign(exports.ReleaseFile.prototype, {
    ...
    normalReleaseSpecified: function () {
      var self = this;
      return !(self.fileMissing() || self.noReleaseSpecified() || self.isCheckout());
    },
    _readFile: function () {
      var self = this; // Start a new watchSet, in case we just overwrote this.

      self.watchSet = new watch.WatchSet();
      var contents = watch.readAndWatchFile(self.watchSet, self.filename); // If file doesn't exist, leave unnormalizedReleaseName empty; fileMissing
      // will be true.

      if (contents === null) return;

      var lines = _.compact(_.map(files.splitBufferToLines(contents), files.trimSpaceAndComments)); // noReleaseSpecified will be true.
    ...
    getDevBundle() {
      ...
      if (meteorToolIndex >= 0) {
        const releaseVersion = this.catalog.getReleaseVersion(this.releaseTrack, this.releaseVersion);
    ...
    ensureDevBundleLink() {
      let makeLink, readLink;
      module1.link("./cli/dev-bundle-links.js", {
        makeLink(v) {
          makeLink = v;
        },
  ...
  exports.FinishedUpgraders = function (options) {
    var self = this;
    self.filename = files.pathJoin(options.projectDir, '.meteor', '.finished-upgraders');
  };
  ...
  Object.assign(exports.FinishedUpgraders.prototype, {
    ...
    appendUpgraders: function (upgraders) {
      ...
      if (current === null) {
        ...
        appendText = "# This file contains information which helps Meteor properly upgrade your\n" + "# app when you run 'meteor update'. You should check it into version control\n" + "# with your project.\n" + "\n";
  ...
  class MeteorConfig {
    ...
    _ensureInitialized() {
      if (!_.has(this, "_config")) {
        ...
        this.watchSet.addFile(this.packageJsonPath, optimisticHashOrNull(this.packageJsonPath));
      }
    ...
    getNodeModulesToRecompileByArch() {
      ...
      if (recompile && typeof recompile === "object") {
        const get = arch => packageNamesByArch[arch] || (packageNamesByArch[arch] = new Set());
    ...
    getNodeModulesToRecompile(arch) {
      let packageNamesByArch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.getNodeModulesToRecompileByArch();
    ...
    getMainModule(arch) {
      let mainModulesByArch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.getMainModulesByArch();
    ...
    getTestModule(arch) {
      let testModulesByArch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.getTestModulesByArch();
      return this._getEntryModule(arch, testModulesByArch);
    }

    _getEntryModulesByArch() {
      const configEntryModule = this.get(...arguments);
      const entryModulesByArch = Object.create(null);

      if (typeof configEntryModule === "string" || configEntryModule === false) {
    ...
    _getEntryModule(arch, entryModulesByArch) {
      const entryMatch = archinfo.mostSpecificMatch(arch, Object.keys(entryModulesByArch));
      ...
      if (entryMatch) {
        ...
        const res = this._resolversByArch[arch].resolve( // Only relative paths are allowed (not top-level packages).
        "./" + files.pathNormalize(entryModule), this.packageJsonPath);

        if (res && typeof res === "object") {
          return files.pathRelative(this.appDirectory, res.path);
        }

        buildmessage.error("Could not resolve meteor.mainModule ".concat(JSON.stringify(entryModule), " in ").concat(files.pathRelative(this.appDirectory, this.packageJsonPath), " (").concat(arch, ")"));
      }
    }
