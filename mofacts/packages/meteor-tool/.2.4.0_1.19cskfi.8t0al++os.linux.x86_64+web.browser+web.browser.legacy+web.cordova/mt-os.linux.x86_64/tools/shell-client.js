...
const EOL = require("os").EOL; // These two values (EXITING_MESSAGE and getInfoFile) must match the
...
function connect(shellDir) {
  new Client(shellDir).connect();
}

class Client {
  constructor(shellDir) {
    _defineProperty(this, "shellDir", void 0);

    _defineProperty(this, "connected", false);

    _defineProperty(this, "exitOnClose", false);

    _defineProperty(this, "firstTimeConnecting", true);
  ...
  setUpSocket(sock, key) {
    ...
    const onConnect = () => {
      ...
      this.connected = true; // Sending a JSON-stringified options object (even just an empty
    ...
    const onClose = () => {
      tearDown(); // If we received the special EXITING_MESSAGE just before the socket
...
function shellBanner() {
  const bannerLines = ["", "Welcome to the server-side interactive shell!"];
