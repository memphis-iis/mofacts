!function (module1) {
  ...
  const _excluded = ["appHost", "appPort", "banner", "disableOplog", "cordovaRunner", "mongoUrl", "onFailure", "oplogUrl", "projectContext", "proxyHost", "proxyPort", "quiet", "rootUrl", "selenium", "seleniumBrowser", "noReleaseCheck"];
  ...
  class Runner {
    constructor(_ref) {
      let {
        ...
        noReleaseCheck
          ...
          optionsForAppRunner = _objectWithoutProperties(_ref, _excluded);
      ...
      self.quiet = quiet;
      self.banner = banner || files.convertToOSPath(files.prettyPath(self.projectContext.projectDir));
      ...
      } else if (hasMongoDevServerPackage || process.env.METEOR_TEST_FAKE_MONGOD_CONTROL_PORT) {
      ...
      const hasHotModuleReplacementPackage = packageMap && packageMap.getInfo('hot-module-replacement') != null;
      self.hmrServer = null;
      let hmrSecret = null;
      ...
      self.appRunner = new AppRunner(_objectSpread(_objectSpread({}, optionsForAppRunner), {}, {
        projectContext: self.projectContext,
        port: self.appPort,
        listenHost: appHost,
        mongoUrl,
    ...
    start() {
      const self = this;
      self.proxy.start(); // print the banner only once we've successfully bound the port
      ...
      function startMongo() {
        let tries = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 3;

        self._startMongoAsync().then(ok => unblockAppRunner(), error => {
          --tries;
          const left = tries + (tries === 1 ? " try" : " tries");
          Console.error("Error starting Mongo (".concat(left, " left): ").concat(error.message));
      ...
      if (!self.stopped && !self.quiet) {
        ...
        if (process.env.UNIX_SOCKET_PATH) {
          runLog.log("App running; UNIX domain socket: ".concat(process.env.UNIX_SOCKET_PATH), {
            arrow: true
          });
        } else {
          runLog.log("App running at: " + self.rootUrl, {
            arrow: true
          });
        }
    ...
    stop() {
      ...
      self.appRunner.stop();
      self.selenium && self.selenium.stop(); // XXX does calling this 'finish' still make sense now that runLog is a
      // singleton?
  ...
  exports.run = function (options) {
    ...
    var promise = new Promise(function (resolve) {
      ...
      runOptions.onRunEnd = function (result) {
        if (once || result.outcome === "conflicting-versions" || result.outcome === "wrong-release" || result.outcome === "outdated-cordova-platforms" || result.outcome === "outdated-cordova-plugins" || result.outcome === "terminated" && result.signal === undefined && result.code === undefined) {
          resolve(result);
    ...
    runOptions.quiet = false; // Ensure process.env.NODE_ENV matches the build mode, with the following precedence:
    ...
    let nodeEnv = process.env.NODE_ENV; // We *never* override buildMode (it can be "test")
    ...
    if (result.outcome === "wrong-release") {
      ...
      Console.error("Your app has been updated to " + to + " from " + from + ".", "Restart meteor to use the new release.");
      return 254;
    }

    if (result.outcome === "failure" || result.outcome === "terminated" && result.signal === undefined && result.code === undefined) {
