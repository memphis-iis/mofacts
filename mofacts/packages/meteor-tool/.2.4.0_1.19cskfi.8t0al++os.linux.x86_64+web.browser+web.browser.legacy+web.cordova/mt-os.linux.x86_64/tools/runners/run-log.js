...
var fiberHelpers = require('../utils/fiber-helpers.js'); // runLog is primarily used by the parts of the tool which run apps locally. It
...
function getLoggingPackage() {
  if (!_Log) {
    _Log = require("../tool-env/isopackets.js").loadIsopackage('logging').Log;
...
var RunLog = function () {
  ...
  self.maxLength = 100; // If non-null, the last thing logged was "server restarted"
  ...
  self.consecutiveClientRestartMessages = null; // If non-null, the last thing that was logged was a temporary
...
Object.assign(RunLog.prototype, {
  ...
  logAppOutput: function (line, isStderr) {
    ...
    var obj = isStderr ? Log.objFromText(line, {
      level: 'warn',
      stderr: true
    ...
    if (self.rawLogs) {
      ...
      return fiberHelpers.noYieldsAllowed(function () {
        Console.rawInfo(Log.format(obj, {
          color: true
        }) + "\n");
      });
    }
  ...
  log: function (msg, options) {
    ...
    var obj = {
      time: new Date(),
      message: msg // in the future, might want to add something else to
    ...
    self._clearSpecial(); // Process the options. By default, we want to wordwrap the message with
...
['log', 'logTemporary', 'logRestart', 'logClientRestart', 'logAppOutput', 'setRawLogs', 'finish', 'clearLog', 'getLog'].forEach(function (method) {
