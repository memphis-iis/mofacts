...
var runLog = require('./run-log.js'); // options: listenPort, proxyToPort, proxyToHost,
...
var Proxy = function (options) {
  ...
  self.listenHost = options.listenHost; // note: run-all.js updates proxyToPort directly
...
Object.assign(Proxy.prototype, {
  ...
  start: function () {
    ...
    self.server.on('error', function (err) {
      if (err.code === 'EADDRINUSE') {
        ...
        runLog.log("Can't listen on host " + self.listenHost + " (" + err.code + " from " + err.syscall + ").");
      } else {
        runLog.log('' + err);
      }
    ...
    self.proxy.on('error', function (err, req, resOrSocket) {
      if (err.code === 'HPE_HEADER_OVERFLOW') {
        const logMessage = 'Error during proxy to server communication ' + 'due to the header size exceeding Node\'s currently ' + 'configured limit. This limit is configurable with a command ' + 'line option (https://nodejs.org/api/cli.html#cli_max_http_header_size_size ' + 'and https://docs.meteor.com/commandline.html#meteorrun).';
  ...
  // Idempotent.
  stop: function () {
    var _self$httpQueue, _self$websocketQueue;
    ...
    self.server.close();
    self.server = null; // It doesn't seem to be necessary to do anything special to
    // destroy an httpProxy proxyserver object.

    self.proxy = null; // Drop any held connections.

    (_self$httpQueue = self.httpQueue) === null || _self$httpQueue === void 0 ? void 0 : _self$httpQueue.forEach(function (c) {
      c.res.statusCode = 500;
      c.res.end();
    });
    self.httpQueue = [];
    (_self$websocketQueue = self.websocketQueue) === null || _self$websocketQueue === void 0 ? void 0 : _self$websocketQueue.forEach(function (c) {
      c.socket.destroy();
    });
    self.websocketQueue = [];
    self.mode = "hold";
...
exports.Proxy = Proxy;
//# sourceMappingURL=run-proxy.js.map