module.export({
  ...
  watch: () => watch,
  disableNativeWatcher: () => disableNativeWatcher
});
let Profile;
...
let statOrNull, pathResolve, convertToOSPath, watchFile, unwatchFile;
module.link("./files", {
  statOrNull(v) {
    statOrNull = v;
  },
...
const watchLibrary = require("pathwatcher"); // Default to prioritizing changed files, but disable that behavior (and
...
if (process.env.METEOR_WATCH_PRIORITIZE_CHANGED && !JSON.parse(process.env.METEOR_WATCH_PRIORITIZE_CHANGED)) {
  PRIORITIZE_CHANGED = false;
}

var DEFAULT_POLLING_INTERVAL = +(process.env.METEOR_WATCH_POLLING_INTERVAL_MS || 5000);
var NO_WATCHER_POLLING_INTERVAL = +(process.env.METEOR_WATCH_POLLING_INTERVAL_MS || 500); // This may seems like a long time to wait before actually closing the
// file watchers, but it's to our advantage if they survive restarts.

const WATCHER_CLEANUP_DELAY_MS = 30000; // Pathwatcher complains (using console.error, ugh) if you try to watch
// two files with the same stat.ino number but different paths on linux, so we have
// to deduplicate files by ino.

const DEDUPLICATE_BY_INO = process.platform !== "win32"; // Set METEOR_WATCH_FORCE_POLLING environment variable to a truthy value to
...
const entries = Object.create(null); // Set of paths for which a change event has been fired, watched with
...
function acquireWatcher(absPath, callback) {
  const entry = entries[absPath] || (entries[absPath] = startNewWatcher(absPath)); // Watches successfully established in the past may have become invalid
  ...
  entry.rewatch(); // The size of the entry.callbacks Set also serves as a reference count
  // for this watcher.
...
function startNewWatcher(absPath) {
  ...
  let watcher = null; // Determines the polling interval to be used for the fs.watchFile-based
  ...
  function fire(event) {
    if (event !== "change") {
      ...
      safeUnwatch(); // Make sure we don't throttle the watchFile callback after a
  ...
  function watchWrapper(event) {
    ...
    fire(event); // It's tempting to call unwatchFile(absPath, watchFileWrapper) here,
  ...
  function rewatch() {
    ...
    lastWatcherEventTime = 0; // We use files.watchFile in addition to watcher.watch as a fail-safe
  ...
  function watchFileWrapper(newStat, oldStat) {
...
function statWatch(absPath, interval, callback) {
  ...
  while (oldWatcher) {
    ...
    // this StatWatcher.
    const countBefore = oldWatcher.stat.listenerCount("change"); // This removes at most one occurrence of the callback from the
    // listeners list...

    oldWatcher.stat.removeListener("change", callback); // ... so we have to keep calling it until the first time
    // it removes nothing.
  ...
  oldWatcher.stat = newStat; // If the interval needs to be changed, manually stop and restart the
  // StatWatcher using lower-level methods than unwatchFile and watchFile.

  if (oldWatcher.interval !== interval) {
    oldWatcher.stat.stop();
    oldWatcher.stat.start(convertToOSPath(pathResolve(absPath)), false, // never persistent
...
function watchLibraryWatch(absPath, callback) {
  if (watcherEnabled) {
    try {
      ...
      maybeSuggestRaisingWatchLimit(e); // ... ignore the error.  We'll still have watchFile, which is good
      // enough.
    }
  }
...
let suggestedRaisingWatchLimit = false; // This function is async so that archinfo.host() (which may call
...
function maybeSuggestRaisingWatchLimit(error) {
  return Promise.asyncApply(() => {
    ...
    if (!suggestedRaisingWatchLimit && // Note: the not-super-documented require('constants') maps from
    ...
    error.errno === constants.ENOSPC && // The only suggestion we currently have is for Linux.
    archinfo.matches(archinfo.host(), 'os.linux')) {
      ...
      if (!Console.isHeadless()) {
        Console.arrowWarn("It looks like a simple tweak to your system's configuration will " + "make many tools (including this Meteor command) more efficient. " + "To learn more, see " + Console.url("https://github.com/meteor/docs/blob/master/long-form/file-change-watcher-efficiency.md"));
      }
    }
  });
}

const watch = Profile("safeWatcher.watch", (absPath, callback) => {
