!function (module1) {
  ...
  let cleanupOnExit;
  module1.link("../tool-env/cleanup.js", {
    onExit(v) {
      cleanupOnExit = v;
    }
  ...
  let wordwrap;
  module1.link("wordwrap", {
    default(v) {
      wordwrap = v;
    }

  }, 5);
  let isEmacs, sleepMs, Throttled, ThrottledYield;
  ...
  const PROGRESS_DEBUG = !!process.env.METEOR_PROGRESS_DEBUG; // Set the default CR to \r unless we're running with cmd

  const CARRIAGE_RETURN = process.platform === 'win32' && process.stdout.isTTY && process.argv[1].toLowerCase().includes('cmd') ? new Array(249).join('\b') : '\r';
  const FORCE_PRETTY = process.env.METEOR_PRETTY_OUTPUT && process.env.METEOR_PRETTY_OUTPUT != '0';
  ...
  const PROGRESS_THROTTLE_MS = 300; // Message to show when we don't know what we're doing
  // XXX: ? FALLBACK_STATUS = 'Pondering';

  const FALLBACK_STATUS = ''; // If there is a part of the larger text, and we really want to make sure that
  ...
  const SPACE_REPLACEMENT = "\u2600"; // In Javascript, replace only replaces the first occurance and this is the
  ...
  const toFixedLength = (text, length) => {
    text = text || ""; // pad or truncate `text` to length
  ...
  class ProgressDisplayStatus {
    ...
    repaint() {// We don't repaint after a log message (is that right?)
    }
  ...
  class SpinnerRenderer {
    constructor() {
      this.frames = ['-', '\\', '|', '/'];
      this.start = +new Date();
      this.interval = 250; //// I looked at some Unicode indeterminate progress indicators, such as:
      ////
      //// spinner = "▁▃▄▅▆▇▆▅▄▃".split('');
      //// spinner = "▉▊▋▌▍▎▏▎▍▌▋▊▉".split('');
      //// spinner = "▏▎▍▌▋▊▉▊▋▌▍▎▏▁▃▄▅▆▇▆▅▄▃".split('');
  ...
  class ProgressDisplayFull {
    constructor(console) {
      ...
      this._headless = !!(process.env.METEOR_HEADLESS && JSON.parse(process.env.METEOR_HEADLESS));
    ...
    _render() {
      ...
      var progressGraphic = ''; // The cursor appears in position 0; we indent it a little to avoid this
      ...
      if (this._status || progressGraphic) {
        ...
        if (this._headless && line === this._lastWrittenLine) {
  ...
  class StatusPoller {
    ...
    _startPoller() {
      ...
      this._pollPromise = (() => Promise.asyncApply(() => {
    ...
    _statusPoll() {
      ...
      const reportState = (state, startTime) => {
        var progressDisplay = this._console._progressDisplay; // Do the % computation, if it is going to be used
  ...
  const LEVEL_ERROR = {
    code: LEVEL_CODE_ERROR
  };
  const LEVEL_WARN = {
    code: LEVEL_CODE_WARN
  };
  const LEVEL_INFO = {
    code: LEVEL_CODE_INFO
  };
  const LEVEL_DEBUG = {
    code: LEVEL_CODE_DEBUG
  };
  ...
  class Console extends ConsoleBase {
    constructor(options) {
      super();
      options = options || Object.create(null);
      this._headless = !!(process.env.METEOR_HEADLESS && JSON.parse(process.env.METEOR_HEADLESS)); // The progress display we are showing on-screen

      this._progressDisplay = new ProgressDisplayNone(this);
      this._statusPoller = null;
      this._throttledYield = new ThrottledYield();
      this.verbose = false;
      this._simpleDebug = false; // Legacy helpers
    ...
    withProgressDisplayVisible(f) {
      ...
      var originalProgressDisplayEnabled = this._progressDisplayEnabled; // Turn both flags on.

      this._pretty = this._progressDisplayEnabled = true; // Update the screen if anything changed.
      ...
      try {
        ...
        this._progressDisplayEnabled = originalProgressDisplayEnabled; // Update the screen if anything changed.
    ...
    _parseVariadicInput(args) {
      var msgArgs;
      var options; // If the last argument is an instance of ConsoleOptions, then we should
    ...
    _print(level, message) {
      ...
      progressDisplay.depaint(); // stdout/stderr is determined by the log level
    ...
    success(message) {
      let uglySuccessKeyword = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "success";
      var checkmark;

      if (!this._pretty) {
        return this.info("".concat(message, ": ").concat(uglySuccessKeyword));
      }
      ...
      return this.info(chalk.green(message), this.options({
    ...
    _fail(message, printFn) {
      ...
      var xmark = chalk.red("\u2717");
      return this[printFn](chalk.red(message), this.options({
        bulletPoint: xmark + " "
      }));
    ...
    labelWarn(message) {
      return this.warn(message, this.options({
        bulletPoint: "WARNING: "
      }));
    ...
    _arrowPrint(printFn, message, indent) {
      indent = indent || 0;
      return this[printFn](message, this.options({
        bulletPoint: ARROW,
        indent: indent
      }));
    ...
    url(message) {
      ...
      var unspaced = replaceAll(message, ' ', '%20'); // There is no need to call noWrap here, since that only handles spaces (and
    ...
    printTwoColumns(rows, options) {
      ...
      var indent = options.indent ? Array(options.indent + 1).join(' ') : "";
      var out = '';
      rows.forEach(row => {
        var col0 = row[0] || '';
        var col1 = row[1] || '';
    ...
    _wrapText(text, options) {
      options = options || Object.create(null); // Compute the maximum offset on the bulk of the message.
      ...
      if (process.env.METEOR_NO_WORDWRAP) {
        ...
        wrappedText = wordwrap(maxIndent, max)(text); // Insert the start string, if applicable.

        if (options.bulletPoint) {
          // Save the initial indent level.
          var initIndent = options.indent ? wrappedText.substring(0, options.indent) : ""; // Add together the initial indent (if any), the bullet point and the
          // remainder of the message.

          wrappedText = initIndent + options.bulletPoint + wrappedText.substring(maxIndent);
        }
    ...
    setHeadless() {
      let headless = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
      this._headless = !!headless;

      if (this._progressDisplay && this._progressDisplay.setHeadless) {
    ...
    readLine(options) {
      ...
      this._setProgressDisplay(new ProgressDisplayNone()); // Read a line, throwing away the echoed characters into our dummy stream.
