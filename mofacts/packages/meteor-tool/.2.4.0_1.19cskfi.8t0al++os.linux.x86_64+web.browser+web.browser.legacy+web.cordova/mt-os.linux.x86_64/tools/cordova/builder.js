...
let bundler;
module.link("../isobuild/bundler.js", {
  default(v) {
    bundler = v;
  }
...
let release;
module.link("../packaging/release.js", {
  default(v) {
    release = v;
  }
...
let XmlBuilder;
module.link("xmlbuilder2", {
  default(v) {
    XmlBuilder = v;
  }

}, 13);
let CORDOVA_ARCH, SWIFT_VERSION;
...
class CordovaBuilder {
  ...
  initalizeDefaults() {
    let {
      cordovaServerPort
    } = this.options; // if --cordova-server-port is not present on run command

    if (!cordovaServerPort) {
      // Convert the appId (a base 36 string) to a number
      const appIdAsNumber = parseInt(this.projectContext.appIdentifier, 36); // We use the appId to choose a local server port between 12000-13000.
    ...
    this.metadata = {
      ...
      email: 'n/a',
      website: 'n/a',
      contentUrl: "http://localhost:".concat(cordovaServerPort, "/")
    ...
    this.custom = []; // Resource files that will be appended to platform bundle and config.xml
    ...
    const serverDomain = mobileServerUrl ? utils.parseUrl(mobileServerUrl).hostname : null; // If the remote server domain is known, allow access to it for XHR and DDP
    // connections.
    ...
    const setDefaultLaunchScreen = (size, name) => {
      const imageFile = files.pathJoin(launchScreensPath, "".concat(size, ".png"));
  ...
  processControlFile() {
    const controlFilePath = files.pathJoin(this.projectContext.projectDir, 'mobile-config.js');

    if (files.exists(controlFilePath)) {
      Console.debug('Processing mobile-config.js');
      buildmessage.enterJob({
        title: "processing mobile-config.js"
      }, () => {
        ...
        try {
          files.runJavaScript(code, {
            ...
            symbols: {
              App: createAppConfiguration(this)
            }
          });
        } catch (error) {
  ...
  writeConfigXmlAndCopyResources() {
    let shouldCopyResources = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    let config = XmlBuilder.create({
      version: '1.0'
    }).ele('widget'); // Set the root attributes
    ...
    }).txt(this.metadata.author); // Set the additional global configuration preferences
    ...
    _.each(this.accessRules, (options, pattern) => {
      ...
      if (type === 'intent') {
        config.ele('allow-intent', {
          href: pattern
        });
      } else if (type === 'navigation') {
        config.ele('allow-navigation', Object.assign({
          href: pattern
        }, options));
      } else {
        config.ele('access', Object.assign({
          origin: pattern
        }, options));
      }
    });
    ...
    const platformElement = {
      ios: config.ele('platform', {
        name: 'ios'
      }),
      android: config.ele('platform', {
        name: 'android'
      })
  ...
  configureAndCopyImages(sizes, xmlElement, tag) {
    ...
    _.each(sizes, (size, name) => {
      ...
      let extension = _.last(suppliedFilename.split('.')); // XXX special case for 9-patch png's
      ...
      const src = files.pathJoin('resources', filename); // Copy the file to the build folder with a standardized name

      files.copyFile(files.pathResolve(this.projectContext.projectDir, suppliedPath), files.pathJoin(this.resourcesPath, filename)); // Set it to the xml tree
  ...
  configureAndCopyResourceFiles(resourceFiles, iosElement, androidElement) {
    _.each(resourceFiles, resourceFile => {
      ...
      files.copyFile(filepath, files.pathJoin(this.projectRoot, "resource-files", filepath)); // And entry in config.xml
  ...
  copyWWW(bundlePath) {
    const wwwPath = files.pathJoin(this.projectRoot, 'www'); // Remove existing www

    files.rm_recursive(wwwPath); // Create www and www/application directories

    const applicationPath = files.pathJoin(wwwPath, 'application');
    files.mkdir_p(applicationPath); // Copy Cordova arch program from bundle to www/application

    const programPath = files.pathJoin(bundlePath, 'programs', CORDOVA_ARCH);
    files.cp_r(programPath, applicationPath); // Load program.json

    const programJsonPath = files.convertToOSPath(files.pathJoin(applicationPath, 'program.json'));
    const program = JSON.parse(files.readFile(programJsonPath, 'utf8')); // Load settings

    const settingsFile = this.options.settingsFile;
    const settings = settingsFile ? JSON.parse(files.readFile(settingsFile, 'utf8')) : {};
    const publicSettings = settings['public']; // Calculate client hash and append to program

    this.appendVersion(program, publicSettings); // Write program.json

    files.writeFile(programJsonPath, JSON.stringify(program), 'utf8');
    const bootstrapPage = this.generateBootstrapPage(applicationPath, program, publicSettings).await();
  ...
  appendVersion(program, publicSettings) {
    ...
    program.version = AUTOUPDATE_VERSION || WebAppHashing.calculateClientHash(program.manifest, null, configDummy);
    program.versionRefreshable = AUTOUPDATE_VERSION || WebAppHashing.calculateClientHash(program.manifest, type => type === "css", configDummy);
    program.versionNonRefreshable = AUTOUPDATE_VERSION || WebAppHashing.calculateClientHash(program.manifest, type => type !== "css", configDummy);
  }

  generateBootstrapPage(applicationPath, program, publicSettings) {
    const meteorRelease = release.current.isCheckout() ? "none" : release.current.name;
    ...
    const {
      Boilerplate
    } = loadIsopackage('boilerplate-generator');
    const boilerplate = new Boilerplate(CORDOVA_ARCH, manifest, {
      urlMapper: _.identity,
      pathMapper: path => files.convertToOSPath(files.pathJoin(applicationPath, path)),
      baseDataExtension: {
  ...
  copyBuildOverride() {
    const buildOverridePath = files.pathJoin(this.projectContext.projectDir, 'cordova-build-override');
...
function createAppConfiguration(builder) {
  ...
  return {
    ...
    setPreference: function (key, value, platform) {
      if (platform) {
        if (!['ios', 'android'].includes(platform)) {
          throw new Error("Unknown platform in App.setPreference: ".concat(platform, ". Valid platforms are: ios, android."));
        }
    ...
    icons: function (icons) {
      var validDevices = Object.keys(iconsIosSizes).concat(Object.keys(iconsAndroidSizes));

      _.each(icons, function (value, key) {
        if (!_.include(validDevices, key)) {
          Console.labelWarn("".concat(key, ": unknown key in App.icons configuration. The key may be deprecated."));
        }
      });
    ...
    launchScreens: function (launchScreens) {
      var validDevices = Object.keys(launchIosSizes).concat(Object.keys(launchAndroidSizes));

      _.each(launchScreens, function (value, key) {
        if (!_.include(validDevices, key)) {
          Console.labelWarn("".concat(key, ": unknown key in App.launchScreens configuration. The key may be deprecated."));
        }
      });
