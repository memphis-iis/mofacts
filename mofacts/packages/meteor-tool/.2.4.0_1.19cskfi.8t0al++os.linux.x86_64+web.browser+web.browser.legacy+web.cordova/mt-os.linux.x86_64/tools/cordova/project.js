...
class CordovaProject {
  ...
  createIfNeeded() {
    ...
    if (!files.exists(this.projectRoot)) {
      ...
      this.runCommands('creating Cordova project', () => Promise.asyncApply(() => {
        ...
        Promise.await(create(files.convertToOSPath(this.projectRoot), undefined, undefined, config));
      }), undefined, null);
    }
  ...
  prepareFromAppBundle(bundlePath, pluginVersions) {
    ...
    this.ensurePlatformsAreSynchronized(); // Temporary workaround for Cordova iOS bug until
...
on an OS X system.");
  ...
  ensurePlatformsAreSynchronized() {
    ...
    for (let platform of installedPlatforms) {
      if (!platforms.includes(platform) && CORDOVA_PLATFORMS.includes(platform)) {
