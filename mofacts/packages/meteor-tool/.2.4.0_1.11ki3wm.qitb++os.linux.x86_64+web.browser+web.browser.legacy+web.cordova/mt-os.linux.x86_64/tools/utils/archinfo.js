module.export({
  ...
  host: () => host,
  acceptableMeteorToolArches: () => acceptableMeteorToolArches,
  canSwitchTo64Bit: () => canSwitchTo64Bit,
  matches: () => matches,
  isLegacyArch: () => isLegacyArch,
  mapWhereToArches: () => mapWhereToArches,
...
function host() {
  if (!_host) {
    const run = function () {
      ...
      if (!result) {
        throw new Error("Can't get arch with ".concat(args.join(" "), "?"));
      }
    ...
    if (platform === "darwin") {
      ...
      // return other values.
      if (run('uname', '-p') !== "i386" || run('sysctl', '-n', 'hw.cpu64bit_capable') !== "1") {
      ...
      if (["x86_64", "amd64", "ia64"].includes(machine)) {
        ...
        throw new Error("Unsupported architecture: ".concat(machine));
      }
...
function acceptableMeteorToolArches() {
  if (os.platform() === "win32") {
    switch (utils.architecture()) {
      ...
      case "x86_64":
        return ["os.windows.x86_64", "os.windows.x86_32"];
    }
  }
...
function canSwitchTo64Bit() {
  ...
  return false && utils.architecture() === "x86_64" && host() === "os.windows.x86_32";
}

function matches(host, program) {
  return host.substr(0, program.length) === program && (host.length === program.length || host.substr(program.length, 1) === ".");
}

const legacyArches = ["web.browser.legacy", // It's important to include web.browser.legacy resources in the Cordova
...
function isLegacyArch(arch) {
  return legacyArches.some(la => matches(arch, la));
}

function mapWhereToArches(where) {
  const arches = []; // Shorthands for common arch prefixes:
  // "server" => os.*
  // "client" => web.*
...
function mostSpecificMatch(host, programs) {
  let best = null;
  const seen = {};
  programs.forEach(program => {
    if (seen[program]) {
      throw new Error("Duplicate architecture: ".concat(program));
    }
...
function leastSpecificDescription(programs) {
  if (programs.length === 0) {
    return '';
  } // Find the longest string


  const longest = String(max(programs, p => p.length)); // If everything else in the list is compatible with the longest,
  ...
  programs.forEach(program => {
    if (!matches(longest, program)) {
      throw new Error("Incompatible architectures: '".concat(program, "' and '").concat(longest, "'"));
    }
  });
  return longest;
}

function withoutSpecificOs(arch) {
...
//# sourceMappingURL=archinfo.js.map