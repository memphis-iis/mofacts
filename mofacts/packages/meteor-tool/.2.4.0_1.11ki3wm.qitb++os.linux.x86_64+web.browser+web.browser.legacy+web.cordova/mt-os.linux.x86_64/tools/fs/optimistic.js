module.export({
  ...
  optimisticLookupPackageJsonArray: () => optimisticLookupPackageJsonArray
});
let assert;
module.link("assert", {
  default(v) {
...
const optimisticLookupPackageJsonArray = wrap((absRootDir, relDir) => {
  const absPkgJsonPath = pathJoin(absRootDir, relDir, "package.json");
  const pkgJson = optimisticReadJsonOrNull(absPkgJsonPath); // Named package.json files always terminate the search. This
  ...
  const parentArray = optimisticLookupPackageJsonArray(absRootDir, relParentDir);
...
const optimisticIsSymbolicLink = wrap(path => {
  try {
    ...
    return (_lstat = lstat(path)) === null || _lstat === void 0 ? void 0 : _lstat.isSymbolicLink();
  } catch (e) {
    if (e.code !== "ENOENT") throw e;
    dependOnParentDirectory(path);
    return false;
