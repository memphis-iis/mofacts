!function (module1) {
  ...
  module1.export({
    getProdPackageNames: () => getProdPackageNames
  });
  let parse;
  module1.link("semver", {
    parse(v) {
  ...
  let getRebuildArgs;
  module1.link("../static-assets/server/npm-rebuild-args.js", {
    get(v) {
      getRebuildArgs = v;
    }
  ...
  let dirtyNodeModulesDirectory, optimisticLStat, optimisticStatOrNull, optimisticReadJsonOrNull, optimisticReaddir;
  module1.link("../fs/optimistic", {
    dirtyNodeModulesDirectory(v) {
      dirtyNodeModulesDirectory = v;
    },
  ...
  var meteorNpm = exports; // Expose the version of npm in use from the dev bundle.

  meteorNpm.npmVersion = npmVersion; // if a user exits meteor while we're trying to create a .npm
  ...
  var NpmFailure = function () {}; // Creates a temporary directory in which the new contents of the
  ...
  const getProdPackageNames = wrapOptimistic(nodeModulesDir => {
    const names = Object.create(null);
    const dirs = Object.create(null);
    const nodeModulesDirStack = []; // Returns true iff dir is a package directory.
    ...
    function walk(dir) {
      ...
      if (packageJsonStat && packageJsonStat.isFile()) {
        ...
        nodeModulesDirStack.push(nodeModulesDir); // Scan all dependencies except pkg.devDependencies.

        scanDeps(pkg.dependencies);
        scanDeps(pkg.peerDependencies);
        scanDeps(pkg.optionalDependencies);
        scanDeps(pkg.bundledDependencies); // This typo is also honored.

        scanDeps(pkg.bundleDependencies);
        assert.strictEqual(nodeModulesDirStack.pop(), nodeModulesDir);
        return true;
      }
  ...
  const currentVersions = {
    ...
    arch: process.arch,
    versions: _objectSpread({}, process.versions)
  };
  const currentVersionsJSON = JSON.stringify(currentVersions, null, 2) + "\n";
  ...
  function rebuildVersionsAreCompatible(pkgPath) {
    const versionFile = files.pathJoin(pkgPath, lastRebuildJSONFilename);
  ...
  meteorNpm.rebuildIfNonPortable = Profile("meteorNpm.rebuildIfNonPortable", function (nodeModulesDir) {
    ...
    const tempDir = files.pathJoin(nodeModulesDir, ".temp-" + utils.randomToken()); // There's a chance the basename of the original nodeModulesDir isn't
    ...
    files.mkdir_p(tempNodeModules); // Map from original package directory paths to temporary package
    // directory paths.

    const tempPkgDirs = {};
    dirsToRebuild.splice(0).forEach(pkgPath => {
      const tempPkgDir = tempPkgDirs[pkgPath] = files.pathJoin(tempNodeModules, files.pathRelative(nodeModulesDir, pkgPath)); // It's possible the pkgPath directory may have been deleted since we
      ...
      if (isDirectory(pkgPath)) {
        ...
        copyNpmPackageWithSymlinkedNodeModules(pkgPath, tempPkgDir); // Record the current process.versions so that we can avoid
    ...
    dirtyNodeModulesDirectory(nodeModulesDir); // If the `npm rebuild` command succeeded, overwrite the original
    // package directories with the rebuilt package directories.

    dirsToRebuild.forEach(function (pkgPath) {
      const actualNodeModulesDir = files.pathJoin(pkgPath, "node_modules");
      const actualNodeModulesStat = files.statOrNull(actualNodeModulesDir);
      ...
      if (actualNodeModulesStat && actualNodeModulesStat.isDirectory()) {
        ...
        const symlinkNodeModulesDir = files.pathJoin(tempPkgDirs[pkgPath], "node_modules");
  ...
  function copyNpmPackageWithSymlinkedNodeModules(fromPkgDir, toPkgDir) {
    ...
    files.readdir(nodeModulesFromPath).forEach(depPath => {
      ...
      const absDepToPath = files.pathJoin(nodeModulesToPath, depPath); // Try to symlink node_modules dependencies if possible (faster),
  ...
  const portableCache = Object.create(null); // Increment this version to trigger the full portability check again.
  ...
  const isPortable = Profile("meteorNpm.isPortable", dir => {
    ...
    const portableFile = files.convertToOSPath(files.pathJoin(dir, ".meteor-portable-" + portableVersion + ".json"));
    ...
    if (canCache) {
      ...
      const result = _.has(portableCache, portableFile) ? portableCache[portableFile] : optimisticReadJsonOrNull(portableFile, {
    ...
    const hasBuildScript = pkgJson && pkgJson.scripts && (pkgJson.scripts.preinstall || pkgJson.scripts.install || pkgJson.scripts.postinstall);
    const result = hasBuildScript ? false // Build scripts may not be portable.
    : optimisticReaddir(dir).every( // Ignore files that start with a ".", such as .bin directories.
    itemName => itemName.startsWith(".") || isPortable(files.pathJoin(dir, itemName)));
  ...
  meteorNpm.dependenciesArePortable = function (nodeModulesDir) {
    ...
    assert.ok(files.pathBasename(nodeModulesDir).startsWith("node_modules"), "Bad node_modules directory: " + nodeModulesDir); // Only check/write .meteor-portable files in each of the top-level
    // package directories.
  ...
  var makeNewPackageNpmDir = function (newPackageNpmDir) {
    ...
    files.mkdir_p(newPackageNpmDir); // create node_modules -- prevent npm install from installing
    // to an existing node_modules dir higher up in the filesystem

    files.mkdir(files.pathJoin(newPackageNpmDir, 'node_modules')); // create .gitignore -- node_modules shouldn't be in git since we
    ...
    files.writeFile(files.pathJoin(newPackageNpmDir, '.gitignore'), ['node_modules', ''
    /*git diff complains without trailing newline*/
    ].join('\n'));
  };

  var updateExistingNpmDirectory = function (packageName, newPackageNpmDir, packageNpmDir, npmDependencies, quiet) {
    ...
    _.each(npmDependencies, (version, name) => {
      npmTree.dependencies[name] = {
        version
      };
    });

    const minInstalledTree = minimizeDependencyTree(installedDependenciesTree);
    const minShrinkwrapTree = minimizeDependencyTree(shrinkwrappedDependenciesTree);
    ...
    if (_.isEmpty(npmDependencies)) {
      ...
      installNpmDependencies(npmDependencies, newPackageNpmDir); // Note: as of npm@4.0.0, npm-shrinkwrap.json files are regarded as
    ...
    if (!_.isEmpty(preservedShrinkwrap && preservedShrinkwrap.dependencies)) {
      const newShrinkwrapFile = files.pathJoin(newPackageNpmDir, 'npm-shrinkwrap.json'); // There are some unchanged packages here. Install from shrinkwrap.

      files.writeFile(newShrinkwrapFile, JSON.stringify(preservedShrinkwrap, null, 2));
      const newPackageJsonFile = files.pathJoin(newPackageNpmDir, "package.json"); // We have to write out a minimal package.json file, else the results
      // of installFromShrinkwrap may be incomplete in npm@5.

      files.writeFile(newPackageJsonFile, JSON.stringify({
  ...
  function isSubtreeOf(subsetTree, supersetTree, predicate) {
    ...
    if (_.isObject(subsetTree)) {
      return _.isObject(supersetTree) && _.every(subsetTree, (value, key) => {
  ...
  var createFreshNpmDirectory = function (packageName, newPackageNpmDir, packageNpmDir, npmDependencies, quiet) {
    if (!quiet) {
      logUpdateDependencies(packageName, npmDependencies);
    }
  ...
  function installNpmDependencies(dependencies, dir) {
    ...
    files.writeFile(packageJsonPath, JSON.stringify({
      dependencies
    }, null, 2));
  ...
  function completeNpmDirectory(packageName, newPackageNpmDir, packageNpmDir, npmDependencies) {
    ...
    shrinkwrap(newPackageNpmDir); // And stow a copy of npm-shrinkwrap too.
  ...
  const npmUserConfigFile = files.pathJoin(__dirname, "meteor-npm-userconfig");
  var runNpmCommand = meteorNpm.runNpmCommand = Profile("meteorNpm.runNpmCommand", function (args, cwd) {
    let getEnv;
    module1.link("../cli/dev-bundle-bin-helpers.js", {
      getEnv(v) {
        getEnv = v;
      }
    ...
    const npmPath = files.convertToOSPath(files.pathJoin(devBundleDir, "bin", isWindows ? "npm.cmd" : "npm")); // On Windows, `.cmd` and `.bat` files must be launched in a shell per:
    ...
    if (meteorNpm._printNpmCalls) {
      ...
      process.stdout.write('cd ' + cwd + ' && ' + commandToRun + ' ' + args.join(' ') + ' ...\n');
    }
    ...
    return getEnv({
      ...
      return new Promise(function (resolve) {
        require('child_process').execFile(commandToRun, args, opts, function (err, stdout, stderr) {
          if (meteorNpm._printNpmCalls) {
          ...
          resolve({
            success: !err,
            error: err ? "".concat(err.message).concat(stderr) : stderr,
            stdout: stdout,
            stderr: stderr
          });
        });
    ...
    function ls(nodeModulesDir) {
      ...
      contents.forEach(item => {
        ...
        if (from) {
          ...
          if (utils.isNpmUrl(fromUrl) && !utils.isNpmUrl(info.version)) {
  ...
  function getShrinkwrappedDependenciesTree(dir) {
    const shrinkwrap = JSON.parse(files.readFile(files.pathJoin(dir, 'npm-shrinkwrap.json')));
  ...
  const installNpmModule = meteorNpm.installNpmModule = (name, version, dir) => {
    const installArg = utils.isNpmUrl(version) ? version : "".concat(name, "@").concat(version); // We don't use npm.commands.install since we couldn't figure out
    ...
    if (!result.success) {
      const pkgNotFound = "404 Not Found - GET ".concat(utils.quotemeta("https://registry.npmjs.org/" + name));
      const versionNotFound = "No matching version found for " + "".concat(utils.quotemeta(name), "@").concat(utils.quotemeta(version));

      if (result.stderr.match(new RegExp(pkgNotFound))) {
        buildmessage.error("there is no npm package named '".concat(name, "' in the npm registry"));
      } else if (result.stderr.match(new RegExp(versionNotFound))) {
        buildmessage.error("".concat(name, " version ").concat(version, " is not available in the npm registry"));
      } else {
        buildmessage.error("couldn't install npm package ".concat(name, "@").concat(version, ": ").concat(result.error));
  ...
  var installFromShrinkwrap = function (dir) {
    ...
    if (!result.success) {
      buildmessage.error("couldn't install npm packages from npm-shrinkwrap: " + result.error); // Recover by returning false from updateDependencies
  ...
  function shrinkwrap(dir) {
    ...
    const packageLockJsonPath = files.pathJoin(dir, "package-lock.json"); // The normal `npm shrinkwrap` commands renames any package-lock.json
  ...
  function minimizeDependencyTree(tree) {
    ...
    return {
      dependencies: newTopLevelDependencies
    };
  }
  ...
  var logUpdateDependencies = function (packageName, npmDependencies) {
    runLog.log(packageName + ': updating npm dependencies -- ' + Object.keys(npmDependencies).join(', ') + '...');
  };
}.call(this, module);
