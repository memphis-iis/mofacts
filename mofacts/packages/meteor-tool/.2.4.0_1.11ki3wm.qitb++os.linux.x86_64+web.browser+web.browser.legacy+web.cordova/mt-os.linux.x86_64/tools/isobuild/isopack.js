...
let fsFixPath;
module.link("../fs/fsFixPath", {
  "*"(v) {
    fsFixPath = v;
  }

}, 2);
let ISOPACKETS, makeIsopacketBuildContext;
module.link("../tool-env/isopackets.js", {
  ISOPACKETS(v) {
    ISOPACKETS = v;
  },

  makeIsopacketBuildContext(v) {
    makeIsopacketBuildContext = v;
  }

}, 3);
let requestGarbageCollection;
module.link("../utils/gc.js", {
  requestGarbageCollection(v) {
    requestGarbageCollection = v;
  }

}, 4);
let Unibuild;
module.link("./unibuild.js", {
  Unibuild(v) {
    Unibuild = v;
  }
...
var Isopack = function () {
  var self = this; // These have the same meaning as in PackageSource.
  ...
  self.prodOnly = false;
  self.testOnly = false; // Unibuilds, an array of class Unibuild.

  self.unibuilds = []; // Plugins in this package. Map from plugin name to {arch -> JsImage}.
  ...
  self.plugins = {};
  self.cordovaDependencies = {}; // isobuild:* pseudo-packages which this package depends on.

  self.isobuildFeatures = []; // -- Information for up-to-date checks --
  ...
  self.pluginWatchSet = new watch.WatchSet(); // -- Loaded plugin state --
  // True if plugins have been initialized (if ensurePluginsInitialized has
  // been called)

  self._pluginsInitialized = false; // The SourceProcessors registered by plugins defined by this package.  Each
  ...
  self.includeTool = false; // This is tools to copy from trees on disk. This is used by the
  // isopack-merge code in tropohouse.

  self.toolsOnDisk = []; // A map of package dependencies that can provide a plugin for this isopack.
  // In practice, it is every direct dependency and implied packages.

  self.pluginProviderPackageMap = null; // A directory on disk that plugins can use for caching. Should be created
  // by the code that initializes the Isopack. If not provided, plugins don't
  // get a disk cache.

  self.pluginCacheDir = null; // An in-memory only buildmessage.MessageSet object that is printed by the
...
Isopack.knownFormats = ["unipackage-pre2", "isopack-1", "isopack-2"]; // These functions are designed to convert isopack metadata between
...
Isopack.convertOneStepForward = function (data, fromFormat) {
  var convertedData = _.clone(data); // XXX COMPAT WITH 0.9.3
...
Isopack.convertIsopackFormat = Profile("Isopack.convertIsopackFormat", function (data, fromFormat, toFormat) {
...
Isopack.readMetadataFromDirectory = Profile("Isopack.readMetadataFromDirectory", function (isopackDirectory) {
  var metadata = null;
  let originalVersion = null; // deal with different versions of "isopack.json", backwards compatible
  ...
  if (files.exists(isopackJsonPath)) {
    ...
    if (isopackJson['isopack-2']) {
      ...
      metadata = Isopack.convertIsopackFormat(isopackJson['isopack-1'], 'isopack-1', 'isopack-2');
    ...
    if (files.exists(unipackageJsonPath)) {
      ...
      metadata = Isopack.convertIsopackFormat(metadata, "unipackage-pre2", "isopack-2");
    ...
    if (metadata.format !== "unipackage-pre2") {
      ...
      throw new Error("Unsupported isopack format: " + JSON.stringify(metadata.format));
    }
  }
...
Object.assign(Isopack.prototype, {
  ...
  getSourceFilesUnderSourceRoot: Profile("Isopack#getSourceFilesUnderSourceRoot", function (sourceRoot) {
    var self = this;
    var sourceFiles = {};
    ...
    var addSourceFilesFromWatchSet = function (watchSet) {
      _.each(watchSet.files, function (hash, filename) {
        ...
        anySourceFiles = true;
        var relativePath = files.pathRelative(sourceRoot, filename); // We only want files that are actually under sourceRoot.
  ...
  architectures: Profile("Isopack#architectures", function () {
    ...
    var arches = Object.keys(archSet).sort(); // Ensure that our buildArchitectures string does not look like
  ...
  getUnibuildAtArch: Profile("Isopack#getUnibuildAtArch", function (arch) {
    var self = this;
    let chosenArch = archinfo.mostSpecificMatch(arch, _.pluck(self.unibuilds, 'arch'));
    ...
    if (!chosenArch && arch.match(/^os\./)) {
      ...
      chosenArch = archinfo.mostSpecificMatch(archinfo.host(), _.pluck(self.unibuilds, 'arch'));
    }

    if (!chosenArch) {
      buildmessage.error((self.name || "this app") + " is not compatible with architecture '" + arch + "'", {
        secondary: true
    ...
    return _.findWhere(self.unibuilds, {
      arch: chosenArch
    });
  }),
  ...
  ensurePluginsInitialized: Profile("Isopack#ensurePluginsInitialized", function () {
    ...
    self.sourceProcessors.compiler = new buildPluginModule.SourceProcessorSet(self.displayName(), {
      hardcodeJs: true,
      singlePackage: true
    });
    self.sourceProcessors.linter = new buildPluginModule.SourceProcessorSet(self.displayName(), {
      singlePackage: true,
      allowConflicts: true
    });
    self.sourceProcessors.minifier = new buildPluginModule.SourceProcessorSet(self.displayName(), {
      singlePackage: true
    });

    _.each(self.plugins, function (pluginsByArch, name) {
      var arch = archinfo.mostSpecificMatch(archinfo.host(), Object.keys(pluginsByArch));

      if (!arch) {
        buildmessage.error("package `" + name + "` is built for incompatible " + "architecture"); // Recover by ignoring plugin
      ...
      buildmessage.enterJob({
        title: "loading plugin `" + name + "` from package `" + self.name + "`" // don't necessarily have rootPath anymore
  ...
  _makePluginApi: function (pluginName) {
    ...
    var Plugin = {
      ...
      _registerSourceProcessor: function (_ref, factory, _ref2) {
        let {
          extensions,
          filenames,
          archMatching,
        ...
        if (!isopack.featureEnabled(featurePackage)) {
          ...
          buildmessage.error("your package must `api.use('".concat(featurePackage, "@1.0.0')` in ") + "order for its plugins to call Plugin.".concat(methodName));
          return;
        }

        const hasExtensions = extensions && extensions instanceof Array && extensions.length > 0;
        const hasFilenames = filenames && filenames instanceof Array && filenames.length > 0;

        if (!(hasExtensions || hasFilenames)) {
          buildmessage.error("Plugin." + methodName + " must specify a " + "non-empty array of extensions or filenames", {
            useMyCaller: 3
          }); // recover by ignoring
        ...
        if (extensions && extensions.some(e => e.endsWith('/'))) {
          buildmessage.error("Plugin.".concat(methodName, ": extensions may not end in /")); // recover by ignoring

          return;
        }

        if (filenames && filenames.some(f => f.endsWith('/'))) {
          buildmessage.error("Plugin.".concat(methodName, ": filenames may not end in /")); // recover by ignoring

          return;
        }

        if (typeof factory !== 'function') {
          buildmessage.error(methodName + " call must " + "specify a factory function", {
            useMyCaller: 3
          }); // recover by ignoring
  ...
  initFromPath: Profile("Isopack#initFromPath", function (name, dir, options) {
    var self = this;
  ...
  _loadUnibuildsFromPath: function (name, dir, options) {
    var self = this;
    options = options || {}; // In the tropohouse, isopack paths are symlinks (which can be updated if
    ...
    var {
      metadata: mainJson
    } = Isopack.readMetadataFromDirectory(dir);
    ...
    if (_.has(mainJson, 'name') && name !== mainJson.name) {
      throw new Error("isopack " + name + " thinks its name is " + mainJson.name);
    ...
    if (options.isopackBuildInfoJson) {
      ...
      _.each(options.isopackBuildInfoJson.unibuildDependencies, function (watchSetJSON, unibuildTag) {
    ...
    _.each(mainJson.builds, function (unibuildMeta) {
      ...
      // directory)
      rejectBadPath(unibuildMeta.path); // Skip unibuilds we already have.
      ...
      const unibuild = Unibuild.fromJSON(JSON.parse(files.readFile(files.pathJoin(dir, unibuildMeta.path))), {
        isopack: self,
        kind: unibuildMeta.kind,
        arch: unibuildMeta.arch,
        unibuildBasePath: files.pathDirname(files.pathJoin(dir, unibuildMeta.path)),
        watchSet: unibuildWatchSets[unibuildMeta.path]
      });

      if (unibuild.uses) {
        unibuild.uses.forEach(use => {
          if (!use.weak && compiler.isIsobuildFeaturePackage(use.package) && self.isobuildFeatures.indexOf(use.package) === -1) {
    ...
    _.each(mainJson.tools, function (toolMeta) {
      toolMeta.rootDir = dir; // XXX check for overlap
  ...
  canWriteLegacyBuilds() {
    ...
    return this.unibuilds.every(unibuild => unibuild.resources.every(isResourceSafeForLegacyBuilds));
  },
  ...
  saveToPath: Profile("Isopack#saveToPath", function (outputDir) {
    ...
    try {
      ...
      const writeLegacyBuilds = includePreCompilerPluginIsopackVersions && self.canWriteLegacyBuilds();
      ...
      builder.reserve("isopack.json"); // Reserve this even if includeIsopackBuildInfo is not set, to ensure
      // nothing else writes it somehow.

      builder.reserve("isopack-buildinfo.json");
      builder.reserve("head");
      builder.reserve("body"); // Map from absolute path to npm directory in the unibuild, to the
      ...
      const npmDirsToCopy = Object.create(null); // Pre-linker versions of Meteor expect all packages in the warehouse to
      ...
      builder.write("package.js", {
        data: Buffer.from("// This file is included for compatibility with the Meteor " + "0.6.4 package downloader.\n", "utf8")
      });
      var unibuildInfos = []; // Unibuilds

      _.each(self.unibuilds, function (unibuild) {
        // Make up a filename for this unibuild
        var baseUnibuildName = unibuild.arch;
        var unibuildDir = builder.generateFilename(baseUnibuildName, {
          directory: true
        });
        var unibuildJsonFile = builder.generateFilename(baseUnibuildName + ".json");
        ...
        if (isopackBuildInfoJson) {
          isopackBuildInfoJson.unibuildDependencies[unibuildJsonFile] = unibuild.watchSet.toJSON();
        }

        const usesModules = !isopackCache || isopackCache.uses(self, "modules", unibuild.arch);
        ...
        const jsResourcesForLegacyPrelink = writeLegacyBuilds ? unibuild.getLegacyJsResources() : []; // Control file for unibuild
      ...
      _.each(self.plugins, function (pluginsByArch, name) {
        _.each(pluginsByArch, function (plugin) {
          ...
          var pluginDir = builder.generateFilename('plugin.' + colonConverter.convert(name) + '.' + plugin.arch, {
            directory: true
          });
      ...
      if (writeLegacyBuilds) {
        ...
        _.each(unibuildInfos, function (unibuildInfo) {
          ...
          var jsResourcesForLegacyPrelink = unibuildInfo.jsResourcesForLegacyPrelink;
          var legacyFilename = builder.generateFilename(unibuild.arch + '-legacy.json');
          ...
          _.each(unibuildJson.resources, function (resource) {
            ...
            } else if (resource.extension === 'js') {// Skip; we saved this in jsResourcesForLegacyPrelink above
              // already, in the format that linker.prelink understands.
            } else {
              throw Error("shouldn't write legacy builds for non-JS/CSS source " + JSON.stringify(resource));
            }
          });

          var prelinkFile, prelinkData, packageVariables;

          if (jsResourcesForLegacyPrelink.length === 1 && jsResourcesForLegacyPrelink[0].legacyPrelink) {
            ...
            prelinkFile = jsResourcesForLegacyPrelink[0]; // XXX It's weird that the type of object going in and out of
            ...
            packageVariables = jsResourcesForLegacyPrelink[0].legacyPrelink.packageVariables;
          } else {
            ...
            if (jsResourcesForLegacyPrelink.length) {
              ...
              var results = linker.prelink({
                ...
                combinedServePath: "/packages/" + colonConverter.convert(unibuild.pkg.name + (unibuild.kind === "main" ? "" : ":" + unibuild.kind) + ".js"),
                name: unibuild.pkg.name
              });

              if (results.files.length !== 1) {
                throw Error("prelink should return 1 file, not " + results.files.length);
              }
          ...
          if (prelinkFile && prelinkData) {
            var prelinkResource = {
              type: 'prelink',
              file: builder.writeToGeneratedFilename(files.pathJoin(legacyDir, prelinkFile.servePath), {
                data: prelinkData
              }),
            ...
            if (prelinkFile.sourceMap) {
              ...
              //      _will_ accept.
              var acceptedByBuffer = _.isString(prelinkFile.sourceMap) || _.isNumber(prelinkFile.sourceMap) || _.isArray(prelinkFile.sourceMap) || prelinkFile.sourceMap instanceof Buffer;
              ...
              prelinkResource.sourceMap = builder.writeToGeneratedFilename(files.pathJoin(legacyDir, prelinkFile.servePath + '.map'), {
        ...
        builder.writeJson("unipackage.json", Isopack.convertIsopackFormat( // Note that mainLegacyJson is isopack-1 (has no "source" resources)
  ...
  _writeTool: Profile("Isopack#_writeTool", function (builder) {
    var self = this;
    var pathsToCopy = utils.runGitInCheckout('ls-tree', '-r', '--name-only', '--full-tree', 'HEAD', // The actual trees to copy!
    'tools', 'examples', 'LICENSE.txt', 'LICENSES', 'meteor', 'meteor.bat', 'scripts/admin/launch-meteor', 'packages/package-version-parser/package-version-parser.js', 'packages/meteor/define-package.js', 'packages/meteor/flush-buffers-on-exit-in-windows.js'); // Trim blank line and unnecessary examples.

    pathsToCopy = _.filter(pathsToCopy.split('\n'), function (f) {
      return f && !f.match(/^examples\/other/) && !f.match(/^examples\/unfinished/);
    });

    function shouldTranspile(path) {
      const parts = path.split("/");

      if (parts[0] === "tools" && (path.endsWith(".js") || path.endsWith(".ts"))) {
        ...
        if (parts[1] !== "node_modules" && parts[1] !== "tests") {
          return true;
        }
      }
    ...
    builder.reserve('isopackets', {
      directory: true
    });
    builder.write('.git_version.txt', {
      data: Buffer.from(gitSha, 'utf8')
    });
    builder.copyDirectory({
    ...
    requestGarbageCollection(); // Build all of the isopackets now, so that no build step is required when
  ...
  getStrongOrderedUsedAndImpliedPackages: Profile("Isopack#getStrongOrderedUsedAndImpliedPackages", function () {
    var self = this;
    var packages = {};
  ...
  _getServePath: function (pathInPackage) {
    ...
    return colonConverter.convert(files.pathJoin(serveRoot, // XXX or should everything in this API use slash already?
...
exports.OldIsopackFormatError = function () {
  ...
  this.toString = function () {
    return "old isopack format!";
  };
};
//# sourceMappingURL=isopack.js.map