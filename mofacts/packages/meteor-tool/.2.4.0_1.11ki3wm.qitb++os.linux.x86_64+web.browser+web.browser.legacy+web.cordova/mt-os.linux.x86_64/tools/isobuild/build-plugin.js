...
exports.SourceProcessor = function (options) {
  ...
  self.id = "".concat(options.isopack.displayName(), "#").concat(nextId++);
...
Object.assign(exports.SourceProcessor.prototype, {
  ...
  instantiatePlugin: function () {
    ...
    buildmessage.enterJob("running ".concat(self.methodName, " callback in package ") + self.isopack.displayName(), () => {
      try {
        self.userPlugin = buildmessage.markBoundary(self.factoryFunction).call(null); // If we have a disk cache directory and the plugin wants it, use it.
...
class SourceProcessorSet {
  constructor(myPackageDisplayName) {
    ...
    this._myPackageDisplayName = myPackageDisplayName; // If this represents the SourceProcessors *registered* by a single package
    // (vs those *available* to a package), use different error messages.

    this._singlePackage = singlePackage; // If this is being used for *compilers*, we hardcode *.js. If it is being
    // used for linters, we don't.

    this._hardcodeJs = !!hardcodeJs; // Multiple linters may be registered on the same extension or filename, but
    // not compilers.

    this._allowConflicts = !!allowConflicts; // Map from extension -> [SourceProcessor]

    this._byExtension = {}; // Map from basename -> [SourceProcessor]

    this._byFilename = {}; // This is just an duplicate-free list of all SourceProcessors in
    // byExtension or byFilename.

    this.allSourceProcessors = []; // extension -> { handler, packageDisplayName, isTemplate, archMatching }
  ...
  _conflictError(package1, package2, conflict) {
    if (this._singlePackage) {
      buildmessage.error("plugins in package ".concat(this._myPackageDisplayName, " define multiple ") + "handlers for ".concat(conflict));
    } else {
      buildmessage.error("conflict: two packages included in ".concat(this._myPackageDisplayName, " ") + "(".concat(package1, " and ").concat(package2, ") are both trying to handle ") + conflict);
    }
  }
  ...
  addSourceProcessor(sp) {
    ...
    this._addSourceProcessorHelper(sp, sp.filenames, this._byFilename, ''); // If everything conflicted, then the SourceProcessors will be in
    ...
    things.forEach(thing => {
      if (byThing.hasOwnProperty(thing)) {
        if (this._allowConflicts) {
          ...
          this._conflictError(sp.isopack.displayName(), byThing[thing][0].isopack.displayName(), errorPrefix + thing); // recover by ignoring this one
  ...
  addLegacyHandler(_ref) {
    ...
    if (this._byExtension.hasOwnProperty(extension)) {
      this._conflictError(packageDisplayName, this._byExtension[extension].isopack.displayName(), '*.' + extension); // recover by ignoring
    ...
    if (this._legacyHandlers.hasOwnProperty(extension)) {
      this._conflictError(packageDisplayName, this._legacyHandlers[extension].packageDisplayName, '*.' + extension); // recover by ignoring
  ...
  classifyFilename(filename, arch) {
    ...
    const parts = filename.split('.'); // don't use iteration functions, so we can return (and start at #1)
...
class SourceClassification {
  constructor(type) {
    ...
    const knownTypes = ['extension', 'filename', 'legacy-handler', 'wrong-arch', 'unmatched', 'meteor-ignore'];

    if (knownTypes.indexOf(type) === -1) {
      throw Error("Unknown SourceClassification type ".concat(type));
    ...
    if (type === 'extension' || type === 'filename') {
      if (sourceProcessors) {
        ...
        const filteredSourceProcessors = sourceProcessors.filter(sourceProcessor => sourceProcessor.relevantForArch(arch));
        ...
        throw Error("missing sourceProcessors for ".concat(type, "!"));
      }
    }
