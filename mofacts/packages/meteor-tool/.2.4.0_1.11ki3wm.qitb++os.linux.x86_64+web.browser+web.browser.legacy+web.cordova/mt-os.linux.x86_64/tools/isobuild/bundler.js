!function (module1) {
  ...
  const _excluded = ["rebuildBinaries"];
  let Builder;
  module1.link("./builder.js", {
    default(v) {
      Builder = v;
    }
  ...
  let JsFile, CssFile;
  module1.link("./minifier-plugin.js", {
    JsFile(v) {
      JsFile = v;
    },
  ...
  let addToTree, LinkerFile;
  module1.link("./linker.js", {
    addToTree(v) {
      addToTree = v;
    },
  ...
  let loadIsopackage;
  module1.link("../tool-env/isopackets.js", {
    loadIsopackage(v) {
      loadIsopackage = v;
    }

  }, 3);
  let CORDOVA_PLATFORM_VERSIONS;
  module1.link("../cordova", {
    CORDOVA_PLATFORM_VERSIONS(v) {
      CORDOVA_PLATFORM_VERSIONS = v;
    }
  ...
  let PackageRegistry;
  module1.link("../../packages/meteor/define-package.js", {
    PackageRegistry(v) {
      PackageRegistry = v;
    }

  }, 6);
  let optimisticLStatOrNull;
  ...
  const MINIFY_RENAMED_FUNCTION = Buffer.from('function __minifyJs(', 'utf8'); // files to ignore when bundling. node has no globs, so use regexps

  exports.ignoreFiles = [/~$/, /^\.#/, /^(\.meteor\/|\.git\/|Thumbs\.db|\.DS_Store\/?|Icon\r|ehthumbs\.db|\..*\.sw.|#.*#)$/
  ...
  */
  ...
  exports._mainJsContents = ["", "process.argv.splice(2, 0, 'program.json');", "process.chdir(require('path').join(__dirname, 'programs', 'server'));", 'require("./programs/server/runtime.js")({ cachePath: process.env.METEOR_REIFY_CACHE_DIR });', "require('./programs/server/boot.js');"].join("\n"); ///////////////////////////////////////////////////////////////////////////////
  ...
  class NodeModulesDirectory {
    constructor(_ref) {
      ...
      this.packageName = packageName; // The absolute path of the root directory of the app or package that
      // contains this node_modules directory.

      assert.strictEqual(typeof sourceRoot, "string");
      this.sourceRoot = sourceRoot; // The absolute path (on local disk) to a directory that contains
      // the built node_modules to use.

      assert.strictEqual(typeof sourcePath, "string");
      this.sourcePath = sourcePath; // The path (relative to the bundle root) where we would preferably
      // like the node_modules to be output.

      this.preferredBundlePath = preferredBundlePath; // Boolean indicating whether the node_modules directory is locally
      // accessible from other modules in the app or package.

      this.local = !!local; // A test package often shares its .sourcePath with the non-test
      // package, so it's important to be able to tell them apart,
      // especially when we'd like to treat .sourcePath as a unique key.

      this.isTestPackage = typeof packageName === "string" && /^local-test[:_]/.test(packageName); // Optionally, files to discard.
    ...
    getPreferredBundlePath(kind) {
      assert.ok(kind === "bundle" || kind === "isopack", kind);
      ...
      if (!isApp) {
        ...
        const name = colonConverter.convert(this.packageName.replace(/^local-test[:_]/, ""));
        ...
        if (relParts[0] === ".npm") {
          ...
          if (relParts[1] === "node_modules" && relParts[2] === "meteor" && relParts[3] === name) {
    ...
    static readDirsFromJSON(node_modules, _ref2) {
      let {
        rebuildBinaries = false
      } = _ref2,
          callerInfo = _objectWithoutProperties(_ref2, _excluded);
      ...
      function add(moreInfo, path) {
        const info = _objectSpread(_objectSpread({}, callerInfo), moreInfo);

        if (!info.packageName) {
          const parts = path.split("/");

          if (parts[0] === "npm" && parts[1] === "node_modules" && parts[2] === "meteor") {
            info.packageName = parts[3];
          } else if (parts.length === 3 && parts[0] === "npm" && parts[2] === "node_modules") {
        ...
        nodeModulesDirectories[info.sourcePath] = new NodeModulesDirectory(info);
      }
    ...
    getProdPackagePredicate() {
      ...
      return this._prodPackagePredicate = function isWithinProdPackage(path) {
        const parts = files.pathRelative(sourcePath, path).split(files.pathSep); // Normalize away trailing files.pathSep characters.
        ...
        if (parts[start] === ".bin") {
          ...
          if (fileStatus && fileStatus.isSymbolicLink()) {
            ...
            if (!realpath) {
              throw new Error("Broken symbolic link encountered at ".concat(path));
            }
  ...
  class File {
    constructor(options) {
      ...
      this.sourcePath = options.sourcePath; // Allows not calculating sri when it isn't needed

      this._skipSri = options.skipSri; // info is just for help with debugging the tool; it isn't written to disk or
      // anything.

      this.info = options.info || '?'; // If this file was generated, a sourceMap (as a string) with debugging
      // information, as well as the "root" that paths in it should be resolved
      // against. Set with setSourceMap.

      this.sourceMap = null;
      this.sourceMapRoot = null; // Where this file is intended to reside within the target's
      // filesystem.

      this.targetPath = null; // The URL at which this file is intended to be served, relative to
      // the base URL at which the target is being served (ignored if this
      // file is not intended to be served over HTTP).

      this.url = null; // If updates to this file will be handled with HMR

      this.replaceable = options.replaceable; // A prefix that will be prepended to this.url.
      // Prefixing is currently restricted to web.cordova URLs.

      if (options.arch.startsWith("web.") && // Using the isModern function from the modern-browsers package,
      ...
      options.arch !== "web.browser" && options.arch !== "web.browser.legacy") {
        this.urlPrefix = "/__" + options.arch.split(".").slice(1).join(".");
      } else {
        this.urlPrefix = "";
      ...
      this.cacheable = options.cacheable || false; // The node_modules directories that Npm.require() should search when
      ...
      this.nodeModulesDirectories = Object.create(null); // For server JS only. Assets associated with this slice; map from the path
    ...
    toString() {
      return "File: [info=".concat(this.info, "]");
    }
    ...
    setContents(b) {
      ...
      this._contents = b; // Bust the hash cache.
    ...
    setUrlToHash(fileAndUrlSuffix, urlSuffix) {
      urlSuffix = urlSuffix || "";
      this.url = this.urlPrefix + "/" + this.hash() + fileAndUrlSuffix + urlSuffix;
      this.cacheable = true;
  ...
  class Target {
    constructor(_ref3) {
      let {
        ...
        bundlerCacheDir // ... see subclasses for additional options

      } = _ref3;
      this.packageMap = packageMap;
      this.isopackCache = isopackCache;
      this.sourceRoot = sourceRoot; // Something like "web.browser" or "os" or "os.osx.x86_64"

      this.arch = arch; // All of the Unibuilds that are to go into this target, in the order
      // that they are to be loaded.

      this.unibuilds = []; // JavaScript files. List of File. They will be loaded at startup in
      // the order given.

      this.js = []; // On-disk dependencies of this target.

      this.watchSet = new watch.WatchSet(); // List of all package names used in this target.

      this.usedPackages = {}; // node_modules directories that we need to copy into the target (or
      ...
      this.nodeModulesDirectories = Object.create(null); // Static assets to include in the bundle. List of File.
      // For client targets, these are served over HTTP.

      this.asset = []; // The project's cordova plugins file (which lists plugins used directly by
      // the project).

      this.cordovaPluginsFile = cordovaPluginsFile; // A mapping from Cordova plugin name to Cordova plugin version number.
    ...
    make(_ref4) {
      ...
      buildmessage.enterJob("building for " + this.arch, () => {
        ...
        this._addDirectCordovaDependencies(); // Minify, with mode requested.
        ...
        const add = function (unibuild) {
          ...
          var processUnibuild = function (usedUnibuild) {
            if (onStack[usedUnibuild.id]) {
              buildmessage.error("circular dependency between packages " + unibuild.pkg.name + " and " + usedUnibuild.pkg.name); // recover by not enforcing one of the depedencies
    ...
    _runCompilerPlugins(_ref6) {
      ...
      const target = this;
      const linkerCacheDir = this.bundlerCacheDir && files.pathJoin(this.bundlerCacheDir, "linker");
      const scannerCacheDir = this.bundlerCacheDir && files.pathJoin(this.bundlerCacheDir, "scanner");
      const processor = new compilerPluginModule.CompilerPluginProcessor({
        ...
        minifyCssResource(resource) {
          if (!minifiersByExt.css || minifyMode === "development") {
          ...
          file.setTargetPathFromRelPath(stripLeadingSlash(resource.servePath));
    ...
    _emitResources(sourceBatches) {
      let onJsOutputFiles = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : () => {};
      buildmessage.assertInJob();
      const isWeb = archinfo.matches(this.arch, 'web');
      const isOs = archinfo.matches(this.arch, 'os');
      const jsOutputFilesMap = compilerPluginModule.PackageSourceBatch.computeJsOutputFilesMap(sourceBatches);
      sourceBatches.forEach(batch => {
        const {
          unibuild,
          sourceRoot
        } = batch; // Depend on the source files that produced these resources.

        this.watchSet.merge(unibuild.watchSet); // Remember the versions of all of the build-time dependencies
      ...
      const versions = {};
      const dynamicImportFiles = new Set(); // Copy their resources into the bundle in order
      ...
      sourceBatches.forEach(sourceBatch => {
        ...
        if (this.cordovaDependencies) {
          _.each(unibuild.pkg.cordovaDependencies, (version, name) => {
            this._addCordovaDependency(name, version, // use newer version if another version has already been added
            false);
          });
        }

        const name = unibuild.pkg.name || null;
        const isApp = !name; // Emit the resources

        const resources = sourceBatch.getResources(jsOutputFilesMap.get(name).files, (linkCacheKey, jsResources) => onJsOutputFiles(jsResources, sourceBatch, linkCacheKey)); // First, find all the assets, so that we can associate them with each js
        ...
        resources.forEach(resource => {
          ...
          if (file.urlPrefix.length > 0) {
            ...
            noPrefix.urlPrefix = ""; // If the file has a URL prefix, add another resource for this
          ...
          assetFiles.forEach(f => {
            const relPath = isOs ? files.pathJoin('assets', resource.servePath) : stripLeadingSlash(resource.servePath);
      ...
      dynamicImportFiles.forEach(file => {
        file.setContents(Buffer.from(file.contents("utf8").replace("__DYNAMIC_VERSIONS__", () => JSON.stringify(versions.dynamic || {})), "utf8"));
      ...
      sourceBatches.forEach(batch => {
        batch.resourceSlots.forEach(slot => {
          const plugin = slot.sourceProcessor && slot.sourceProcessor.userPlugin;
        ...
        batch.unibuild.resources.forEach(resource => {
          ...
          assert.strictEqual(typeof resource._dataUsed, "boolean");
    ...
    minifyJs(minifierDef, minifyMode) {
      ...
      var markedMinifier = buildmessage.markBoundary(minifierDef.userPlugin.processFilesForBundle, minifierDef.userPlugin);
      buildmessage.enterJob('minifying app code', function () {
        try {
          Promise.all([markedMinifier(staticFiles, {
            minifyMode
          }), ...dynamicFiles.map(file => markedMinifier([file], {
            minifyMode
          }))]).await();
        } catch (e) {
      ...
      function handle(source, dynamic) {
        ...
        source._minifiedFiles.forEach(file => {
          ...
          const replaceable = possiblyReplaceable && file.data.equals(source._source.contents());
          ...
          if (file.stats && !dynamic && minifyMode === "production") {
            ...
            statsFile.targetPath = newFile.targetPath.replace(/\.js\b/, ".stats.json");
            statsFile.cacheable = true;
            statsFile.type = "json";

            if (statsFile.url !== newFile.url && statsFile.targetPath !== newFile.targetPath) {
    ...
    _addCordovaDependency(name, version, override) {
      ...
      if (override) {
        ...
        if (_.has(this.cordovaDependencies, id)) {
          ...
          const versionToSet = packageVersionParser.lessThan(existingVersion, version) ? version : existingVersion;
    ...
    _addDirectCordovaDependencies() {
      ...
      _.each(this.cordovaPluginsFile.getPluginVersions(), (version, name) => {
        this._addCordovaDependency(name, version, true
    ...
    mostCompatibleArch() {
      return archinfo.leastSpecificDescription(_.pluck(this.unibuilds, 'arch').filter(arch => archinfo.matches(this.arch, arch)));
    }

  } // mark methods for profiling


  ['make', '_runCompilerPlugins', '_emitResources', 'minifyJs', 'rewriteSourceMaps'].forEach(method => {
    Target.prototype[method] = Profile("Target#".concat(method), Target.prototype[method]);
  ...
  function addNodeModulesDirToObject(nmd, obj) {
    if (_.has(obj, nmd.sourcePath)) {
      const old = obj[nmd.sourcePath]; // If the old NodeModulesDirectory object is not a test package, or
      // the new one is a test package, keep the old one.

      if (!old.isTestPackage || nmd.isTestPackage) {
        return false;
      }
    }
  ...
  class ClientTarget extends Target {
    constructor(options) {
      super(options); // CSS files. List of File. They will be loaded in the order given.

      this.css = []; // List of segments of additional HTML for <head>/<body>.
    ...
    write(builder, _ref7) {
      ...
      builder.reserve("program.json"); // Helper to iterate over all resources that we serve over HTTP.
      ...
      }.bind(this); // Reserve all file names from the manifest, so that interleaved
      ...
      eachResource((file, type) => {
        ...
        if (file.sourceMap) {
          let mapData = null; // don't need to do this in devel mode
          ...
          manifestItem.sourceMap = builder.writeToGeneratedFilename(file.targetPath + '.map', {
            data: mapData
          }); // Use a SHA to make this cacheable.

          const sourceMapBaseName = file.hash() + '.map';
          manifestItem.sourceMapUrl = require('url').resolve(file.url, sourceMapBaseName);
        ...
        manifestItem.type = "dynamic js"; // Add the dynamic module to the manifest so that it can be
      ...
      ['head', 'body'].forEach(type => {
        ...
        if (data) {
          ...
          const dataFile = builder.writeToGeneratedFilename(type + '.html', {
            data: dataBuffer
          });
          manifest.push({
            path: dataFile,
      ...
      if (this.arch === 'web.cordova') {
        ...
        const {
          WebAppHashing
        } = loadIsopackage('webapp-hashing');

        const cordovaCompatibilityVersions = _.object(_.map(CORDOVA_PLATFORM_VERSIONS, (version, platform) => {
          const pluginsExcludedFromCompatibilityHash = (process.env.METEOR_CORDOVA_COMPAT_VERSION_EXCLUDE || '').split(',');
          const cordovaDependencies = Object.assign(Object.create(null), _.omit(this.cordovaDependencies, pluginsExcludedFromCompatibilityHash));
          const hash = process.env["METEOR_CORDOVA_COMPAT_VERSION_".concat(platform.toUpperCase())] || WebAppHashing.calculateCordovaCompatibilityHash(version, cordovaDependencies);
  ...
  const minifyCssFiles = Profile("minifyCssFiles", wrap(function (files, _ref8) {
    let {
      arch,
      minifier,
      minifyMode
    ...
    const markedMinifier = buildmessage.markBoundary(minifier.userPlugin.processFilesForBundle, minifier.userPlugin);
    buildmessage.enterJob('minifying app stylesheet', function () {
      try {
        Promise.await(markedMinifier(sources, {
          minifyMode
        }));
      } catch (e) {
        buildmessage.exception(e);
    ...
    makeCacheKey(files, _ref9) {
      let {
        arch,
        minifier,
        minifyMode
      } = _ref9;
      return defaultMakeCacheKey(minifier, arch, minifyMode, hashOfFiles(files));
    }
  ...
  ['minifyCss', 'write'].forEach(method => {
    ClientTarget.prototype[method] = Profile("ClientTarget#".concat(method), ClientTarget.prototype[method]);
  ...
  class JsImage {
    constructor() {
      ...
      this.jsToLoad = []; // node_modules directories that we need to copy into the target (or
      // otherwise make available at runtime). A map from an absolute path
      // on disk (NodeModulesDirectory.sourcePath) to a
      // NodeModulesDirectory object that we have created to represent it.
      //
      ...
      this.nodeModulesDirectories = Object.create(null); // Architecture required by this image
    ...
    load(bindings) {
      var self = this;
      var ret = new PackageRegistry(); // XXX This is mostly duplicated from
      ...
      var getAsset = function (assets, assetPath, encoding, callback) {
        ...
        if (!callback) {
          if (!Fiber.current) {
            throw new Error("The synchronous Assets API can " + "only be called from within a Fiber.");
          }
      ...
      const devBundleLibNodeModulesDir = files.pathJoin(files.getDevBundle(), "lib", "node_modules");
      const appDir = files.findAppDir();
      const appNodeModules = appDir && files.pathJoin(appDir, "node_modules"); // Eval each JavaScript file, providing a 'Npm' symbol in the same
      ...
      _.each(self.jsToLoad, function (item) {
        ...
        var env = Object.assign({
          ...
          Npm: {
            require: Profile(function (name) {
              ...
              let fullPath; // Replace all backslashes with forward slashes, just in case
              ...
              if (resolved && resolved === name && !files.pathIsAbsolute(resolved)) {
              ...
              function tryLookup(nodeModulesPath, name) {
                ...
                var nodeModulesTopDir = files.pathJoin(nodeModulesPath, name.split("/", 1)[0]);

                if (files.exists(nodeModulesTopDir)) {
                  return fullPath = files.convertToOSPath(files.pathJoin(nodeModulesPath, name));
                }
              }
              ...
              if (!found && bindings.Plugin && typeof bindings.Plugin.name === "string") {
                ...
                const nmdSourcePaths = nodeModulesDirsByPackageName.get(bindings.Plugin.name);
              ...
              if (appDir && resolved) {
                const isOutsideAppDir = files.pathRelative(appDir, resolved).startsWith("..");
        ...
        try {
          ...
          buildmessage.exception(e); // Recover by skipping the rest of the load
    ...
    _makeNpmRequire(nodeModulesDirsByPackageName) {
      ...
      function npmResolve(id) {
        ...
        if (parts[start] === "node_modules" && parts[start + 1] === "meteor" && parts[start + 3] === "node_modules") {
        ...
        if (dirs && dirs.length > 0) {
          ...
          _.some(dirs, dir => {
            const osPath = files.convertToOSPath(files.pathJoin(dir, relativePath));
    ...
    write(builder) {
      ...
      var self = this;
      builder.reserve("program.json"); // Finalize choice of paths for node_modules directories -- These
      ...
      _.each(self.nodeModulesDirectories || [], function (nmd) {
        ...
        if (!includeNodeModules || includeNodeModules === 'symlink') {
          ...
          // Fail hard.
          throw new Error("Option includeNodeModules wasn't falsy or 'symlink'. " + "It was: " + includeNodeModules);
        }
      ...
      var assetFilesBySha = {}; // JavaScript sources
      ...
      _.each(self.jsToLoad, function (item) {
        ...
        _.each(item.nodeModulesDirectories, nmd => {
          ...
          if (generatedNMD) {
            ...
            loadItem.node_modules[generatedNMD.preferredBundlePath] = generatedNMD.toJSON();
          }
        });
        ...
        if (item.sourceMap) {
          const sourceMapBuffer = Buffer.from(JSON.stringify(item.sourceMap), "utf8");
          loadItem.sourceMap = builder.writeToGeneratedFilename(item.targetPath + ".map", {
            data: sourceMapBuffer
          });
          const sourceMappingURL = "data:application/json;charset=utf8;base64," + sourceMapBuffer.toString("base64"); // Remove any existing sourceMappingURL line. (eg, if roundtripping
          // through JsImage.readFromDisk, don't end up with two!)

          sourceBuffer = addSourceMappingURL(item.source, sourceMappingURL, item.targetPath);
        ...
        loadItem.path = builder.writeToGeneratedFilename(item.targetPath, {
          data: sourceBuffer
        });
        ...
        if (!_.isEmpty(item.assets)) {
          ...
          _.each(item.assets, function (data, relPath) {
            ...
            if (_.has(assetFilesBySha, sha)) {
              ...
              loadItem.assets[relPath] = assetFilesBySha[sha] = builder.writeToGeneratedFilename(files.pathJoin(assetBundlePath, relPath), {
                data: data
              });
            }
          });
      ...
      _.each(nodeModulesDirectories, function (nmd) {
        assert.strictEqual(typeof nmd.preferredBundlePath, "string"); // Skip calculating isPortable in 'meteor run' since the
        ...
        if (nmd.sourcePath !== nmd.preferredBundlePath) {
          ...
          const prodPackagePredicate = // This condition essentially means we don't strip devDependencies
          ...
          (buildMode === "production" || buildMode === "development") && nmd.local && // Only filter local node_modules directories.
      ...
      builder.write("npm-rebuilds.json", {
        data: Buffer.from(JSON.stringify(Object.keys(rebuildDirs), null, 2) + "\n", "utf8")
      }); // Control file
    ...
    static readFromDisk(controlFilePath) {
      ...
      if (json.format !== "javascript-image-pre1") {
        throw new Error("Unsupported plugin format: " + JSON.stringify(json.format));
      }

      ret.arch = json.arch; // Rebuild binary npm packages if host arch matches image arch.
      ...
      _.each(json.load, function (item) {
        ...
        if (item.node_modules) {
          Object.assign(ret.nodeModulesDirectories, nodeModulesDirectories = NodeModulesDirectory.readDirsFromJSON(item.node_modules, {
            sourceRoot: dir,
            rebuildBinaries
          }));
        }
        ...
        if (item.sourceMap) {
          ...
          loadItem.sourceMap = JSON.parse(files.readFile(files.pathJoin(dir, item.sourceMap), 'utf8'));
          loadItem.sourceMapRoot = item.sourceMapRoot;
        }
  ...
  ['load', 'write'].forEach(method => {
    JsImage.prototype[method] = Profile("JsImage#".concat(method), JsImage.prototype[method]);
  });
  ...
  class ServerTarget extends JsImageTarget {
    ...
    write(builder, _ref10) {
      let {
        buildMode,
        // falsy or 'symlink', documented in exports.bundle
        includeNodeModules
      } = _ref10;
      var self = this; // This is where the dev_bundle will be downloaded and unpacked

      builder.reserve('dependencies'); // We will write out config.json, the dependency kit, and the
      ...
      const serverPkgJson = JSON.parse(files.readFile(files.pathJoin(files.getDevBundle(), 'etc', 'package.json')));
      serverPkgJson.scripts = serverPkgJson.scripts || {};
      serverPkgJson.scripts.install = "node npm-rebuild.js";
      serverPkgJson.dependencies["node-gyp"] = require("node-gyp/package.json").version;
      serverPkgJson.dependencies["node-pre-gyp"] = require("node-pre-gyp/package.json").version;
      builder.write('package.json', {
        data: Buffer.from(JSON.stringify(serverPkgJson, null, 2) + "\n", "utf8")
      });
      ...
      const toolsDir = files.pathDirname(files.convertToStandardPath(__dirname));
      builder.copyTranspiledModules(["profile.ts"], {
        sourceRootDir: files.pathJoin(toolsDir, "tool-env")
      }); // Server bootstrap

      builder.copyTranspiledModules(["boot.js", "boot-utils.js", "debug.ts", "server-json.js", "mini-files.ts", "npm-require.js", "npm-rebuild.js", "npm-rebuild-args.js", "runtime.js"], {
        sourceRootDir: files.pathJoin(toolsDir, "static-assets", "server"),
      ...
      if (archinfo.VALID_ARCHITECTURES[self.arch] !== true) {
        throw new Error("Meteor Software does not publish dev_bundles for arch: ".concat(self.arch));
  ...
  ['write'].forEach(method => {
    ServerTarget.prototype[method] = Profile("ServerTarget#".concat(method), ServerTarget.prototype[method]);
  });
  ...
  const newLineBuffer = Buffer.from("\n", "utf8"); // The data argument may be either a Buffer or a string, but this function
  ...
  function addSourceMappingURL(data, url, targetPath) {
    ...
    if (targetPath) {
      ...
      parts.push(newLineBuffer, Buffer.from("//# sourceURL=".concat(SOURCE_URL_PREFIX, "/").concat(targetPath), "utf8"));
    }

    parts.push(newLineBuffer, Buffer.from("//# sourceMappingURL=" + url, "utf8"), newLineBuffer // trailing newline
    ); // Always return a Buffer.
  ...
  var writeTargetToPath = Profile("bundler writeTargetToPath", function (name, target, outputPath, _ref11) {
    let {
      includeNodeModules,
  ...
  var writeSiteArchive = Profile("bundler writeSiteArchive", function (targets, outputPath, _ref12) {
    let {
      includeNodeModules,
      builtBy,
      releaseName,
    ...
    try {
      ...
      if (targets.server) {
        ...
        builder.write('README', {
          data: Buffer.from("This is a Meteor application bundle. It has only one external dependency:\nNode.js ".concat(process.version, ". To run the application:\n\n  $ (cd programs/server && npm install)\n  $ export MONGO_URL='mongodb://user:password@host:port/databasename'\n  $ export ROOT_URL='http://example.com'\n  $ export MAIL_URL='smtp://user:password@mailhost:port/'\n  $ node main.js\n\nUse the PORT environment variable to set the port where the\napplication will listen. The default is 80, but that will require\nroot on most systems.\n\nFind out more about Meteor at meteor.com.\n"), 'utf8')
        });
      ...
      builder.writeJson('star.json', json); // We did it!

      builder.complete(); // Now, go and "fix up" the outputPath properties of the sub-builders.
  ...
  function bundle(_ref13) {
    ...
    if (buildOptions.webArchs) {
      ...
      webArchs = _.intersection(buildOptions.webArchs, projectContext.platformList.getWebArchs());
    } else {
    ...
    var releaseName = release.current.isCheckout() ? "none" : release.current.name;
    var builtBy = "Meteor" + (release.current.name ? " " + release.current.name : "");
    var appIdentifier = projectContext.appIdentifier;
    var success = false;
    var serverWatchSet = new watch.WatchSet();
    var clientWatchSet = new watch.WatchSet();
    var starResult = null;
    var lintingMessages = null; // If delayed client builds are allowed, this array will be populated
    // with callbacks to run after the application process has started up.

    const postStartupCallbacks = allowDelayedClientBuilds && [];
    const bundlerCacheDir = projectContext.getProjectLocalDirectory('bundler-cache');
    ...
    var messages = buildmessage.capture({
      ...
      var makeClientTarget = Profile("bundler.bundle..makeClientTarget", function (app, webArch, options) {
        var client = new ClientTarget({
          ...
          arch: webArch,
          cordovaPluginsFile: webArch === 'web.cordova' ? projectContext.cordovaPluginsFile : null,
      ...
      var makeServerTarget = Profile("bundler.bundle..makeServerTarget", function (app, clientArchs) {
      ...
      const mergeAppWatchSets = () => {
        var projectAndLocalPackagesWatchSet = projectContext.getProjectAndLocalPackagesWatchSet();
      ...
      const hasOwn = Object.prototype.hasOwnProperty; // Write to disk
      ...
      function writeClientTarget(target) {
        ...
        const written = writeTargetToPath(arch, target, outputPath, _objectSpread({
          buildMode: buildOptions.buildMode,
      ...
      webArchs.forEach(arch => {
        if (allowDelayedClientBuilds && hasOwn.call(previousBuilders, arch) && projectContext.platformList.canDelayBuildingArch(arch)) {
          ...
          postStartupCallbacks.push(_ref14 => Promise.asyncApply(() => {
            let {
              pauseClient,
              refreshClient,
              runLog
            } = _ref14;
            const start = +new Date(); // Build the target first.

            const target = makeClientTarget(app, arch, {
              minifiers
            ...
            Promise.await(pauseClient(arch).catch(ignoreHarmlessErrors)); // Now write the target to disk. Note that we are rewriting the
            // bundle in place, so this work is not atomic by any means,
            // which is why we needed to pause the client.

            writeClientTarget(target); // Refresh and unpause the client, now that writing is finished.
            // If the child process exited for some reason, don't worry if
            // this message fails.

            Promise.await(refreshClient(arch).catch(ignoreHarmlessErrors)); // Let the webapp package running in the child process know it
            // should regenerate the client program for this arch.

            if (Profile.enabled) {
              runLog.log("Finished delayed build of ".concat(arch, " in ").concat(new Date() - start, "ms"), {
                arrow: true
              });
            }
          }));
          ...
          targets[arch] = makeClientTarget(app, arch, {
            minifiers
          });
        }
      }); // Server
      ...
      if (outputPath !== null) {
        if (hasCachedBundle) {
          ...
          starResult = writeSiteArchive(targets, outputPath, _objectSpread({
            buildMode: buildOptions.buildMode,
            previousBuilders,
            sourceRoot: packageSource.sourceRoot
          }, writeOptions));
  ...
  function lintBundle(projectContext, isopack, packageSource) {
    ...
    const localPackagesMessages = projectContext.getLintingMessagesForLocalPackages();
  ...
  exports.readJsImage = Profile("bundler.readJsImage", function (controlFilePath) {
