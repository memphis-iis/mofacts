const _excluded = ["LANG", "LC_ALL", "LANGUAGE"];
...
let MongoExitCodes;
module.link("../utils/mongo-exit-codes", {
  MongoExitCodes(v) {
    MongoExitCodes = v;
  }
...
var Console = require('../console/console.js').Console; // Given a Mongo URL, open an interative Mongo shell on this terminal
// on that database.


var runMongoShell = function (url) {
  var mongoPath = files.pathJoin(files.getDevBundle(), 'mongodb', 'bin', 'mongo'); // XXX mongo URLs are not real URLs (notably, the comma-separation for
...
function spawnMongod(mongodPath, port, dbPath, replSetName) {
  ...
  const args = [// nb: cli-test.sh and findMongoPids make strong assumptions about the
  // order of the arguments! Check them before changing any arguments.
  '--bind_ip', process.env.METEOR_MONGO_BIND_IP || '127.0.0.1', '--port', port, '--dbpath', dbPath, // Use an 8MB oplog rather than 256MB. Uses less space on disk and
  // initializes faster. (Not recommended for production!)
  '--oplogSize', '8', '--replSet', replSetName, '--noauth', // Starting with version 4.0.8/4.1.10, MongoDB performs a step down
  ...
  '--setParameter', 'waitForStepDownOnNonCommandShutdown=false']; // Use mmapv1 on 32bit platforms, as our binary doesn't support WT
...
if (process.platform === 'win32') {
  ...
  findMongoPids = function (dbDir_unused, port) {
    ...
    child_process.exec('tasklist /fi "IMAGENAME eq mongod.exe"', function (error, stdout, stderr) {
      if (error) {
        ...
        promise.reject(new Error("Couldn't run tasklist.exe: " + additionalInfo));
        return;
      } else {
        ...
        child_process.exec('netstat -ano', {
          ...
          if (error) {
            ...
            stdout.split('\n').forEach(function (line) {
              ...
              if (m) {
                ...
                var found_port = parseInt(m[1], 10); // We can't check the path app_dir so assume it always matches
  ...
  findMongoPids = function (dbDir, port) {
    var promise = fiberHelpers.makeFulfillablePromise(); // 'ps ax' should be standard across all MacOS and Linux.
    ...
    if (process.platform === 'darwin') {
      psScript = 'if type pgrep >/dev/null 2>&1; then ' + // -lf means to display and match against full argument lists.
    ...
    const _process$env = process.env,
      ...
      LANGUAGE = LANG
          ...
          env = _objectWithoutProperties(_process$env, _excluded); // Make sure all three properties are set to the same value, which
    ...
    child_process.exec(psScript, {
      ...
      if (error) {
        promise.reject(new Error("Couldn't run ps ax: " + JSON.stringify(error) + "; " + error.message));
        return;
      }
...
var findMongoAndKillItDead = function (port, dbPath) {
  var pids = findMongoPids(null, port); // Go through the list serially. There really should only ever be
  // at most one but we're not taking any chances.

  _.each(pids, function (processInfo) {
    var pid = processInfo.pid; // Send kill attempts and wait. First a SIGINT, then if it isn't
...
var StoppedDuringLaunch = function () {}; // Starts a single instance of mongod, and configures it properly as a singleton
...
var launchMongo = function (options) {
  ...
  var noOplog = false;
  var mongod_path = files.pathJoin(files.getDevBundle(), 'mongodb', 'bin', 'mongod');
  var replSetName = 'meteor'; // Automated testing: If this is set, instead of starting mongod, we
  ...
  if (process.env.METEOR_TEST_FAKE_MONGOD_CONTROL_PORT) {
    ...
    var fakeMongodCommand = process.platform === "win32" ? "fake-mongod.bat" : "fake-mongod";
    mongod_path = files.pathJoin(files.getCurrentToolsDir(), 'tools', 'tests', 'fake-mongod', fakeMongodCommand); // oplog support requires sending admin commands to mongod, so
  ...
  var yieldingMethod = function (object, methodName) {
    ...
    return Promise.race([stopPromise, new Promise((resolve, reject) => {
  ...
  var launchOneMongoAndWaitForReadyForInitiate = function (dbPath, port, portFile) {
    files.mkdir_p(dbPath, 493);
    var proc = null;
    ...
    var procExitHandler = fiberHelpers.bindEnvironment(function (code, signal) {
      ...
      proc = null; // Kill any other processes too. This will also remove
      // procExitHandler from the other processes, so onExit will only be called
      // once.

      handle.stop(); // Invoke the outer onExit callback.
    ...
    var readyToTalkPromise = new Promise(function (resolve) {
      maybeReadyToTalk = function () {
        if (resolve && listening && (noOplog || replSetReadyToBeInitiated || replSetReady)) {
    ...
    var stdoutOnData = fiberHelpers.bindEnvironment(function (data) {
      ...
      // lines
      if (/replica set config in use/.test(data) || /Did not find local replica set configuration document at startup/.test(data) || /\[.*\] Locally stored replica set configuration does not have a valid entry for the current node/.test(data)) {
        replSetReadyToBeInitiated = true;
        maybeReadyToTalk();
      }

      if (/Waiting for connections/.test(data) || / \[.*\] waiting for connections on port/.test(data)) {
        listening = true;
        maybeReadyToTalk();
      }
  ...
  var initiateReplSetAndWaitForReady = function () {
    try {
      ...
      const {
        MongoClient,
        Server
      } = loadIsopackage('npm-mongo').NpmModuleMongodb; // Connect to the intended primary and start a replset.

      const client = new MongoClient(new Server('127.0.0.1', options.port, {
        poolSize: 1,
        socketOptions: {
      ...
      try {
        const config = yieldingMethod(db.admin(), "command", {
          replSetGetConfig: 1
        }).config; // If a replication set configuration already exists, it's
      ...
      let writableTimestamp = Date.now(); // Wait until the primary is writable. If it isn't writable after one
      ...
      client.close(true
      /* means "the app is closing the connection" */
      );
    } catch (e) {
...
Object.assign(MRp, {
  ...
  start: function () {
    ...
    self._startOrRestart(); // Did we properly start up? Great!
  ...
  _exited: function (code, signal, stderr, detectedErrors) {
    var self = this;
    self.handle = null; // If Mongo exited because (or rather, anytime after) we told it
    ...
    if (!self.suppressExitMessage) {
      ...
      runLog.log(stderr.split('\n').slice(-20).join('\n') + "Unexpected mongo exit code " + code + (self.multiple ? "." : ". Restarting."));
    ...
    if (explanation && explanation.symbol === 'EXIT_UNCAUGHT' && detectedErrors.freeSpace) {
      message += "\n\n" + "Looks like you are out of free disk space under .meteor/local.";
    } else if (explanation) {
      message += "\n" + explanation.longText;
    } else if (process.platform === 'win32') {
      message += "\n\n" + "Check how to troubleshoot here " + "https://docs.meteor.com/windows.html#cant-start-mongo-server";
    }
...
//# sourceMappingURL=run-mongo.js.map