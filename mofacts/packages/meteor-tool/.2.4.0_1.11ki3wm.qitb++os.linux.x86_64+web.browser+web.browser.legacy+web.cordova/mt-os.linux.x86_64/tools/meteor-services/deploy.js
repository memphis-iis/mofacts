!function (module1) {
  ...
  module1.export({
    ...
    deleteApp: () => deleteApp,
    temporaryMongoUrl: () => temporaryMongoUrl,
    listAuthorized: () => listAuthorized,
    changeAuthorized: () => changeAuthorized,
    listSites: () => listSites
  });
  let pathJoin, createTarGzStream, getSettings, mkdtemp, changeTempDirStatus, exists, findGitCommitHash;
  ...
  let request;
  module1.link("../utils/http-helpers.js", {
    request(v) {
      request = v;
    }
  ...
  let pollForRegistrationCompletion, doInteractivePasswordLogin, loggedInUsername, isLoggedIn, maybePrintRegistrationLink;
  module1.link("./auth.js", {
    pollForRegistrationCompletion(v) {
      pollForRegistrationCompletion = v;
    },

    doInteractivePasswordLogin(v) {
      doInteractivePasswordLogin = v;
    },
    ...
    maybePrintRegistrationLink(v) {
      maybePrintRegistrationLink = v;
    }

  }, 3);
  let recordPackages;
  module1.link("./stats.js", {
    recordPackages(v) {
      recordPackages = v;
    }
  ...
  const CAPABILITIES = ['showDeployMessages', 'canTransferAuthorization']; // Make a synchronous RPC to the "classic" Meteor Software deploy API. The
  ...
  function deployRpc(options) {
    ...
    }, options.deployWithTokenProps || {}); // If we are waiting for deploy, we let Galaxy know so it can
    ...
    if (options.operand) {
      operand = "/".concat(options.operand);
    } else if (options.site) {
      operand = "/".concat(options.site);
    } // XXX: Reintroduce progress for upload


    try {
      var result = request(Object.assign(options, {
        url: deployURLBase + '/' + options.operation + operand,
    ...
    if (response.statusCode !== 200) {
      if (body.length > 0) {
        ...
        ret.errorMessage = "Server error " + response.statusCode + " (please try again later)";
      }
    ...
    const hasAllExpectedKeys = (options.expectPayload || []).map(key => ret.payload && hasOwn.call(ret.payload, key)).every(x => x);

    if (options.expectPayload && !hasOwn.call(ret, 'payload') || options.expectMessage && !hasOwn.call(ret, 'message') || !hasAllExpectedKeys) {
      delete ret.payload;
      delete ret.message;
  ...
  function authedRpc(options) {
    ...
    delete rpcOptions.preflight; // Fetch auth info
    ...
    if (info.protection === "account") {
      if (!hasOwn.call(info, 'authorized')) {
        ...
        if (preflight) {
          ...
          return {
            statusCode: null,
            errorMessage: isLoggedIn() ? // XXX better error message (probably need to break out of
            // the 'errorMessage printed with brief prefix' pattern)
            "Not an authorized user on this site" : "Not logged in"
          };
        }
  ...
  function printUnauthorizedMessage() {
    ...
    Console.error();
    Console.error("Either have the site owner use " + Console.command("'meteor authorized --add'") + " to add you as an " + "authorized developer for the site, or switch to an authorized account " + "with " + Console.command("'meteor login'") + ".");
  }
  ...
  function canonicalizeSite(site) {
    ...
    if (site.length > 63) {
      Console.error("The maximum hostname length currently supported is 63 characters: " + site + " is too long. " + "Please try again with a shorter URL for your site.");
      return false;
    }
    ...
    if (!parsed.hostname) {
      Console.info("Please specify a domain to connect to, such as www.example.com or " + "http://www.example.com/");
      return false;
    }

    if (parsed.pathname != '/' || parsed.hash || parsed.query) {
      Console.info("Sorry, Meteor does not yet support specific path URLs, such as " + Console.url("http://www.example.com/blog") + " .  Please specify the root of a domain.");
      return false;
    }
  ...
  function pollForDeploymentSuccess(versionId, deployPollTimeout, result, site, deployWithTokenProps) {
    return Promise.asyncApply(() => {
      ...
      Promise.await(sleepForMilliseconds(pollingState.initialWaitTimeMs));
      const deploymentPollResult = Promise.await(pollForDeploy(pollingState, versionId, site, deployWithTokenProps));
  ...
  class PollingState {
    constructor(timeoutMs, initialWaitTimeMs, pollIntervalMs, maxErrors) {
      const FIFTEEN_MINUTES_MS = 15 * 60 * 1000;
      const MAX_ERRORS = 5;
      this.initialWaitTimeMs = initialWaitTimeMs || 10 * 1000;
      this.pollIntervalMs = pollIntervalMs || 700;
      this.deadline = timeoutMs ? new Date(new Date().getTime() + timeoutMs) : new Date(new Date().getTime() + FIFTEEN_MINUTES_MS);
      this.start = new Date();
      this.currentMessage = '';
      this.errors = 0;
  ...
  function pollForDeploy(pollingState, versionId, site, deployWithTokenProps) {
    return Promise.asyncApply(() => {
      const {
        deadline,
        pollIntervalMs,
        currentMessage
      } = pollingState; // Do a call to the version-status endpoint for the specified versionId
      ...
      if (versionStatusResult && versionStatusResult.payload && versionStatusResult.payload.message) {
        ...
        if (pollingState.errors >= pollingState.maxErrors) {
          Console.error("Error checking deploy status; giving up: ".concat(errorMessage));
          return 1;
        } else if (new Date() < deadline) {
          Console.warn("Error checking deploy status; will retry: ".concat(errorMessage));
          Promise.await(sleepForMilliseconds(pollIntervalMs));
          return Promise.await(pollForDeploy(pollingState, versionId, site, deployWithTokenProps));
        }
      }

      const finishStatus = versionStatusResult.payload.finishStatus; // Poll again if version isn't finished and we haven't exceeded the timeout
  ...
  function bundleAndDeploy(options) {
    return Promise.asyncApply(() => {
      ...
      if (options.isBuildOnly) {
        ...
        if (!site) {
          ...
          Console.error("For more help, see " + Console.command("'meteor deploy --help'") + ".");
          return 1;
        ...
        const promptIfAuthFails = loggedInUsername() !== null; // Check auth up front, rather than after the (potentially lengthy)
        // bundling process.
        ...
        const preflight = authedRpc({
          ...
          qs: Object.assign({}, options.rawOptions, {
        ...
        if (preflight.protection === "account" && !preflight.authorized) {
      ...
      if (options.isCacheBuildEnabled) {
        if (!buildCache || !exists(buildCache.buildDir) || !exists(buildCache.bundlePath) || !buildCache.gitCommitHash || !gitCommitHash || buildCache.gitCommitHash !== gitCommitHash) {
      ...
      function getBuildDirAndBundlePath() {
        ...
        if (options.isCacheBuildEnabled) {
          ...
          Console.info("The --cache-build was used so the build folder (".concat(buildDir, ") will not be deleted on exit..."));
        }
      ...
      const result = buildmessage.enterJob({
        ...
        return authedRpc({
          ...
          operation: 'deploy',
          site: site,
          qs: Object.assign({}, options.rawOptions, settings !== null ? {
            settings: settings
          } : {}, _objectSpread({
            free: options.free,
            plan: options.plan,
      ...
      if (options.waitForDeploy && result.payload.newVersionId) {
        ...
        return Promise.await(pollForDeploymentSuccess(result.payload.newVersionId, options.deployPollingTimeoutMs, result, site, deployWithTokenProps));
      }
  ...
  function checkAuthThenSendRpc(site, operation, what) {
    ...
    if (preflight.protection === "account" && !preflight.authorized) {
      if (!isLoggedIn()) {
        ...
        var loginResult = doUsernamePasswordLogin({
          retry: true
        });
        ...
        if (loginResult) {
          ...
          Console.error("\nYou must be logged in to " + what + " for this app. Use " + Console.command("'meteor login'") + "to log in.");
          Console.error();
          Console.error("If you don't have a Meteor developer account yet, you can quickly " + "create one at www.meteor.com.");
          return null;
        }
      } else {
        // User is logged in but not authorized for this app
        Console.error();
        printUnauthorizedMessage();
        return null;
      }
  ...
  function temporaryMongoUrl(site) {
    site = canonicalizeSite(site);
  ...
  function listAuthorized(site) {
    ...
    if (info.protection === "account") {
      if (!hasOwn.call(info, 'authorized')) {
        Console.error("Couldn't get authorized users list: " + "You are not authorized");
        return 1;
      }
  ...
  function changeAuthorized(site, action, username) {
    site = canonicalizeSite(site);
    ...
    Console.info("".concat(site, ": ").concat(verbs[action], " ").concat(username));
    return 0;
  }
  ...
  function listSites() {
    ...
    if (!result.payload || !result.payload.sites || !result.payload.sites.length) {
      Console.info("You don't have any sites yet.");
    } else {
      result.payload.sites.sort().forEach(site => Console.info(site));
    }
  ...
  const galaxyDiscoveryCache = new Map(); // getDeployURL returns the a Promise for the base deploy URL for the given app.
  ...
  function getDeployURL(site) {
    ...
    const defaultURL = "https://us-east-1.galaxy-deploy.meteor.com"; // No site? Just use the default.
    ...
    const p = discoverGalaxy(site, "https").catch(() => discoverGalaxy(site, "http")).catch(() => defaultURL);
  ...
  function discoverGalaxy(site, scheme) {
    return Promise.asyncApply(() => {
      const discoveryURL = scheme + "://" + site + "/.well-known/meteor/deploy-url"; // If httpHelpers.request throws, the returned Promise will reject, which is
      // fine.
      ...
      if (body.galaxyDiscoveryVersion !== "galaxy-1") {
        throw new Error("unexpected galaxyDiscoveryVersion: " + body.galaxyDiscoveryVersion);
      }
