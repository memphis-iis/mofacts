!function (module1) {
  ...
  var fiberHelpers = require("../utils/fiber-helpers.js"); // Wrapper to manage a connection to a DDP service. The main difference between
  ...
  var ServiceConnection = function (endpointUrl, options) {
    const self = this;
    const ddpClient = loadIsopackage('ddp-client'); // ServiceConnection never should retry connections: just one TCP connection
    ...
    self.connection = ddpClient.DDP.connect(endpointUrl, options); // Wait until we have some sort of initial connection or error (including the
    // 10-second timeout built into our DDP client).

    var connectPromise = self.currentPromise = fiberHelpers.makeFulfillablePromise();
    ...
    self.connection._stream.on('disconnect', function (error) {
      ...
      if (self.currentPromise) {
        ...
        self.currentPromise = null;
        promise.reject(error || new ddpClient.DDP.ConnectionError("DDP disconnected while connection in progress"));
      } else if (error) {
...
//# sourceMappingURL=service-connection.js.map