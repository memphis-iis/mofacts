...
function doRunCommand(options) {
  ...
  return runAll.run(_objectSpread({
...
main.registerCommand({
  ...
  if (!options.bare && !options.minimal && !options.full && !options.react && !options.typescript) {
...
main.registerCommand(_objectSpread({
  ...
  return Promise.asyncApply(() => {
    ...
    Console.error();
    return Profile.run("meteor bundle", () => Promise.await(buildCommand(_objectSpread({}, options, {
      _bundleOnly: true
    }))));
  });
});
