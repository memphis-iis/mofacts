...
var Profile = require('./profile').Profile; // TL;DR: Isopacket is a set of isopacks. Isopackets are used only inside
// meteor-tool.
...
const ISOPACKETS = {
  ...
  'ddp-client', // mongo
  'npm-mongo', // ejson
  'ejson', // constraint-solver
  'constraint-solver', // cordova-support
  'boilerplate-generator', 'webapp-hashing', 'xmlbuilder', // cordova-support, logging
  'logging', // support for childProcess.sendMessage(topic, payload)
...
// the tool itself.
var loadedIsopackets = {}; // The main entry point: loads the specified isopacket ("combined" by
...
function loadIsopackage(packageName) {
  ...
  const isopacket = function () {
    if (_.has(loadedIsopackets, isopacketName)) {
      ...
      return loadedIsopackets[isopacketName] = loadIsopacketFromDisk(isopacketName);
    }
...
function ensureIsopacketsLoadable() {
  if (calledEnsure) {
    throw Error("can't ensureIsopacketsLoadable twice!");
  }

  calledEnsure = true; // If we're not running from checkout, then there's nothing to build and we
  ...
  var failedPackageBuild = false; // Look at each isopacket. Check to see if it's on disk and up to date. If
    ...
    return buildmessage.capture(function () {
      _.each(ISOPACKETS, function (packages, isopacketName) {
        ...
        buildmessage.enterJob({
          ...
          builder.complete(); // It's loadable now.
...
var newIsopacketBuildingCatalog = function () {
  ...
  var messages = buildmessage.capture({
    ...
    const packagesDir = files.pathJoin(files.getCurrentToolsDir(), 'packages'); // When running from a checkout, isopacket building does use local
...
var loadIsopacketFromDisk = function (isopacketName) {
  var image = bundler.readJsImage(files.pathJoin(isopacketPath(isopacketName), 'program.json')); // An incredibly minimalist version of the environment from
