  ...
  */
  ...
  class NodeModulesDirectory {
    ...
    static readDirsFromJSON(node_modules, _ref2) {
      ...
      function add(moreInfo, path) {
        const info = _objectSpread({}, callerInfo, {}, moreInfo);
    ...
    getProdPackagePredicate() {
      ...
      return this._prodPackagePredicate = function isWithinProdPackage(path) {
        ...
        if (parts[start] === ".bin") {
          ...
          if (typeof real === "string" && real !== path) {
  ...
  class File {
    constructor(options) {
      ...
      this.url = null; // A prefix that will be prepended to this.url.
  ...
  class Target {
    ...
    make(_ref4) {
      ...
      buildmessage.enterJob("building for " + this.arch, () => {
        ...
        this._emitResources(sourceBatches); // Add top-level Cordova dependencies, which override Cordova
    ...
    _emitResources(sourceBatches) {
      ...
      sourceBatches.forEach(sourceBatch => {
        ...
        const resources = sourceBatch.getResources(jsOutputFilesMap.get(name).files); // First, find all the assets, so that we can associate them with each js
    ...
    minifyJs(minifierDef, minifyMode) {
      ...
      this.js.forEach(file => {
        ...
        if (file.targetPath.startsWith("dynamic/")) {
          ...
          file._contents = Buffer.concat([MINIFY_RENAMED_FUNCTION, file.contents().slice(MINIFY_PLAIN_FUNCTION.length)]);
      ...
      function handle(source, dynamic) {
        source._minifiedFiles.forEach(file => {
          ...
          if (typeof file.data === 'string') {
            file.data = Buffer.from(file.data.replace(/^\s*function\s+__minifyJs\s*\(/, "function("), "utf8");
          } else if (dynamic) {
            file.data = Buffer.concat([MINIFY_PLAIN_FUNCTION, file.data.slice(MINIFY_RENAMED_FUNCTION.length)]);
          }
