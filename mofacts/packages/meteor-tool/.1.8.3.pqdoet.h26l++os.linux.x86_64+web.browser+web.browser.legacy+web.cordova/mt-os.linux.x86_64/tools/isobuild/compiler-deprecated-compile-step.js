!function (module1) {
  ...
  exports.makeCompileStep = function (sourceItem, file, inputSourceArch, options) {
    ...
    var hash = file.hash;
    var contents = file.contents; // This object is called a #CompileStep and it's the interface
    ...
    var compileStep = {
      ...
      // public API.
      _fullInputPath: files.convertToOSPath(absPath),
      // avoid, see above..
       ...
       * @instance
       */
      pathForSourceMap: files.convertToOSPath(inputSourceArch.pkg.name ? inputSourceArch.pkg.name + "/" + relPath : files.pathBasename(relPath), true),
       ...
       * @instance
       */
      rootOutputPath: files.convertToOSPath(inputSourceArch.pkg.serveRoot, true),
      ...
      addHtml: function (options) {
        if (!archinfo.matches(inputSourceArch.arch, "web")) {
          throw new Error("Document sections can only be emitted to " + "web targets");
        }
      ...
      addStylesheet: function (options) {
        if (!archinfo.matches(inputSourceArch.arch, "web")) {
          throw new Error("Stylesheets can only be emitted to " + "web targets");
        }
        ...
        resources.push({
          ...
          refreshable: true,
          data: Buffer.from(files.convertToStandardLineEndings(options.data), 'utf8'),
          servePath: colonConverter.convert(files.pathJoin(inputSourceArch.pkg.serveRoot, files.convertToStandardPath(options.path, true))),
          sourceMap: convertSourceMapPaths(options.sourceMap, files.convertToStandardPath),
          lazy: this._getOption("lazy", options)
        });
      },
      ...
      addJavaScript: function (options) {
        ...
        var data = Buffer.from(files.convertToStandardLineEndings(options.data), 'utf8');
        resources.push({
          type: "js",
          data: data,
          sourcePath,
          targetPath,
          servePath: colonConverter.convert(files.pathJoin(inputSourceArch.pkg.serveRoot, files.convertToStandardPath(targetPath, true))),
      ...
      error: function (options) {
        ...
        if (compileStep._getOption("lazy") === true) {
          ...
          resources.push({
            ...
            data: Buffer.from("throw new Error(" + JSON.stringify(message) + ");\n", "utf8"),
            lazy: true,
            error: {
              message,
              info
...
//# sourceMappingURL=compiler-deprecated-compile-step.js.map