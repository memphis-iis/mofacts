!function (module1) {
  ...
  var _ = require("underscore"); // This class encapsulates a structured specification of files and
  ...
  var NDp = NpmDiscards.prototype; // Update the current specification of discarded files with additional
  ...
  function merge(into, from) {
    _.each(from, function (fromValue, packageName) {
      ...
      if (_.isString(fromValue) || _.isRegExp(fromValue)) {
        if (intoValue) {
  ...
  NDp.shouldDiscard = function shouldDiscard(candidatePath, isDirectory) {
    ...
    for (var currentPath = candidatePath, parentPath; (parentPath = files.pathDirname(currentPath)) !== currentPath; currentPath = parentPath) {
