...
module.link("./protect-string-proto.js");
...
let superspawn;
module.link("cordova-common/src/superspawn.js", {
  default(v) {
    superspawn = v;
  }
...
class CordovaProject {
  ...
  prepareForPlatform(platform) {
    ...
    const commandOptions = _objectSpread({}, this.defaultOptions, {
      platforms: [platform]
    });
  ...
  run(platform, isDevice) {
    return Promise.asyncApply(() => {
      ...
      let command = files.convertToOSPath(files.pathJoin(this.projectRoot, 'platforms', platform, 'cordova', 'run'));
      this.runCommands("running Cordova app for platform ".concat(displayNameForPlatform(platform), " with options ").concat(options), execFileAsync(command, options, {
        env: env,
...
on an OS X system.");
  ...
  removePlugins(plugins) {
    ...
    this.runCommands("removing plugins ".concat(plugins, " from Cordova project"), cordova_lib.plugin.bind(undefined, 'rm', plugins, this.defaultOptions));
