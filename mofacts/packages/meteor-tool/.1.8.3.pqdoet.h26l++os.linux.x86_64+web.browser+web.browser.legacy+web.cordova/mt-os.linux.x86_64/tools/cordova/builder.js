...
class CordovaBuilder {
  ...
  writeConfigXmlAndCopyResources() {
    ...
    const {
      XmlBuilder
    } = loadIsopackage('xmlbuilder');
    let config = XmlBuilder.create('widget'); // Set the root attributes
    ...
    _.each(this.accessRules, (options, pattern) => {
      ...
      if (type === 'intent') {
        config.element('allow-intent', {
          href: pattern
        });
      } else if (type === 'navigation') {
        config.element('allow-navigation', _.extend({
          href: pattern
        }, options));
      } else {
        config.element('access', _.extend({
          origin: pattern
        }, options));
      }
    });
    ...
    const platformElement = {
      ios: config.element('platform', {
        name: 'ios'
      }),
      android: config.element('platform', {
        name: 'android'
      })
  ...
  configureAndCopyResourceFiles(resourceFiles, iosElement, androidElement) {
    _.each(resourceFiles, resourceFile => {
      ...
      files.copyFile(files.pathResolve(this.projectContext.projectDir, resourceFile.src), files.pathJoin(this.projectRoot, filename)); // And entry in config.xml
