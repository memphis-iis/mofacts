...
var Release = function (options) {
  var self = this; // If an actual, proper, "released" release, the name of the
...
_.extend(Release.prototype, {
  ...
  getDisplayName: function (options) {
    ...
    return catalogUtils.displayRelease(self.getReleaseTrack(), self.getReleaseVersion(), options);
  }
...
release.current = null; // True if we are using release.current because we were forced to do that by the
...
release.forced = null; // True if the release was explicitly specified by the user with the --release
// flag. Unlike release.forced, this is false when the release is overridden via
// SpringboardToLatestRelease.

release.explicit = null; // True if release.current is the release we'd use if we wanted to run the app
