let isIsobuildFeaturePackage;
module.link("../isobuild/compiler.js", {
  isIsobuildFeaturePackage(v) {
    isIsobuildFeaturePackage = v;
  }
...
exports.PackageMap = function (versions, options) {
  ...
  _.each(versions, function (version, packageName) {
    ...
    var packageSource = self._localCatalog && self._localCatalog.getPackageSource(packageName);
...
exports.PackageMap.fromReleaseVersion = function (releaseVersion) {
  var toolPackageVersion = releaseVersion.tool && utils.parsePackageAndVersion(releaseVersion.tool);
  ...
  versionMap[toolPackage] = toolVersion; // As described in this function's description, all packages in this map are
...
exports.PackageMapDelta = function (options) {
  ...
  options.packageMap.eachPackage(function (packageName, info) {
    var oldVersion = _.has(options.cachedVersions, packageName) ? options.cachedVersions[packageName] : null;

    self._storeAddOrChange(packageName, info, oldVersion, options.anticipatedPrereleases, options.neededToUseUnanticipatedPrereleases);
  });
...
_.extend(exports.PackageMapDelta.prototype, {
  _storeAddOrChange: function (packageName, newInfo, oldVersion, anticipatedPrereleases, neededToUseUnanticipatedPrereleases) {
    var self = this; // Store nothing if nothing has changed.

    if (newInfo.version === oldVersion) {
      return;
    }

    var backwardsIncompatible = oldVersion !== null && packageVersionParser.majorVersion(newInfo.version) !== packageVersionParser.majorVersion(oldVersion);
    var isPrerelease = /-/.test(newInfo.version);

    var isAnticipatedPrerelease = _.has(anticipatedPrereleases, packageName) && _.has(anticipatedPrereleases[packageName], newInfo.version);

    self._changedPackages[packageName] = {
      oldVersion: oldVersion,
      newVersion: newInfo.version,
      isBackwardsIncompatible: backwardsIncompatible,
      isUnanticipatedPrerelease: neededToUseUnanticipatedPrereleases && isPrerelease && !isAnticipatedPrerelease
    };
  },
  ...
  displayOnConsole: function (options) {
    ...
    }, options); // Print nothing at all if nothing changed.
    ...
    self.eachChangedPackage(function (packageName, info) {
      ...
      } else if (packageVersionParser.lessThan(info.oldVersion, info.newVersion)) {
        description = "upgraded from " + info.oldVersion + " to " + info.newVersion;
      } else {
        description = "downgraded from " + info.oldVersion + " to " + info.newVersion;
      }
...
//# sourceMappingURL=package-map.js.map