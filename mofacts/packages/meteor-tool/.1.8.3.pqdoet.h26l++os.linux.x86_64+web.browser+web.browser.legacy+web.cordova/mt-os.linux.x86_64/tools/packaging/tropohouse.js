...
var defaultWarehouseDir = function () {
  ...
  var warehouseBase = files.inCheckout() ? files.getCurrentToolsDir() : files.getHomeDir(); // XXX This will be `.meteor` soon, once we've written the code to make the
...
exports._extractAndConvert = function (packageTarball, forceConvert) {
  ...
  if (process.platform === "win32" || forceConvert) {
    ...
    var {
      metadata,
      originalVersion
    } = Isopack.readMetadataFromDirectory(targetDirectory); // By the time that isopack-2 came out (around Meteor 1.2) nobody should be
    ...
    if (originalVersion === 'unipackage-pre2' || originalVersion === 'isopack-1') {
      var convertedMetadata = colonConverter.convertIsopack(metadata); // Step 2. Write the isopack.json file.  Keep it as isopack-1;
      ...
      if (files.exists(isopackJsonPath)) {
        files.chmod(isopackJsonPath, 511);
      }

      files.writeFile(isopackJsonPath, Buffer.from(JSON.stringify(isopackFileData, null, 2), 'utf8'), {
        mode: 292
      }); // Step 3. Clean up old unipackage.json file if it exists

      files.unlink(files.pathJoin(targetDirectory, "unipackage.json")); // Result: Now we are in a state where the isopack.json file paths are
      ...
      _.each(convertedMetadata.builds, function (unibuildMeta) {
        ...
        if (unibuildJson.format !== "unipackage-unibuild-pre1") {
          throw new Error("Unsupported isopack unibuild format: " + JSON.stringify(unibuildJson.format));
        }

        var convertedUnibuild = colonConverter.convertUnibuild(unibuildJson);
        files.chmod(unibuildJsonPath, 511);
        files.writeFile(unibuildJsonPath, Buffer.from(JSON.stringify(convertedUnibuild, null, 2), 'utf8'), {
          mode: 292
      ...
      _.each(convertedMetadata.plugins, function (pluginMeta) {
        ...
        if (programJson.format !== "javascript-image-pre1") {
          throw new Error("Unsupported plugin format: " + JSON.stringify(programJson.format));
        }

        var convertedPlugin = colonConverter.convertJSImage(programJson);
        files.chmod(programJsonPath, 511);
        files.writeFile(programJsonPath, Buffer.from(JSON.stringify(convertedPlugin, null, 2), 'utf8'), {
          mode: 292
...
_.extend(exports.Tropohouse.prototype, {
  ...
  packagePath: function (packageName, version, relative) {
    ...
    var relativePath = files.pathJoin(config.getPackagesDirectoryName(), colonConverter.convert(packageName), version);
  ...
  wipeAllPackages: function () {
    ...
    var currentToolVersion = null; // Warning: we can't examine release.current here, because we might be
    ...
    if (!files.inCheckout()) {
      ...
      var toolsDir = files.getCurrentToolsDir(); // eg, 'meteor-tool'

      currentToolPackageEscaped = files.pathBasename(files.pathDirname(files.pathDirname(toolsDir))); // eg, '.1.0.17-xyz1.2.ut200e++os.osx.x86_64+web.browser+web.cordova' on Unix
      ...
      if (latestMeteorSymlink.startsWith(packagesDirectoryName + files.pathSep)) {
        var rest = latestMeteorSymlink.substr(packagesDirectoryName.length + files.pathSep.length);
    ...
    _.each(escapedPackages, function (packageEscaped) {
      ...
      _.each(versions, function (version) {
        ...
        if (packageEscaped === latestToolPackageEscaped && (version === latestToolVersion || version.startsWith('.' + latestToolVersion + '.'))) {
          return;
        } // Skip the currently-executing tool (ie, don't break the current
        // operation).


        if (packageEscaped === currentToolPackageEscaped && (version === currentToolVersion || version.startsWith('.' + currentToolVersion + '.'))) {
          return;
        }
  ...
  _alreadyDownloaded: function (options) {
    ...
    var downloadedArches = []; // Find out which arches we have by reading the isopack metadata

    var {
      metadata: packageMetadata
    } = Isopack.readMetadataFromDirectory(packagePath); // packageMetadata is null if there is no package at packagePath
  ...
  _saveIsopack: function (isopack, packageName) {
    ...
    if (self.platform === "win32") {
      ...
      var newPackageLinkTarget = '.' + isopack.version + '.' + utils.randomToken() + '++' + isopack.buildArchitectures();
      var combinedDirectory = self.packagePath(packageName, newPackageLinkTarget);
      isopack.saveToPath(combinedDirectory, {
        includePreCompilerPluginIsopackVersions: true
      });
      files.symlinkOverSync(newPackageLinkTarget, self.packagePath(packageName, isopack.version));
    }
  },
  ...
  _makeDownloader: function (options) {
    ...
    var packageName = options.packageName;
    var version = options.version; // Look up which arches we have already downloaded
    ...
    var buildsToDownload = catalog.official.getBuildsForArches(packageName, version, archesToDownload);

    if (!buildsToDownload) {
      buildmessage.error("No compatible binary build found for this package. " + "Contact the package author and ask them to publish it " + "for your platform.", {
        tags: {
          refreshCouldHelp: true
        }
      });
      return null;
    }
    ...
    var download = function download() {
      ...
      Console.debug("Downloading missing local versions of package", packageName + "@" + version, ":", archesToDownload);
      buildmessage.enterJob({
        title: "downloading " + packageName + "@" + version + "..."
      }, function () {
        var buildInputDirs = [];
        var buildTempDirs = [];
        var packageLinkTarget = null; // Find the previous actual directory of the package
        ...
        if (self.platform === "win32") {
          ...
          if (packageLinkTarget) {
            buildInputDirs.push(files.pathResolve(files.pathDirname(packagePath), packageLinkTarget));
          }
  ...
  downloadPackagesMissingFromMap: function (packageMap, options) {
    ...
    packageMap.eachPackage(function (packageName, info) {
      ...
      buildmessage.enterJob("checking for " + packageName + "@" + info.version, function () {
    ...
    if (downloaders.length === 1) {
      ...
      buildmessage.enterJob("downloading " + downloader.packageName + "@" + downloader.version, function () {
        downloader.download();
      });
      return;
...
//# sourceMappingURL=tropohouse.js.map