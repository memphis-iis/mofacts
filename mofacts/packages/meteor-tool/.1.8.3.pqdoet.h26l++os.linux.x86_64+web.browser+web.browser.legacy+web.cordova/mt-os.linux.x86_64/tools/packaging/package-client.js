!function (module1) {
  module1.export({
    ...
    createAndPublishBuiltPackage: () => createAndPublishBuiltPackage
  });
  let requestGarbageCollection;
  module1.link("../utils/gc.js", {
  ...
  var loadRemotePackageData = function (conn, syncToken, options) {
    options = options || {}; // Did we get disconnected between retries somehow? Then we should open a new
  ...
  var _updateServerPackageData = function (dataStore, options) {
    ...
    var start = undefined; // Guess that we're about an hour behind, as an opening guess
    ...
    var conn = openPackageServerConnection(options.packageServerUrl); // Provide some progress indication for connection
    ...
    var getSomeData = function () {
      ...
      useProgressbar && buildmessage.reportProgress(state);
      var compress = !!process.env.METEOR_CATALOG_COMPRESS_RPCS; // (loadRemotePackageData may throw)
      ...
      if (remoteData.resetData) {
        dataStore.reset(); // The caller may want to take this as a cue to delete packages from the
        // tropohouse.
      ...
      if (remoteData.collectionsCompressed) {
        ...
        var colsGzippedBuffer = Buffer.from(remoteData.collectionsCompressed, 'base64');
      ...
      dataStore.insertData(remoteData, syncComplete); // If there is no new data from the server, don't bother writing things to
  ...
  _updateServerPackageData = Profile('package-client _updateServerPackageData', _updateServerPackageData); // Returns a logged-in DDP connection to the package server, or null if
  ...
  exports.loggedInPackagesConnection = function () {
    return authClient.loggedInConnection(config.getPackageServerUrl(), config.getPackageServerDomain(), "package-server");
  ...
  var bundleSource = function (isopack, includeSources, packageDir) {
    ...
    var dirToTar = files.pathJoin(tempDir, 'source', colonConverter.convert(packageTarName)); // XXX name probably needs to be escaped for windows?
    ...
    if (!files.mkdir_p(sourcePackageDir)) {
      buildmessage.error('Failed to create temporary source directory: ' + sourcePackageDir);
      return null;
    ...
    packageMapFile.write(pluginProviderPackageMap); // We put this inside the temp dir because mkdtemp makes sure that the
  ...
  function bundleBuild(isopack, isopackCache) {
    buildmessage.assertInJob();
    var tempDir = files.mkdtemp('bp-');
    var packageTarName = isopack.tarballName();
    var tarInputDir = files.pathJoin(tempDir, packageTarName); // Note that we do need to do this even though we already have the isopack on
    ...
    var treeHash = files.treeHash(tarInputDir, {
      ...
      ignore: function (relativePath) {
        ...
        return pieces.length && _.last(pieces) === 'package.json' && _.contains(pieces, 'npm');
      }
    });
    return {
  ...
  function createBuiltPackage(isopack, isopackCache) {
    buildmessage.assertInJob();
    var name = isopack.name; // Note: we really want to do this before createPackageBuild, because the URL
  ...
  var publishBuiltPackage = function (conn, isopack, bundleResult) {
    ...
    buildmessage.enterJob("uploading build", function () {
      uploadFile(uploadInfo.uploadUrl, bundleResult.buildTarball);
    });
    ...
    buildmessage.enterJob('publishing package build for ' + name, function () {
      callPackageServerBM(conn, 'publishPackageBuild', uploadInfo.uploadToken, bundleResult.tarballHash, bundleResult.treeHash);
    });
  ...
  function createAndPublishBuiltPackage(conn, isopack, isopackCache) {
    publishBuiltPackage(conn, isopack, createBuiltPackage(isopack, isopackCache));
  }
  ...
  exports.updatePackageMetadata = function (options) {
    ...
    if (!version) {
      buildmessage.error("Package cannot be updated because it doesn't have a version");
      return;
    ...
    if (dataToUpdate["description"] && dataToUpdate["description"].length > 100) {
      buildmessage.error("Summary must be under 100 chars.");
      return;
    }

    if (dataToUpdate["longDescription"].length > 1500) {
      buildmessage.error("Longform package description is too long. Meteor uses the section of " + "the Markdown documentation file between the first and second " + "headings. That section must be less than 1500 characters long.");
      return;
    ...
    buildmessage.enterJob('updating metadata', function () {
      callPackageServerBM(conn, "changeVersionMetadata", versionIdentifier, dataToUpdate);
    });
    ...
    buildmessage.enterJob('uploading documentation', function () {
      ...
      var uploadInfo = callPackageServerBM(conn, "createReadme", versionIdentifier);
      ...
      callPackageServerBM(conn, "publishReadme", uploadInfo.uploadToken, {
        hash: readmeInfo.hash
      });
    });
  ...
  exports.publishPackage = function (options) {
    ...
    if (!version) {
      buildmessage.error("Package cannot be published because it doesn't have a version");
      return;
    ...
    if (!packageSource.metadata.summary) {
      buildmessage.error("Please describe what your package does. Set a summary " + "in Package.describe in package.js.");
      return;
    }
    ...
    if (!options['new']) {
      ...
      if (!packRecord) {
        buildmessage.error('There is no package named ' + name + '. If you are creating a new package, use the --create flag.');
        return;
      }

      if (!exports.amIAuthorized(name, conn, false)) {
        buildmessage.error('You are not an authorized maintainer of ' + name + '.  Only ' + 'authorized maintainers may publish new versions.');
      }
    } // Check that our documentation exists (or we know that it doesn't) and has
    // been filled out.


    var readmeInfo = buildmessage.enterJob("processing documentation", function () {
    ...
    if (readmeInfo && readmeInfo.hash === files.blankHash) {
      buildmessage.error("Your documentation file is blank, so users may have trouble figuring " + "out how to use your package. Please fill it out, or " + "set 'documentation: null' in your Package.describe");
      return;
    }
    ...
    var readmePath = saveReadmeToTmp(readmeInfo);
    var packageDeps = packageSource.getDependencyMetadata(); // Check that the package does not have any unconstrained references.

    _.each(packageDeps, function (refs, label) {
      if (refs.constraint == null) {
        if (packageSource.isCore && files.inCheckout() && projectContext.localCatalog.getPackage(label)) {
          ...
          var versionString = projectContext.localCatalog.getLatestVersion(label).version; // modify the constraint on this dep that will be sent to troposphere

          refs.constraint = versionString;
        } else if (label === "meteor") {// HACK: We are willing to publish a package with a "null"
          ...
          buildmessage.error("You must specify a version constraint for package " + label);
        }
      }
    });
    ...
    if (!isopack.canWriteLegacyBuilds() && !_.has(packageDeps, 'isobuild:isopack-2')) {
    ...
    var sourceFiles = isopack.getSourceFilesUnderSourceRoot(packageSource.sourceRoot);
    ...
    if (packageSource.testName) {
      var testIsopack = projectContext.isopackCache.getIsopack(packageSource.testName);

      if (!testIsopack) {
        throw Error("no testIsopack " + packageSource.testName);
      }

      var testSourceFiles = testIsopack.getSourceFilesUnderSourceRoot(packageSource.sourceRoot);
    ...
    buildmessage.enterJob("bundling source for " + name, function () {
      sourceBundleResult = bundleSource(isopack, sourceFiles, packageSource.sourceRoot);
    });
    ...
    if (options.existingVersion) {
      ...
      if (!options.doNotPublishBuild) {
        createAndPublishBuiltPackage(conn, isopack, projectContext.isopackCache);
      ...
      if (!options.doNotPublishBuild) {
        var bundleResult = createBuiltPackage(isopack, projectContext.isopackCache);
      ...
      buildmessage.enterJob("publishing package version", function () {
        callPackageServerBM(conn, 'publishPackageVersion', uploadInfo.uploadToken, hashes);
      });
  ...
  exports.amIAuthorized = function (name, conn, isRelease) {
    var methodName = "amIAuthorized" + (isRelease ? "Release" : "Package");
...
//# sourceMappingURL=package-client.js.map