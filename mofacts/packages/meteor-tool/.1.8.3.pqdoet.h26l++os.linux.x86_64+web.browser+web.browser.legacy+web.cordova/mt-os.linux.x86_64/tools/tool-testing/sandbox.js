...
let PhantomClient;
module.link("./clients/phantom/index.js", {
  default(v) {
    PhantomClient = v;
  }
...
let PuppeteerClient;
module.link("./clients/puppeteer/index.js", {
  default(v) {
    PuppeteerClient = v;
  }
...
module.link("./clients/browserstack/index.js", {
  default(v) {
...
let RemoteCatalog;
module.link("../packaging/catalog/catalog-remote.js", {
  RemoteCatalog(v) {
    RemoteCatalog = v;
  }
...
let IsopackCache;
module.link("../isobuild/isopack-cache.js", {
  IsopackCache(v) {
    IsopackCache = v;
  }
...
let PackageMap;
module.link("../packaging/package-map.js", {
  PackageMap(v) {
    PackageMap = v;
  }
...
class Sandbox {
  ...
  testWithAllClients(f) {
    for (var _len2 = arguments.length, args = new Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
      args[_key2 - 1] = arguments[_key2];
    }

    args = args.filter(arg => arg); // Lazy-populate the clients, only when this method is called.
    ...
    console.log("Running test with ".concat(this.clients.length, " client(s)..."));
    Object.keys(this.clients).forEach((clientKey, index, array) => {
      const client = this.clients[clientKey];
      console.log("(".concat(index + 1, "/").concat(array.length, ") Testing with ").concat(client.name, "..."));
...
'meteor-base', 'mobile-experience', 'mongo', 'blaze-html-templates', "jquery", // necessary when using Blaze
