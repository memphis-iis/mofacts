...
exports.AlreadyPrintedMessageError = function () {}; // Opens a DDP connection to a package server. Loads the packages needed for a
...
exports.openServiceConnection = function (serverUrl) {
  return new ServiceConnection(serverUrl, {
    headers: {
...
exports.handleConnectionError = function (error, label) {
  if (error instanceof exports.AlreadyPrintedMessageError) {// do nothing
...
exports.loggedInConnection = function (url, domain, sessionType) {
  ...
  // do an OAuth flow.
  if (auth.maybePrintRegistrationLink({
    onlyAllowIfRegistered: true
  })) {
