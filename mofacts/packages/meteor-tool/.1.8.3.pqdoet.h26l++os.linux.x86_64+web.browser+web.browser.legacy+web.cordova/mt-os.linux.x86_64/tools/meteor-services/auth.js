...
function loadDDP() {
  return require("../tool-env/isopackets.js").loadIsopackage("ddp-client").DDP;
...
var loggedInAccountsConnection = function (token) {
  var connection = loadDDP().connect(config.getAuthDDPUrl());
  return new Promise(function (resolve, reject) {
    connection.apply('login', [{
      resume: token
    }], {
      wait: true
    }, function (err) {
...
var readSessionData = function () {
  ...
  return JSON.parse(files.readFile(sessionPath, {
    encoding: 'utf8'
  }));
};
...
var writeSessionData = function (data) {
  ...
  while (true) {
    ...
    var tempPath = files.pathJoin(files.pathDirname(sessionPath), '.meteorsession.' + Math.floor(Math.random() * 999999));

    try {
      var fd = files.open(tempPath, 'wx', 384);
    } catch (e) {
      continue;
    }
...
var tryRevokeOldTokens = function (options) {
  ...
  _.each(readSessionData().sessions || {}, function (session, domain) {
    if (session.pendingRevoke && session.pendingRevoke.length) {
  ...
  var logoutFailWarning = function (domain) {
    if (!warned) {
      ...
      Console.error("warning: " + (options.firstTry ? "couldn't" : "still trying to") + " confirm logout with " + domain);
      warned = true;
    }
  };
...
var sendAuthorizeRequest = function (clientId, redirectUri, state) {
  var authCodeUrl = config.getOauthUrl() + "/authorize?" + querystring.stringify({
    state: state,
    response_type: "code",
    client_id: clientId,
  ...
  if (url.parse(response.headers.location).hostname !== url.parse(redirectUri).hostname) {
...
var oauthFlow = function (conn, options) {
  ...
  var authorizeResult = sendAuthorizeRequest(options.clientId, options.redirectUri, credentialToken); // XXX We're using a test-only flag here to just get the raw
  ...
  var response = redirectResult.response; // 'access-denied' isn't exactly right because it's possible that the server
...
exports.loginCommand = withAccountsConnection(function (options, connection) {
  var data = readSessionData();

  if (!getSession(data, config.getAccountsDomain()).token || options.overwriteExistingToken) {
    var loginOptions = {};
  ...
  Console.error();
  Console.error("Logged in" + (currentUsername(data) ? " as " + currentUsername(data) : "") + ". Thanks for being a Meteor developer!");
  return 0;
});
...
exports.pollForRegistrationCompletion = function (options) {
  ...
  new Promise(function (resolve) {
    ...
    timer = setTimeout(function () {
      resolve(null);
    }, 5000); // Intentionally calling bindEnvironment on the .then callback rather
...
exports.whoAmICommand = function (options) {
  ...
  if (!loggedIn(data)) {
    Console.error("Not logged in. " + Console.command("'meteor login'") + " to log in.");
    return 1;
  }
...
exports.registerOrLogIn = withAccountsConnection(function (connection) {
  ...
  while (true) {
    ...
    try {
      var methodCaller = sessionMethodCaller('tryRegister', {
        connection: connection
      });
  ...
  if (!result.alreadyExisted) {
    ...
    Console.error();
    Console.error("You need to pick a password for your account so that you can log in.", "An email has been sent to you with the link.");
    Console.error();
    var animationFrame = 0;
    var lastLinePrinted = "";
    var timer = setInterval(function () {
      var spinner = ['-', '\\', '|', '/'];
      lastLinePrinted = "Waiting for you to register on the web... " + spinner[animationFrame];
      Console.rawError(lastLinePrinted + Console.CARRIAGE_RETURN);
      animationFrame = (animationFrame + 1) % spinner.length;
    }, 200);

    var stopSpinner = function () {
      Console.rawError(new Array(lastLinePrinted.length + 1).join(' ') + Console.CARRIAGE_RETURN);
      clearInterval(timer);
    };

    try {
      var waitForRegistrationResult = connection.call('waitForRegistration', email);
    } catch (e) {
      stopSpinner();
      ...
      Console.error("When you've picked your password, run " + Console.command("'meteor login'") + " to log in.");
      return false;
    }
    ...
    Console.error("\nThere was a problem. Please log in with " + Console.command("'meteor login'") + ".");
    return false;
  }
...
exports.maybePrintRegistrationLink = function (options) {
  ...
  if (session.userId && !session.username && session.registrationUrl) {
    ...
    if (options.onlyAllowIfRegistered) {
      ...
      // to do!
      Console.error("You need to claim a username and set a password on your Meteor", "developer account to run this command. It takes about a minute at:", session.registrationUrl);
      Console.error();
      ...
      // registration url.
      Console.error("You should set a password on your Meteor developer account.", "It takes about a minute at:", session.registrationUrl);
      Console.error();
    } else {
      Console.error("You can set a password on your account or change your email", "address at:", session.registrationUrl);
      Console.error();
    }
...
exports.getAccountsConfiguration = function (conn) {
  ...
  var accountsConfiguration = null; // We avoid the overhead of creating a 'ddp-and-mongo' isopacket (or
  ...
  conn.connection.registerStore('meteor_accounts_loginServiceConfiguration', {
    update: function (msg) {
      if (msg.msg === 'added' && msg.fields && msg.fields.service === 'meteor-developer') {
  ...
  var serviceConfigurationsSub = conn.subscribeAndWait('meteor.loginServiceConfiguration');
...
exports.loginWithTokenOrOAuth = function (conn, accountsConfiguration, url, domain, sessionType) {
  ...
  var loginResult; // Try to log in with an existing login token, if we have one.
  ...
  var redirectUri = url + '/_oauth/meteor-developer'; // Duplicate code from packages/oauth/oauth_common.js. In Meteor 0.9.1, we
